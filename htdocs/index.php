...
if ($ibforums->input['showtopic'])
{
	...
	// the skins...
	$cmd = Ibf::instance()->db->prepare("SELECT
			t.*,
			f.topic_mm_id,
		...
		AND f.id=t.forum_id
		AND f.category=c.id");
	$cmd->execute([':topic' => $ibforums->input['t']]);
	$ibforums->topic_cache = $cmd->fetch();
...
} else if ($ibforums->input['showuser'])
{
	...
	$ibforums->input['act'] = $ibforums->input['act'] == ''
		? "idx"
		: $ibforums->input['act'];
...
if (!empty($colcat))
	$std->set_board_visibility($colcat, false, false); else
if (!empty($expcat))
	$std->set_board_visibility($expcat, false, true);


if (!empty($colfor))
	$std->set_board_visibility($colfor, true, false); else
if (!empty($expfor))

...
//  }
  ...
  $this->html = $std->load_template('skin_quiz');
...
//			$ibforums->lang['takequiz_nav']
  ...
  $DB->query("SELECT id,item_name,stock
	    FROM ibf_store_shopstock
	    WHERE stock > 0
	    ORDER BY item_name DESC");
  $items[] = array('none','No Items for Prize');

  if($DB->get_num_rows() >= 16)
  { 
    $settings['items_rows'] = 16;
  } else $settings['items_rows'] = $DB->get_num_rows();
...
//form_multiselect( "quiz_items[]", $items, $settings['items_won'],$rows);										    
  ...
  $this->output .= $this->html->quiz_edit_header($settings);
...
//  $ibforums->input['quiz_desc'] = str_replace("<br>","\n",$ibforums->input['quiz_desc']);
  ...
  foreach($ibforums->input as $field => $info)
  {
    ...
    if(!preg_match("#mid_(.+?)_type#",$field,$match)) continue;
...
//    $question = str_replace("&lt;br&gt;","<br>",$question);
	...
	)
    {
      ...
      if(!$ibforums->input['q_'.$mid.'_1_correct']) $ibforums->input['q_'.$mid.'_1_correct'] = 0;
      if(!$ibforums->input['q_'.$mid.'_2_correct']) $ibforums->input['q_'.$mid.'_2_correct'] = 0;
      if(!$ibforums->input['q_'.$mid.'_3_correct']) $ibforums->input['q_'.$mid.'_3_correct'] = 0;
      if(!$ibforums->input['q_'.$mid.'_4_correct']) $ibforums->input['q_'.$mid.'_4_correct'] = 0;
      if(!$ibforums->input['q_'.$mid.'_5_correct']) $ibforums->input['q_'.$mid.'_5_correct'] = 0;
      if(!$ibforums->input['q_'.$mid.'_6_correct']) $ibforums->input['q_'.$mid.'_6_correct'] = 0;
      if(!$ibforums->input['q_'.$mid.'_7_correct']) $ibforums->input['q_'.$mid.'_7_correct'] = 0;
      if(!$ibforums->input['q_'.$mid.'_8_correct']) $ibforums->input['q_'.$mid.'_8_correct'] = 0;
      if(!$ibforums->input['q_'.$mid.'_9_correct']) $ibforums->input['q_'.$mid.'_9_correct'] = 0;

      $answer =   "{answer1_".$ibforums->input['q_'.$mid.'_1_correct'].":".$ibforums->input['q_'.$mid.'_answer_1'].'}||'.
			"{answer2_".$ibforums->input['q_'.$mid.'_2_correct'].":".$ibforums->input['q_'.$mid.'_answer_2'].'}||'.
			"{answer3_".$ibforums->input['q_'.$mid.'_3_correct'].":".$ibforums->input['q_'.$mid.'_answer_3'].'}||'.
			"{answer4_".$ibforums->input['q_'.$mid.'_4_correct'].":".$ibforums->input['q_'.$mid.'_answer_4'].'}||'.
			"{answer5_".$ibforums->input['q_'.$mid.'_5_correct'].":".$ibforums->input['q_'.$mid.'_answer_5'].'}||'.
			"{answer6_".$ibforums->input['q_'.$mid.'_6_correct'].":".$ibforums->input['q_'.$mid.'_answer_6'].'}||'.
			"{answer7_".$ibforums->input['q_'.$mid.'_7_correct'].":".$ibforums->input['q_'.$mid.'_answer_7'].'}||'.
			"{answer8_".$ibforums->input['q_'.$mid.'_8_correct'].":".$ibforums->input['q_'.$mid.'_answer_8'].'}||'.
			"{answer9_".$ibforums->input['q_'.$mid.'_9_correct'].":".$ibforums->input['q_'.$mid.'_answer_9'].'}';				

    }
    else if($info == 'opinion')
    {
      $answer = "";
    }
...
//			$ibforums->lang['takequiz_nav']
  ...
  $this->output .= $this->html->quiz_question_header($settings);
  ...
  while($quiz = $DB->fetch_row())
  {
    ...
    $this->output .= $this->html->edit_question($n,$quiz);
    $n++;
  }
...
//    foreach($q as $k=>$v) echo $k."=".$v."<br>";
    ...
    if($q['type'] != 'opinion') $quiz['real_questions']++;
  }
  ...
  $this->output .= $this->html->quiz_u_a_header($quiz,$member);
...
//echo $answers[$q['mid']]."<br>";
    ...
    $this->output .= $this->html->user_answer($q);
...
function get_quiz_questions($qid=0)
{
  ...
  if($DB->get_num_rows() <= 0) return $ans; 
...
//    $row['post'] = str_replace("\n","<br>",$row['post']);
    ...
    $this->output .= $this->html->quiz_results_header($qid,$row['quizname']);
...
*/
  ...
  function show_quiz()
  {
    ...
    if(!$qid) $this->error("cannot_find_quiz");
    ...
    if($DB->get_num_rows() == 0 ) $this->error("cannot_find_quiz");
...
//    $row['amount_won'] = $std->do_number_format($row['amount_won']);
    ...
    $this->output .= $this->html->quiz_show($row,$member);
...
*/
    ...
    $this->output .=	$this->html->quiz_header(array(
...
*/
      ...
      {
        if(!$quiz['answer'] || !$quiz['question']) continue;
    	$quiz['answer'] = stripslashes($quiz['answer']);
        $n++;
//echo $n.". ".$quiz['answer']."<br>\n";
    	$this->output .= $this->html->single_question($n,$quiz);
//echo "single.<br>\n";
      }
...
//echo "dropdown.<br>\n";
        ...
        {
//echo $n.". ".$answer."<br>\n";
          if(!preg_match("#{answer([1-9])_.+?:(.+)}#is",$answer,$match)) continue;
...
//    	$quiz['question'] = stripslashes($quiz['question']);
        $n++;
    	$this->output .= $this->html->dropdown_question($n,$quiz);
...
//    	$quiz['question'] = stripslashes($quiz['question']);
        $n++;
    	$this->output .= $this->html->opinion_question($n,$quiz);
...
//echo "???.<br>\n";
  ...
  function do_take_quiz()
  {
    ...
    {
      if($debug > 1) echo $field."=".$value."<br>\n";

      if(!preg_match("#uanswer_([^_]+)_*(\d+)*#is",$field,$match)) continue;
      if($match['2'])
      {
        $temp_answer[$match['1']][$match['2']] = $value;
        if($debug > 1) echo "temp_answer[".$match['1']."][".$match['2']."]=".$value."<br>\n";
      }
      else
      {
        $temp_answer[$match['1']] = $value;
      if($debug > 1) echo "temp_answer[".$match['1']."]=".$value."<br>\n";
      }
    }
    ...
    if($debug > 1) echo "serial=".$serial_answer."<br>\n";
    ...
    while($check = $DB->fetch_row())
    {
      ...
      if($debug > 1) echo "quiz_answer [{$check['type']}] {$question_num}:<br><b>".$check['answer']."</b><br>\n";
      ...
      {
        if($debug) echo "{$question_num}. $correct_answer [1]. ";
        if($debug) echo "user_answer {$question_num} = <b>{$user_answer}</b> ";

        if($this->tag_convert($user_answer) === $this->tag_convert($correct_answer))
        {
    	  $correct++;
          if($debug) echo "CORRECT.<br>\n";
    	}
        else if($debug) echo "WRONG.<br>\n";
		...
		)
      {
    	...
    	foreach($answers as $answer)
        {
          ...
          if($debug) echo "{$question_num}.{$match[1]}. {$match[3]} [{$match[2]}]";
...
//else echo "The user answer is wrong.<br>\n";
        ...
        if($debug) echo "correct_answers={$correct}<br>\n";
        ...
        */
    	...
    	foreach($answers as $answer)
        {
          ...
          if($match[2])       // This is a correct answer
          {
            ...
            {
              $user_score++;
              if($debug) echo " user_answer {$question_num} =<b>{$temp_answer[$question_num][$question_number]}</b> CORRECT";
              if($debug) echo " user_score= {$user_score}";
            }
          }
          else                // This is a non-correct answer
          {
            ...
            {
              $user_score--;
              if($debug) echo " user_answer {$question_num} =<b>{$temp_answer[$question_num][$question_number]}</b> WRONG";
              if($debug) echo " user_score= {$user_score}";
            }
          }
        ...
        if($debug) echo " quiz right_answers={$right_answers}<br>\n";
        if($debug) echo " user_score={$user_score}<br>\n";
        if($debug) echo " total correct_answers={$correct}<br>\n";
      ...
      {
        if($debug) echo "{$question_num}. Opinion:<br>\n";
//        if($debug) echo "<b>".addslashes($temp_answer[$question_num])."</b><br>\n";
        ...
        if($debug) echo "<b>".$temp_answer[$question_num]."</b><br>\n";
        if($debug) echo "SKIPPED.<br>\n";

      }

      if($check['type'] != 'opinion') $total_questions++;
    ...
    if($debug) echo "total_questions={$total_questions}<br>\n";
    if($debug) echo "correct_answers={$correct}<br>\n";
    if($debug) echo "correct_percent={$correct_percent}<br>\n";
...
// !!! vot: HAVE TO CHECK - If the item NOT sold!!!
        ...
        {
    	  if(!preg_match("#(.+)=(.+)#is",$items,$match)) continue;
    	  $DB->query("INSERT INTO ibf_store_inventory
...
//	$html .= "<br/><div align='center' class='copyright'>Powered by <a href=\"http://www.subzerofx.com/shop/\" target='_blank'>IBStore</a> {$this->store_version} &copy; 2003-04 &nbsp;<a href='http://www.subzerofx.com/' target='_blank'>SubZeroFX.</a></div><br>";
	...
	if ( ( !$ibforums->member['id'] or $ibforums->member['show_ratting'] ) and $member['show_ratting'] and !$member['use_sig'] ) {
		...
		{
		    $down = ( $ibforums->member['view_img'] ) ? "<{REP_MINUS}>" : "<span style='color:red'>-</span>";
		    $up  =  ( $ibforums->member['view_img'] ) ? "<{REP_ADD}>"   : "<span style='color:green'>+</span>";
	...
	if ( $ibforums->vars['warn_on'] and !stristr( $ibforums->vars['warn_protected'], ','.$member['mgroup'].',') )
	{
	    ...
	    {
		$down = ( $ibforums->member['view_img'] ) ? "<{WARN_MINUS}>" : "<span style='color:green'>-</span>";
...
{
  global $INFO, $DB, $ibforums, $MEMBER;
  ...
  $DB->query("INSERT INTO ibf_admin_logs
		({$str['FIELD_NAMES']})

 ...
 if(confirm(type)) {
  	...
  	";
        ...
        if ($ibforums->vars['showplaysleft']) {
            ...
            $this->output = str_replace(
                "<!--Plays Left Middle-->",
                View::make("quiz.plays_left_middle", ['plays' => $close_check['plays_left']]),
                $this->output
            );
        }
    ...
    function list_quiz()
    {
        ...
        while ($quiz = $stmt->fetch($list_quiz)) {
            ...
            if ($ibforums->vars['showplaysleft']) {
                ...
                $this->output = str_replace(
                    "<!--Plays Left Header-->",
                    View::make("quiz.plays_left_header"),
                    $this->output
                );
                $this->output = str_replace(
                    "<!--Plays Left Middle-->",
    ...
    function do_take_quiz()
    {
        ...
        while ($check = $stmt->fetch()) {
            ...
            elseif ($check['type'] == 'dropdown' || $check['type'] == 'multiq' || $check['type'] == 'radio'
            ) {

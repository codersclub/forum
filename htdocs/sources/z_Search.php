...
//    	if ( $ibforums->member['g_use_search'] != 1 ) $std->Error( array( 'LEVEL' => 1, 'MSG' => 'no_permission' ) );
	...
	$res_d = $DB->query("SELECT VERSION() AS version");
	
	if ( ! $row = $DB->fetch_row($res_d) )
	{
		$res_d = $DB->query("SHOW VARIABLES LIKE 'version'");
		$row = $DB->fetch_row($res_d);
	}
...
// Song * mine new messages
	...
	function show_simple_results()	{ $result = $this->lib->do_simple_search(); }
	...
	global $ibforums, $DB, $std, $HTTP_POST_VARS, $print;
		...
		if ( $ibforums->member['g_search_flood'] > 0 )
		{
			...
			$res_d = $DB->query("SELECT id FROM ibf_search_results WHERE (member_id='".$ibforums->member['id']."' OR ip_address='".$ibforums->input['IP_ADDRESS']."') AND search_date > '$flood_time'");
		...
		if ( !$forums ) $std->Error( array( 'LEVEL' => 1, 'MSG' => 'no_search_forum') );

		$mid    = intval($ibforums->input['mid']);
		
		if ( !$mid ) $std->Error( array( 'LEVEL' => 1, 'MSG' => 'no_search_results' ) );
		...
		{
			$DB->query("SELECT tid FROM ibf_topics WHERE forum_id IN($forums) AND approved=1 and starter_id=$mid");
	
			$max_hits = $DB->get_num_rows();
		
			while ( $row = $DB->fetch_row() ) $posts .= $row['tid'].",";
		}
		...
		if ( !$posts ) $std->Error( array( 'LEVEL' => 1, 'MSG' => 'no_search_results' ) );
 	...
 	function get_new_posts($mine = 0)
 	{
		...
		if ( !$ibforums->member['id'] )	$std->Error( array( 'LEVEL' => 1, 'MSG' => 'no_search_results' ) );
		...
		$ibforums->input['nav']    = ( $mine ) ? 'my_lv' : 'lv';
...
// Song * selected search
		...
		$DB->query("SELECT sf.fid,f.read_perms FROM ibf_search_forums sf, ibf_forums f WHERE 
		...
		if ( $ibforums->member['search_days'] == -1 ) $time = $ibforums->input['last_visit']; else
		{
			$time = time();
...
// Song * selected search
		...
		if ( $all ) $query .= " and t.forum_id IN (".$all."0)";
...
// Song * selected search
		...
		$max_hits = $DB->get_num_rows($res_d);
		
		$posts = "";

		while ( $topic = $DB->fetch_row($res_d) )
		{
			$last_time = '';
...
// Song * NEW
			if ( $ibforums->member['id'] ) 
			{
				if ( !isset($this->read_mark[ $fid ]) )
				{
					...
					$this->read_mark[ $fid ] = ($ibforums->member['board_read'] > $this->read_mark[ $fid ]) 
								 ? $ibforums->member['board_read'] : $this->read_mark[ $fid ];
		
					$this->read_mark[ $fid ] = ($this->read_mark[ $fid ] < (time() - 60*60*24*30 )) 
								? (time() - 60*60*24*30) : $this->read_mark[ $fid ];
				}

		 		if ( $this->read_array[ $tid ] ) $last_time = $this->read_array[ $tid ]; else
				{
					$last_time = -1;
			...
			if ( $last_time && ($topic['last_post'] > $last_time) ) $posts .= $tid.",";
// Song * NEW
		}

		$DB->free_result($res_d);
		...
		$res_d = $DB->query("INSERT INTO ibf_search_results ({$str['FIELD_NAMES']}) VALUES ({$str['FIELD_VALUES']})");
 	...
 	function get_last_ten()
 	{
		...
		$res_d = $DB->query("SELECT p.*,t.*,f.id as forum_id,f.name as forum_name 
...
// Song * club tool
		...
		} else $std->Error( array( 'LEVEL' => 1, 'MSG' => 'no_search_results' ) );

// Song * club tool

		if ( $count <= 0 ) $std->Error( array( 'LEVEL' => 1, 'MSG' => 'no_search_results' ) );
...
// Song * club tool
 	...
 	function get_replies()
 	{
		...
		$res_d = $DB->query("SELECT tid FROM ibf_topics WHERE starter_id='".$ibforums->member['id']."'
		...
		while ($row = $DB->fetch_row($res_d) ) $topics .= $row['tid'].",";
		...
		if ( !$topics ) $std->Error( array( 'LEVEL' => 1, 'MSG' => 'no_search_results' ) );
...
// Song * quick search from special tag
	...
	global $ibforums, $HTTP_POST_VARS, $DB;
		...
		if ( $fid )
		{
			$DB->query("SELECT parent_id FROM ibf_forums WHERE id='".$fid."'");
...
// Song * quick search from special tag
 	...
 	function show_form()
 	{
		...
		$res_d = $DB->query("SELECT f.id as forum_id, f.parent_id, f.subwrap, f.sub_can_post, f.name as forum_name, f.position, f.read_perms, c.id as cat_id, c.name as cat_name
		            FROM ibf_forums f
...
// Song * NEW
	...
	if ( $ibforums->member['id'] )
	{
		...
		$DB->query("SELECT tid,fid,logTime FROM ibf_log_topics WHERE 
...
// Song * NEW
	...
	function show_results()
	{
		...
		$res_d = $DB->query("DELETE FROM ibf_search_results WHERE search_date < '$t_time'");
		
		$this->unique_id = $ibforums->input['searchid'];
		
		if ( !$this->unique_id ) $std->Error( array( 'LEVEL' => 1, 'MSG' => 'no_search_results' ) );
		
		$res_d = $DB->query("SELECT * FROM ibf_search_results WHERE id='{$this->unique_id}'");
		...
		if ($topics == "," and $posts == ",") $std->Error( array( 'LEVEL' => 1, 'MSG' => 'no_search_results' ) );
...
// vot
				if ( $ibforums->input['new'] ) $query .= "f.id ASC,";
...
// Song * search new topics of user
			...
			} else
			{
				...
				if ( $posts != "," )
				{
					$res_d = $DB->query("SELECT forum_id,topic_id FROM ibf_posts WHERE pid IN(0{$posts}0) 
								and queued != 1");
...
// Song * club tool
		...
		} else 
		{
			...
			if ( $this->search_in == 'titles' )
			{
				...
				$res_d = $DB->query("SELECT t.*, p.pid, p.author_id, p.author_name, p.post_date, p.post, 
			...
			} else
			{
				...
				if ( $topics != "," )
				{
					$res_d = $DB->query("SELECT pid FROM ibf_posts WHERE topic_id IN(0{$topics}0) AND 
...
// Song * club tool
		...
		{
			$this->nav = array( "<a href='{$this->base_url}&act=Search2'>{$ibforums->lang['search_form']}</a>", $ibforums->lang['search_title'] );
		}
	}
...
// Song * select forums for "new" search
		...
		if ( $ibforums->input['new'] and $ibforums->member['id'] ) 
		{
			...
			$selected = ( $ibforums->member['search_days'] == -1) ? " selected='selected'" : "";

			$selector .= $this->html->search_days(-1, $ibforums->lang['search_last_visit'], $selected);

			for ($i = 1 ; $i < 15 ; ++$i ) 
			{
				$selected = ( $i == $ibforums->member['search_days'] ) ? " selected='selected'" : "";
...
// Song * select forums for "new" search
		...
		$out = array( 'SHOW_PAGES' => $this->links,
			      'BUTTON'	   => $button,
	...
	global $ibforums, $DB, $std, $HTTP_POST_VARS;
		...
		else 
		{
			...
			$res_d = $DB->query("SELECT * FROM ibf_search_results WHERE id='$unique_id'");
			$sr = $DB->fetch_row($res_d);
		...
		$res_d = $DB->query("SELECT t.*, f.id as forum_id, f.name as forum_name
		             FROM ibf_topics t, ibf_forums f
...
// Song * club tool
	...
	function parse_entry($topic, $view_as_post=0) {
		...
		if ($topic['poll_state']) $topic['prefix']     = $ibforums->vars['pre_polls'].' ';
...
// Song * NEW
		if ( !$ibforums->member['id'] ) $last_time = ''; else
		{
			if ( !isset($this->read_mark[ $topic['forum_id'] ]) ) 
			{
	 		...
	 		if ( $this->read_array[ $topic['tid'] ] ) $last_time = $this->read_array[ $topic['tid'] ]; else
			{
				$last_time = -1;
...
// Song * NEW
    ...
    function get_searchable_forums()
    {
		...
		} else
		{
			...
			{
				$res_d = $DB->query("SELECT id, read_perms, password FROM ibf_forums");
			...
			else
			{
				if ( $ibforums->input['forums'] != "" )
				{
					...
					if ( preg_match( "/^c_/", $l ) )
					{
						...
						if ($c)
						{
							$res_d = $DB->query("SELECT id, read_perms, password FROM ibf_forums WHERE category=$c");
					...
					else
					{
						...
						if ($f)
						{
							...
							$res_d = $DB->query("SELECT id, read_perms, password FROM ibf_forums WHERE id=$f".$qe);
	...
	$DB->query("SELECT tid,fid,logTime FROM ibf_log_topics WHERE mid='".$ibforums->member['id']."' AND logTime > '".$date."'");
...
// Song * forum_list
	...
	if ( isset($children[ $id ]) and count($children[ $id ]) > 0)
	{
		...
		{
			if ( $std->check_perms($r['read_perms']) != TRUE ) continue;

			$r['qid']="f_";

			$r['sh'] = ( $r['checked'] or !$all_checkboxes ) ? "checked='checked'" : "";
	...
	$DB->query("SELECT fid FROM ibf_search_forums WHERE mid='".$ibforums->member['id']."' LIMIT 1");
	$all_checkboxes = $DB->get_num_rows();

	$cats = array();
	$forums = array();
	$children = array();

	$DB->query("SELECT * FROM ibf_categories ORDER BY position");

	while ( $r = $DB->fetch_row() ) $cats[$r['id']] = $r;
		
	$DB->query("SELECT f.*, sf.mid as checked FROM ibf_forums f 
	...
	// delete old settings
	$DB->query("DELETE FROM ibf_search_forums WHERE mid='".$ibforums->member['id']."'");
	...
	foreach ($ibforums->input as $key => $value)
	{
		if ( preg_match( "/^f_(\d+)$/", $key, $match ) )
		{
			...
			{
				$DB->query("INSERT INTO ibf_search_forums VALUES (".$ibforums->member['id'].",".$match[1].")");
			}
		}
	}
...
// Song * endless forums, 20.12.04
	...
	if ( count($children[$id]) > 0 ) foreach($children[$id] as $ii => $tt)
	{
		...
		// visuality depth
		for ($i = 0; $i < $level; $i++) $prefix .= "&middot;&middot;";
...
// Song * selected search, 12.03.05
	...
	if ( $days )
	{
		$DB->query("UPDATE ibf_members SET search_days='".$days."' WHERE id='".$ibforums->member['id']."'");
	}

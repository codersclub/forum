...
class Memberlist
{
	...
	function __construct()
	{
		...
		$stmt = $ibforums->db->query("SELECT g_title, g_id, g_icon from ibf_groups WHERE g_hide_from_list <> 1 ORDER BY g_title");

		while ($row = $stmt->fetch())
		{
			$see_groups[] = $row['g_id'];

			$this->mem_groups[$row['g_id']] = array(
				'TITLE' => $row['g_title'],
				'ICON'  => $row['g_icon'],
			);
		}
		...
		$stmt = $ibforums->db->query("SELECT title, id, posts, pips from ibf_titles ORDER BY posts DESC");

		while ($row = $stmt->fetch())
		{
			$this->mem_titles[$row['id']] = array(
				'TITLE' => $row['title'],
				'POSTS' => $row['posts'],
				'PIPS'  => $row['pips'],
		...
		*/
		...
		{
			$stmt = $ibforums->db->query("SELECT COUNT(m.id) as total_members FROM ibf_members m WHERE m.id > 0" . $q_extra);
		}
		...
		$stmt = $ibforums->db->query("SELECT m.name, m.id, m.posts, m.rep, m.points, m.fined,

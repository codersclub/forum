...
class  warn {
	...
	$this->html      = $std->load_template('skin_mod');
        ...
        if ( !$pass ) $std->Error( array( LEVEL => 1, MSG => 'no_permission') );
        
        if ( !$ibforums->vars['warn_on'] ) $std->Error( array( LEVEL => 1, MSG => 'no_permission') );
...
// Song * new warning system
		...
		{
			$DB->query("UPDATE ibf_members SET is_new_warn_exixts=0 WHERE id='".$this->warn_member['id']."'");
		}
// Song
		$perpage = 50;
	...
	if ( $this->type == 'member' ) $std->Error( array( LEVEL => 1, MSG => 'no_permission') );
        
	$fid = intval($ibforums->input['f']);
	$pid = intval($ibforums->input['p']);

	if ( !$fid ) $std->Error( array( 'LEVEL' => 1, 'MSG' => 'incorrect_use') );
...
// Song * new time warning system
	...
	if ( $ibforums->input['level'] == 'add' ) 
	{
		...
		if ( !$restrict_time ) $std->Error( array( 'LEVEL' => 1, 'MSG' => 'incorrect_use') );

		if ( !$pid ) $std->Error( array( 'LEVEL' => 1, 'MSG' => 'incorrect_use') );

// Song * post pid for that violation be, 15.11.2004

		if ( !$ibforums->member['g_is_supmod'] ) 
		{
			$DB->query("SELECT m.id,m.name FROM ibf_members m, ibf_warn_logs wl WHERE wl.pid='".$pid."' 
					and m.id=wl.wlog_addedby and wlog_type != 'pos' LIMIT 1");

			if ( $moderator = $DB->fetch_row() )
			{
				$std->error( array(  'LEVEL' => 1, 'MSG' => "warn_pid", 'EXTRA' => "<a href='{$ibforums->base_url}showuser={$moderator['id']}'>{$moderator['name']}</a>" ) );
			}
		}
...
// Song * new time warning system
	...
	if ( $ibforums->input['level'] != 'null' )
	{
		...
		if ( $ibforums->input['level'] == 'add' ) $ibforums->input['reason'] .= "\r\nСрок действия предупреждения: ".$restrict_time." дней(я).";
...
// Song * ban group, 09.03.05
		...
		if ( $this->warn_member['mgroup'] == $ibforums->vars['ban_group'] ) $group = $ibforums->vars['ban_group']; else
		 if ( $ibforums->input['level'] == 'add' )
		 {
		         // increase level
			$warn_level++;
			...
			$DB->query("INSERT INTO ibf_warnings ({$dbs['FIELD_NAMES']}) VALUES({$dbs['FIELD_VALUES']})");
        
			 // update old warnings with new time
			if ( $warn_level > 1 )
			{
				$DB->query("UPDATE ibf_warnings SET RestrictDate=RestrictDate+".$restrict_time." 
		 ...
		 } else
		 {
		        ...
		        $DB->query("DELETE FROM ibf_warnings WHERE mid='".$this->warn_member['id'].
...
// Song * post pid for that violation be, 15.11.2004
	...
	if ( $ibforums->input['level'] != 'null' )
	{
		...
		$DB->query("UPDATE ibf_members SET mgroup='".$group."',
...
// Song * new ban control
		...
		$DB->query("DELETE FROM ibf_preview_user WHERE mid='".$this->warn_member['id']."' and fid='".$fid."'");
		...
		if ( $mod_queue or $restrict_post or $susp )
		 if ( $ibforums->member['g_is_supmod'] )
		 {
			if ( $ibforums->input['RCanMod'] ) $main .= "mod_posts='".$mod_queue."',"; else
			{ 
				if ( !$mod_queue ) $forum .= "mod_posts=NULL,"; else 
				{
			...
			if ( $ibforums->input['RCanRemPost'] ) $main .= "restrict_post='".$restrict_post."',"; else
			{ 
				if ( !$restrict_post ) $forum .= "restrict_posts=NULL,"; else
				{
			...
			if ( $ibforums->input['RCanBan'] ) $main .= "temp_ban='".$susp."',"; else 
			{
				if ( !$susp ) $forum .= "temp_ban=NULL,"; else 
				{
...
//	for gloabal forum
		...
		if ( $main ) 
		{
			...
			$DB->query("UPDATE ibf_members SET ".$main." WHERE id='".$this->warn_member['id']."'");
		}
...
//       for one forum only
		...
		if ( $forum2 )
		{
			...
			$DB->query("INSERT INTO ibf_preview_user SET mid='".$this->warn_member['id'].
				   "',fid='".$fid."',".$forum);
		}
	} else 
	{
		$DB->query("UPDATE ibf_members SET warn_lastwarn='".time()."',
						   is_new_warn_exixts=1 
			    WHERE id='".$this->warn_member['id']."'");
	}
...
// Song * new ban control
	...
	if ( $errors ) $this->output .= $this->html->warn_errors($ibforums->lang[$errors]);

	$fid = intval($ibforums->input['f']);
	$pid = intval($ibforums->input['p']);

	if ( !$fid or !$pid ) $std->Error( array( 'LEVEL' => 1, 'MSG' => 'incorrect_use') );
...
// Song * post pid for that violation be, 15.11.2004
	...
	$DB->query("SELECT mod_posts,restrict_posts,temp_ban FROM ibf_preview_user WHERE mid='".
                   $this->warn_member['id']."' and fid='".$fid."'");

        $row = $DB->fetch_row();

	$type = array( 'minus' => "", 'add' => "" );
	
	if ( $ibforums->input['type'] == 'minus' ) $type['minus'] = 'checked="checked"'; else $type['add'] = 'checked="checked"';
	...
	if ( $ibforums->member['g_is_supmod'] )
	{
		$DB->query("SELECT name FROM ibf_forums WHERE id='".$fid."'");
	...
	if ( $this->can_mod_q )
	{
		...
		if ($ibforums->member['g_is_supmod']) $this->output .= $this->html->add_radio_buttons('RCanMod',$forum['name']); else $this->output .= "</tr>";
	}
	...
	if ( $this->can_rem_post )
	{
		...
		if ($ibforums->member['g_is_supmod']) $this->output .= $this->html->add_radio_buttons('RCanRemPost',$forum['name']); else $this->output .= "</tr>";
...
// Song * ban group, 09.03.05
	...
	if ( $ibforums->member['g_is_supmod'] )
	{
		$ban = ( $this->warn_member['mgroup'] == $ibforums->vars['ban_group'] ) ? " checked='checked'" : "";
...
// Song * ban group, 09.03.05
	...
	if ( $ibforums->input['type'] == "add" ) $this->output .= $this->html->warn_time();

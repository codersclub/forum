...
class Printable {
	...
	if ( $this->topic['club'] and $std->check_perms( $ibforums->member['club_perms'] ) == FALSE )
	{
		...
		$print->add_output("$this->output");
        	...
        	$print->do_output( array( 'TITLE' => $this->topic['title']." -> ".$ibforums->vars['board_name'], 'JS' => 0, NAV => $this->nav ) );
	...
	function get_posts()
	{
		...
		$DB->query("SELECT * FROM ibf_posts WHERE topic_id='".$this->topic['tid']."' and queued !='1' and use_sig=0 ORDER BY pid");
		...
		while ($i = $DB->fetch_row() )
		{
			...
			if ($i['author_id'] != 0)
			{
				if (preg_match( "/'".$i['author_id']."',/", $mem_ids) ) continue; else $mem_ids .= "'".$i['author_id']."',";
			}
		}
		...
		foreach ($the_posts as $row) {
			...
			if ( trim($row['post']) ) $posts_html .= $this->html->pp_postentry( $poster, $row );
		}

...
#    var $email     = "";
	...
	global $ibforums, $DB, $std, $print;
		...
		$mod = $DB->query("SELECT member_id as id, member_name as name FROM ibf_moderators WHERE forum_id='".$fid."'");

		if ( !$DB->get_num_rows($mod) )
		{
//Jureth + vot		$mod = $DB->query("SELECT id,name FROM ibf_members WHERE mgroup='".$ibforums->vars['admin_group']."'");
			$mod = $DB->query("SELECT id,name FROM ibf_members WHERE mgroup IN (".$ibforums->vars['supermoderator_group'].",".$ibforums->vars['admin_group'].")");
		}

		while ( $moderator = $DB->fetch_row($mod) )
		{
	...
	function mail_member_form($errors="", $extra = "")
	{
		...
		$DB->query("SELECT name, id, email, hide_email, disable_mail FROM ibf_members WHERE id='".$ibforums->input['MID']."'");
	...
	function mail_member_send()
	{
		...
		$DB->query("SELECT name, id, email, hide_email, disable_mail FROM ibf_members WHERE id='".$ibforums->input['to']."'");

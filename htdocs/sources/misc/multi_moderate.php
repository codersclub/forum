...
class  multi_mod
{
	...
	function multi_mod()
	{
		...
		$stmt = $ibforums->db->query("SELECT t.*, f.*
		...
		$stmt = $ibforums->db->query("SELECT * FROM ibf_topic_mmod WHERE mm_id={$this->mm_id}");
		...
		if ($this->mm_data['topic_move'])
		{
			...
			$stmt = $ibforums->db->query("SELECT id, name, subwrap, sub_can_post FROM ibf_forums WHERE id=" . $this->mm_data['topic_move']);

			if ($r = $stmt->fetch())
			{
				if ($r['subwrap'] == 1 AND $r['sub_can_post'] != 1)
				{
					$ibforums->db->exec("UPDATE ibf_topic_mmod SET topic_move=0 WHERE mm_id=" . $this->mm_id);
				} else
				{
		...
		if ($this->mm_data['topic_reply'] and $this->mm_data['topic_reply_content'])
		{
			$print->redirect_screen(sprintf($ibforums->lang['mm_applied'], $this->mm_data['mm_title']), "showtopic={$this->topic['tid']}&view=findpost&p={$this->modfunc->last_id}", "html");
		} else
		{

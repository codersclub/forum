...
class stats {
 	...
 	function check_access($tid)
    {
 	...
 	function show_leaders()
 	{
    	...
    	$DB->query("SELECT id,name,email,hide_email,location,aim_name,icq_number FROM ibf_members 
	...
	$main = $DB->query("SELECT id,read_perms FROM ibf_forums");
		  ...
		  if ( $std->check_perms($row['read_perms']) == TRUE )
		     {
			  ...
			  $res = $DB->query("SELECT
						...
						f.id='".$row['id']."'
					     ORDER BY md.mid");
			  while ( $i = $DB->fetch_row( $res ) ) $data[] = $i;
		    }
	}
...
//------------------------------------------------------------------------------------------------
	...
	function OneLeader()
	{
     ...
     $res2 = $DB->query("
        ...
        WHERE 
      md.member_id='".intval($ibforums->input['mid'])."' and
      p.author_id=md.member_id and
  		p.forum_id=md.forum_id
      GROUP BY md.forum_id
      ORDER BY md.forum_id");
    while ( $i2 = $DB->fetch_row( $res2 ) ) $counts[$i2['forum_id']] = $i2['count'];
    while ( $i = $DB->fetch_row( $res ) ) $data[] = $i;
   	          
    if ( count($data) > 0 )
    {
      $num=1;
    	$this->output =  $this->html->one_leader_strip("<a href='{$ibforums->base_url}showuser=".$data[0][ 'id' ]."' target='_blank'>".$data[0][ 'name' ]."</a>"); 
    	foreach ( $data as $idx => $i )
    	{
    	   $this->output .= $this->html->one_leader($num, "<a href='{$ibforums->base_url}showforum=".$i['forum_id']."' target='_blank'>".$i['forum_name']."</a><br>", intval($counts[$i['forum_id']]));
    	   $num++;
      }
      $this->output .= $this->html->close_strip();    
    }

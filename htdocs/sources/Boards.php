...
// Song + Shaman 
    ...
    function Boards()
    {
        ...
        if ( !$ibforums->member['id'] ) $ibforums->input['last_visit'] = time();
        
	// Song * show all forums link, 09.03.05

        $show_all_link = ( $ibforums->member['id'] and !$ibforums->input['show'] ) ? $this->html->ShowAllLink() : "";
	...
	{
		$DB->query("SELECT id,is_forum,is_visible
			    FROM ibf_boards_visibility
			    WHERE user_id='".$ibforums->member['id']."'");
	...
	$DB->query("SELECT
		...
		if ( !isset($visible) ) $visible = 1;
		if ( !$ibforums->member['id'] ) $visible = 0;
...
//-- mod_global_poll begin
	        ...
	        if ( $ibforums->vars['global_poll'] )
		{
			if ( file_exists($ibforums->vars['base_dir']."sources/mods/global_poll/mod_global_poll_func.php") ) 
			{
			       require $ibforums->vars['base_dir']."sources/mods/global_poll/mod_global_poll_func.php";
		
			} else die("Could not call required function from file 'sources/mods/global_poll/mod_global_poll_func.php'<br>Does it exist?");
...
//-- mod_global_poll end
	...
	{
		if ( !$ibforums->vars['au_cutoff'] ) $ibforums->vars['au_cutoff'] = 15;
		...
		if ( $ibforums->member['id'] )
		{
			...
			while ( $contact = $DB->fetch_row() ) $friends[ $contact['contact_id'] ] = $contact['contact_id'];
		}
		...
		$DB->query("SELECT
				...
				ON (g.g_id=s.member_group) 
		            WHERE
				s.running_time > $time AND
				g.g_id NOT IN(1,5)"); // Not BAN, Not Awaiting
		...
		while ( $result = $DB->fetch_row() )
		{
			if ( strstr( $result['id'], '_session' ) )
			{
				...
				} else $active['NAMES'] .= "{$result['prefix']}{$result['member_name']}{$result['suffix']}{$this->sep_char} \n";
			...
			} else
			{
				if ( empty($cached[ $result['member_id'] ]) )
				{
					...
					if ( $result['login_type'] == 1 )
					{
						...
						if ( $ibforums->member['mgroup'] == $ibforums->vars['admin_group'] and 
						     $ibforums->vars['disable_admin_anon'] != 1 )
						{
							// Song * friends show online, 05.03.05
							if ( isset($friends[ $result['member_id'] ]) )
							{
								$active['FRIENDS'] .= "<a href='{$ibforums->base_url}showuser={$result['member_id']}'>{$result['prefix']}{$result['member_name']}{$result['suffix']}</a>*{$this->sep_char} \n";
							} else
							{
		...
		if ( $ibforums->vars['allow_online_list'] ) $active['links'] = $this->html->active_user_links();
		
		$ibforums->lang['active_users'] = sprintf( $ibforums->lang['active_users'], $ibforums->vars['au_cutoff'] );

		// Song * friends show online, 05.03.05
		
		$friends = ( $active['FRIENDS'] ) ? $this->html->ActiveFriends($active) : "";
		...
		$user_stat_html = $ibforums->lang['users_online_today'];

		// fetch today info
		$DB->query("SELECT members,guests,bots
			    FROM ibf_users_stat
			    WHERE day='".$std->current_day()."'");

		if ( !$stat = $DB->fetch_row() ) 
		{
			$stat['members'] = "0";
			$stat['guests'] = "0";
			$stat['bots'] = "0";
		...
		// members
		$user_stat_html = str_replace( "#USERS#", $stat['members'], $user_stat_html);
		$members = $stat['members'];

		// guests
		$user_stat_html = str_replace( "#GUESTS#", $stat['guests'], $user_stat_html);
		$guests = $stat['guests'];

		// bots
		$user_stat_html = str_replace( "#BOTS#", $stat['bots'], $user_stat_html);
		$bots = $stat['bots'];
		...
		$blank = $ibforums->lang['users_online_yesterday'];
		...
		// all
		$user_stat_html = str_replace( "#ALL#", $stat['members'] + $stat['guests'] + $stat['bots'], $user_stat_html);
	...
	{
		if ( $ibforums->vars['calendar_limit'] < 2 ) $ibforums->vars['calendar_limit'] = 2;
		...
		$DB->query("SELECT
				eventid, title, read_perms, priv_event,
				userid, unix_stamp
		...
		} elseif ( !$ibforums->vars['autohide_calendar'] )
		{
	...
	if ( $ibforums->vars['show_totals'] )
	{
		...
		{
			$query .= "MOST_DATE='".time()."',MOST_COUNT='".$active[TOTAL]."'";
			$stats['MOST_COUNT'] = $active[TOTAL];
			$stats['MOST_DATE']  = time();
			$DB->query("UPDATE ibf_stats SET ".$query);
		}
		...
		if ( $stat['members'] > $members ) $members = $stat['members'];
		if ( $members > $stats['h_members'] )
		{
			$query .= "h_members='".$members."',";
		}

		if ( $stat['guests'] > $guests )   $guests = $stat['guests'];
		if ( $guests > $stats['h_guests'] )
		{
			$query .= "h_guests='".$guests."',";
		}

		if ( $stat['bots'] > $bots ) 	   $bots = $stat['bots'];
		if ( $bots > $stats['h_bots'] )
		{
			$query .= "h_bots='".$bots."',";
		}
		...
		$ibforums->lang['online_record'] = str_replace( "#USERS#", 	$stats['members']	 , $ibforums->lang['online_record']);
		...
		$ibforums->lang['category_record'] = str_replace( "#BOTS#", 	$stats['h_bots']	 , $ibforums->lang['category_record']);

		$ibforums->lang['online_record'] = str_replace( "#ALL#", 	$stats['record']	 , $ibforums->lang['online_record']);
		$ibforums->lang['online_record'] = str_replace( "#DATE#", 	$record_date		 , $ibforums->lang['online_record']);
		...
		if ( $user_stat_html ) $stats_html .= $this->html->TodayOnline($user_stat_html);
...
// vot	$this->output .= '<% BOTTOM BANNER %>';
	...
	if ( $ibforums->vars['is_message_on'] )
	{
        ...
        if ( $ibforums->vars['ips_cp_purchase'] ) $cp = "";
        
	// Song * RSS, 31.01.05

	$rss = ( $cat ) ? $skin_universal->rss("?c=".$ibforums->input['c']) : "";



        $print->do_output( array( 'TITLE' => $ibforums->vars['board_name'], 
				  'JS'    => 0, 
				  'NAV'   => $this->nav, 
				  'RSS'	  => $rss,
			 ) 	);
	...
	$record = $members + $guests + $bots;

	if ( $record > $stats['record'] ) 
	{
		$time = time();

		if ( $daysdown ) $time = $time - 3600 * 24 * $daysdown;

		$query .= "record='".$record."',record_date='".$time."',";
		$stats['record'] = $record;
		$stats['record_date'] = $time;
	...
	if ( $ibforums->member['id'] ) 
	{
		...
		foreach ($list as $l)
		{
			...
			{
				if(isset($c_id)) 
				 if( !isset($this->contr[$c_id])) $this->contr[$c_id]=$fm_s; else 
				 {
...
//		}
		...
		{
			$plus = ( $ibforums->member['id'] ) ? $this->html->CatPlus($cat_data['id']) : "";
...
//			$this->output .= $this->html->draw_middle_banner();
		...
		if ( $temp_html )
		{
			$minus = ( $ibforums->member['id'] ) ? $this->html->CatMinus($cat_data['id']) : "";
...
//	$this->output .= "\$cat_counter=".$cat_counter."<br>";
	...
	if ( $cat_data['state'] == 0 ) $std->Error( array( 'LEVEL' => 1, 'MSG' => 'missing_files' ) );
			     ...
			     !$ibforums->input['show'] )
			{
				...
				if ( !$st )
				{
					...
					foreach($this->children[ $forum_data['id'] ] as $idx => $data) 
					{
						...
						if ( $std->check_perms($data['read_perms']) != TRUE ) continue;
...
// Song * NEW
				...
				if ( $op ) 
				{
					   ...
					   ) 
					{
					           ...
					           )
						{
							...
							if ( $showfor ) 
							{
								if ( $can_see_root != TRUE and !$level ) $level--;

								$result .= $this->process_forum($idx, $data, $level + 1, 0);

							} else $printed_children--;
						}
					}
				}
			}
			   ...
			   )
			{
				...
				$newest['img_new_post'] = $std->forum_new_posts($newest, 
					( count($this->children[ $forum_data['fid'] ]) > 0 or $printed_children > 0 ) ? 1: 0, 
					$state, $this->mods);
...
// Shaman
				...
				} elseif ( $printed_children ) 
				{
					...
					$plus = ( $ibforums->member['id'] ) ? "<a href='{$ibforums->base_url}expfor={$forum_id}' style='text-decoration:none'><br><{F_PLUS}></a>"
									    : "";

					$minus = ( $ibforums->member['id'] ) ? "<a href='{$ibforums->base_url}colfor={$forum_id}' style='text-decoration:none'><br><{F_MINUS}></a>"
									     : "";
        
					$forum_data['tree'] = $expanded ? $minus : $plus;
        
					if ( $expanded ) $style = 'row2'; else $style = 'row4';

					$forum_data['tree'] = '  <td valign="top"'.($expanded? ' rowspan="'.($printed_children+1).'" ' : ' ').'class="'.$style.'">'.$forum_data['tree'].'</td>
...
*/
		...
		// added by À∏ı‡ ( http://forum.sources.ru/index.php?showtopic=218110 )
//				} elseif ( 0 == $level || (isset($this->fs[ $forum_data['parent_id'] ]) && $this->fs[ $forum_data['parent_id'] ] == 0) )
		...
		// end of  À∏ı‡ code
				...
				if ( count($this->children[ $forum_data['id'] ]) AND !$expanded)
				{
					...
					{
					        if ( $std->check_perms( $children['read_perms'] ) == FALSE ) continue;
						...
						if ( $ibforums->member['show_filter'] )
						{
							...
							if ( $ftime > $rtime ) $style = " style='text-decoration:none'";
					...
					if ( $html != "(" )
					{
						...
						if ( $forum_data['description'] ) $html .= "<br>";
...
// Shaman
		...
		if ( $std->check_perms($forum_data['read_perms']) != TRUE ) return "";
...
// Shaman
			...
			if ( 0 == $level ) $forum_data['colspan'] = 'colspan="2" ';

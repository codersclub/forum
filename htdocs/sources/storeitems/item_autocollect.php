...
class item
{
	...
	function do_on_use($blank = "", $blank = "", $blank = "")
	{
		...
		if ($ibforums->member['auto_collect'] == 1)
		{
			...
			$lib->write_log($ibforums->member['id'], $ibforums->member['name'], $ibforums->member['id'], $ibforums->member['name'], 0, "Auto Collect Off", "", "item");
		...
		} else
		{
			...
			$lib->write_log($ibforums->member['id'], $ibforums->member['name'], $ibforums->member['id'], $ibforums->member['name'], 0, "Auto Collect On", "", "item");
			$lib->delete_item($ibforums->input['itemid']);

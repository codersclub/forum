<?php
//---------------------------------------------------
// IBStore Open/Close Any Topic
//---------------------------------------------------
class item
{
    var $name = "Open/Close Any Topics.";
    var $desc = "Open/Close Any Topic!";
    var $extra_one = "";
    var $extra_two = "";
    var $extra_three = "";
    ...
    function on_use($itemid = "")
    {
        ...
        return <<<EOF
			 <form action='{$ibforums->base_url}act=store&code=useitem&itemid={$itemid}' name='item' method='post'>
			  <tr>
				<td class='pformstrip' width='100%' colspan='4'>Open/Close Any Topic</td>
			</tr>
			  <tr>
...
EOF;
    ...
    function do_on_use($type, $overwrite, $blank)
    {
        ...
        $stmt      = $ibforums->db->query("SELECT tid,title,starter_id,state FROM ibf_topics WHERE tid='{$topic_num}' LIMIT 1");
        if ($stmt->rowCount() <= 0) {
            $lib->itemerror("We could not find the topics whos URL you entered, This could be due to a invalid URL.");
        }
        $topic = $stmt->fetch();
        if ($topic['state'] == 'open') {
            $state = 'closed';
        ...
        $lib->write_log($ibforums->member['id'], $ibforums->member['name'], $ibforums->member['id'], $ibforums->member['name'], 0, "Topic \"<a href='{$ibforums->base_url}showtopic={$topic['tid']}'>{$topic['title']}</a>\" " . ucfirst(mb_strtolower($state)) . "!", "", "item");

...
function do_on_use($type,$overwrite,$blank) {
		...
		$DB->query("SELECT tid,title,starter_id,state FROM ibf_topics WHERE tid='{$topic_num}' LIMIT 1");
		...
		$DB->query("UPDATE ibf_topics SET state='{$state}' WHERE tid='{$topic['tid']}' LIMIT 1");
		...
		$lib->write_log($ibforums->member['id'],
				...
				$ibforums->member['name'],
				0,
				"Topic \"<a href='{$ibforums->base_url}showtopic={$topic['tid']}'>{$topic['title']}</a>\" ".ucfirst(strtolower($state))."!",
				"",
				"item");

...
class item {
	...
	function on_add($EXTRA) {
		...
		$DB->query("SELECT id,name FROM ibf_forums WHERE password!='' ORDER BY name DESC");
		while ( $r = $DB->fetch_row() ) {
			$forums[] = array($r['id'],$r['name']);
		}
		$ADMIN->HTML .= $SKIN->add_td_row( array("<b>Forum to give password from?</b><br>The Forum that a user will get the Password from when used.",

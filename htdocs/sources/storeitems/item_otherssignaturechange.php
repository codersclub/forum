...
EOF;
	...
	function do_on_use($protected, $blank = "", $blank = "")
	{
		...
		$ibforums->db->exec("UPDATE ibf_members SET signature='{$ibforums->input['signature']}' WHERE id='{$member['id']}' LIMIT 1");
		$lib->delete_item($ibforums->input['itemid']);
		$lib->write_log($ibforums->member['id'], $ibforums->member['name'], $ibforums->member['id'], $ibforums->member['name'], 0, "Changed <a href='{$ibforums->base_url}showuser={$member['id']}'>{$member['name']}'s</a> Signature.", "", "item");

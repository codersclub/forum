...
class item {
	...
	function on_add($EXTRA) {
		...
		$ADMIN->HTML .= $SKIN->add_td_row( array( "<b>Minimum amount of characters for title?</b><br>The least amount of charaters the new title can have." ,
								  $SKIN->form_input( "extra_one", $EXTRA['extra_one']  )
					 )      );
		$ADMIN->HTML .= $SKIN->add_td_row( array( "<b>Maximum amount of characters for title?</b><br>The maximum amount of characters the new title can have." ,
								  $SKIN->form_input( "extra_two", $EXTRA['extra_two']  )
					 )      );
		$ADMIN->HTML .= $SKIN->add_td_row( array( "<b>Protected Member Groups:</b><br>The user groups that can not have their titles changed. (Seperate with a ',' so 5,4,2,1 ect)" ,
...
function do_on_use($minimum,$maximum,$protected) {
		...
		$DB->query("SELECT id,name,mgroup FROM ibf_members WHERE name='{$ibforums->input['username']}' LIMIT 1");
		if($DB->get_num_rows() <= 0) {
			$lib->itemerror("We cannot find the member whos title you are trying to change. Please check you\'re spelling.");
		} 
		$change_info = $DB->fetch_row();
		...
		$DB->query("UPDATE ibf_members SET title='{$ibforums->input['new_title']}' WHERE id='{$change_info['id']}' LIMIT 1");
		$lib->write_log("Changed ".$change_info['name']." Title To ".$ibforums->input['new_title'],"item");

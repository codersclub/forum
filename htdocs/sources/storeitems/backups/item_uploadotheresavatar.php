...
class item {
	...
	function on_add($EXTRA) {
		...
		$ADMIN->HTML .= $SKIN->add_td_row( array( "<b>Allowed File Types?</b><br>The files types the image is allowed to have. Seperator with a comma " ,
...
EOF;
	...
	function do_on_use($blank="",$blank="",$allowed) {
		global $ibforums,$DB,$print,$lib,$HTTP_POST_FILES;
		$DB->query("SELECT id,name FROM ibf_members WHERE LOWER(name)='".strtolower($ibforums->input['username'])."' LIMIT 1");
		$user = $DB->fetch_row();
		...
		$DB->query("UPDATE ibf_members SET avatar='upload:{$name}' WHERE id='{$user['id']}' LIMIT 1");

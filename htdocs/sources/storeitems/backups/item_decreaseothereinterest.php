...
class item {
	...
	function on_add($EXTRA) {
		...
		$ADMIN->Html .= $SKIN->add_td_row( array( "<b>How much it will decrease the interest?</b><br>" ,
...
function do_on_use($increase,$protected,$blank="") {
		...
		$DB->query("SELECT id,name,mgroup FROM ibf_members WHERE LOWER(name)='".strtolower($ibforums->input['name'])."' LIMIT 1");
		if($DB->get_num_rows() <= 0) {
			$lib->itemerror("Cannot find member.");
		} 

		$DB->query("UPDATE ibf_members SET extra_intrest=extra_intrest-$increase WHERE id='{$change['id']}' LIMIT 1");
		$lib->write_log("Interest Decrease By ".$increase,"item");

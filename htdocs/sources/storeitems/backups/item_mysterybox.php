...
class item {
	...
	function on_add($EXTRA) {
		...
		$Admin->html .= $SKIN->add_td_row( array( "<b>Only give stocked items?</b><br>Only give items that are in stock?" ,
...
function do_on_use($price,$stock,$blank="") {
		...
		$DB->query("SELECT * FROM ibf_store_shopstock WHERE sell_price>'{$price}' ".$stock);
		if(!$DB->get_num_rows()) $lib->redirect("Unable to find any items to give, item has not been delete.");

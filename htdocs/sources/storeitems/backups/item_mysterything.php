...
class item {
	...
	function on_add($EXTRA) {
		...
		$ADMIN->Html .= $SKIN->add_td_row( array( "<b>Only give items with a sell price above?</b><br>This will only give items out with a sellprice above what you set." ,
								  $SKIN->form_input( "extra_one", $EXTRA['extra_one']  )
					 )      );
		$ADMIN->Html .= $SKIN->add_td_row( array( "<b>Lowest Random Amount to get?</b><br>If user gets points the lowest random amount they can get." ,
								  $SKIN->form_input( "extra_two", $EXTRA['extra_two']  )
					 )      );					 

		$ADMIN->Html .= $SKIN->add_td_row( array( "<b>Highest Random Amounts</b><br>If user gets points the highest random amount they can get." ,
	...
	function do_on_use($above_price,$random_min,$random_max) {
		...
		$action = 100;
		if($action > 50) {
			$DB->query("SELECT * FROM ibf_store_shopstock WHERE sell_price>'{$above_price}'");
			...
			if($temp < 1) {
				...
				$DB->query("UPDATE ibf_members SET points='{$points}' WHERE id='{$ibforums->member['id']}' LIMIT 1");
				if($points < 0) {
				...
				$DB->query("INSERT INTO ibf_store_inventory VALUES('','{$ibforums->member['id']}','{$temp}','{$price}')");			
			}
		} else {
			$points = mt_rand($random_min,$random_max);
			$DB->query("UPDATE ibf_members SET points=points+{$points} WHERE id='{$ibforums->member['id']}' LIMIT 1");
			if($points < 0) {
				$log_msg = "Lost {$points} {$ibforums->vars['currency_name']} from using Mystery Thing.";
				$msg	= "You lost {$points} {$ibforums->vars['currency_name']}.";
			} else {

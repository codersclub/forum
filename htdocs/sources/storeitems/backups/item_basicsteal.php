...
class item {
	...
	function on_add($EXTRA) {
		...
		$ADMIN->HTML .= $SKIN->add_td_row( array( "<b>Minumum amount aloud to steal?</b><br>The lowest amount a user can steal from another user." ,
								  $SKIN->form_input( "extra_one", $EXTRA['extra_one']  )
					 )      );
		$ADMIN->HTML .= $SKIN->add_td_row( array( "<b>Maximum amount aloud to steal</b><br>The highest amount a user can steal from another user." ,
								  $SKIN->form_input( "extra_two", $EXTRA['extra_two']  )
					 )      );
		$ADMIN->HTML .= $SKIN->add_td_row( array( "<b>Chance of a Successful</b><br>The percentage of a Successful steal. (e.g 50 would give them 50/50 odds of a successful steal)" ,
...
function do_on_use($minimum,$maximum,$percentage) {
		...
		} else if($ibforums->input['amount'] > $maximum) {
			$lib->itemerror("The amount you are trying to steal is to high. (max is {$maximum} )");		
		}
		$DB->query("SELECT id,name,mgroup,points FROM ibf_members WHERE LOWER(name)='".strtolower($ibforums->input['username'])."' LIMIT 1");
		...
		if($chance <= $percentage) {
			...
			$DB->query("UPDATE ibf_members SET points=points+".$ibforums->input['amount']." WHERE id='{$ibforums->member['id']}' LIMIT 1");
			$DB->query("UPDATE ibf_members SET points='{$lost}' WHERE id='{$user['id']}' LIMIT 1");
			$lib->write_log("Stole {$ibforums->input['amount']} from {$user['name']} Successfully.");
		} else {
			...
			$lib->write_log("Tried to steal {$ibforums->input['amount']} from {$user['name']} lost {$lost_amount}.");
		}

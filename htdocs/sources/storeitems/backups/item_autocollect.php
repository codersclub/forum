...
function do_on_use($blank="",$blank="",$blank="") {
		...
		if($ibforums->member['auto_collect'] == 1) {
			$DB->query("UPDATE ibf_members SET auto_collect='0' WHERE id='{$ibforums->member['id']}' LIMIT 1");
			...
			$DB->query("UPDATE ibf_members SET auto_collect='1' WHERE id='{$ibforums->member['id']}' LIMIT 1");

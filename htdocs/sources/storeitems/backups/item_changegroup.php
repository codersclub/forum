...
class item {
	...
	function on_add($EXTRA) {
		...
		$DB->query("SELECT g_id,g_title FROM ibf_groups WHERE g_access_cp='0' ORDER BY g_id DESC");
		while($group = $DB->fetch_row()) {
			$groups[] = array($group['g_id'],$group['g_title']);
		}
		$ADMIN->HTML .= $SKIN->add_td_row( array( "<b>User Group To Upgrade To:</b>",
...
function do_on_use($group,$blank="",$blank="") {
		...
		$DB->query("UPDATE ibf_members SET mgroup='{$group}' WHERE id='{$ibforums->member['id']}' LIMIT 1");

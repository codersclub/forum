...
class item
{
    ...
    function on_add($EXTRA)
    {
        ...
        $ADMIN->Html .= $SKIN->add_td_row(array(
                                               "<b>Highest Amount?</b><br>The highest random amount that can be given.",
    ...
    function do_on_use($random_min, $random_max, $blank = "")
    {
        ...
        $ibforums->db->exec("UPDATE ibf_members SET points=points+$random WHERE id='{$ibforums->member['id']}' LIMIT 1");
        $lib->write_log($ibforums->member['id'], $ibforums->member['name'], $ibforums->member['id'], $ibforums->member['name'], 0, "Got {$random} {$ibforums->vars['currency_name']}", "", "item");

...
class item
{
    ...
    function on_add($EXTRA)
    {
        ...
        $ADMIN->HTML .= $SKIN->add_td_row(array(
                                               "<b>Maximum Emoticion Size?</b><br>The Maximum size in Bytes that a emoticion can be.",
                                               $SKIN->form_input("extra_two", $EXTRA['extra_two'])
                                          ));

        $ADMIN->HTML .= $SKIN->add_td_row(array(
                                               "<b>Allowed File Types?</b><br>The files types the emoticion is allowed to have. Seperate with a comma",
...
EOF;
    ...
    function do_on_use($click, $max_size, $allowed)
    {
        ...
        $file['name'] = $_FILES['emoticion']['name'];
        $file['size'] = $_FILES['emoticion']['size'];
        $file['type'] = $_FILES['emoticion']['type'];
        ...
        if ($_FILES['emoticion']['name'] == "" || !$_FILES['emoticion']['name'] || ($_FILES['emoticion']['name'] == "none")) {
            $lib->itemerror("Please choose a emoticion to upload.");
        }
        @chmod($ibforums->vars['html_dir'] . 'emoticons', 0777);
        if (!@move_uploaded_file($_FILES['emoticion']['tmp_name'], $ibforums->vars['html_dir'] . 'emoticons' . "/" . $file['name'])) {
        ...
        $ibforums->db->exec("INSERT INTO ibf_emoticons (id,typed,image,clickable) VALUES('','{$ibforums->input['ecode']}','{$file['name']}','{$click}')");

...
class item
{
	...
	function on_add_edits($admin)
	{
		...
		$istheir    = file_check("../app/classes/functions.php", "m.flood_control");
	...
	function do_on_use($add_flood, $blank = "", $blank = "")
	{
		...
		$lib->write_log($ibforums->member['id'], $ibforums->member['name'], $ibforums->member['id'], $ibforums->member['name'], 0, "Flood Control is now " . $add_flood, "item", "", "item");

<?
//---------------------------------------------------
// IBStore Pin Unpin Any Topic
//---------------------------------------------------
class item
{
	var $name = "Pin/Unpin Any Topic.";
	var $desc = "Pin/Unpin any topic!";
	var $extra_one = "";
	var $extra_two = "";
	var $extra_three = "";
	...
	function on_use($itemid = "")
	{
		...
		return <<<EOF
			 <form action='{$ibforums->base_url}act=store&code=useitem&itemid={$itemid}' name='item' method='post'>
			  <tr>
				<td class='pformstrip' width='100%' colspan='4'>Pin/Unpin Any Topic</td>
			</tr>
			  <tr>
...
EOF;
	...
	function do_on_use($type, $overwrite, $blank)
	{
		...
		$stmt      = $ibforums->db->query("SELECT tid,title,starter_id,pinned FROM ibf_topics WHERE tid='{$topic_num}' LIMIT 1");
		if ($stmt->rowCount() <= 0)
		{
		...
		$lib->write_log($ibforums->member['id'], $ibforums->member['name'], $ibforums->member['id'], $ibforums->member['name'], 0, "Topic \"{$topic['title']}\" {$pin}!", "", "item");

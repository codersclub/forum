...
class item {
	...
	function on_add($EXTRA) {
		...
		$Admin->html .= $SKIN->add_td_row( array( "<b>Add how much to Interest for User?</b><br>" ,
...
function do_on_use($increase,$blank="",$blank="") {
		...
		$DB->query("UPDATE ibf_members SET extra_intrest='{$increase}' WHERE id='{$ibforums->member['id']}' LIMIT 1");
		$lib->write_log($ibforums->member['id'],
				$ibforums->member['name'],
				$ibforums->member['id'],
				$ibforums->member['name'],
				0,
				"Interest Increase By ".$increase,
				"",
				"item");

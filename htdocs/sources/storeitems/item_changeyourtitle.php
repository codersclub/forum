...
class item {
	...
	function on_add($EXTRA) {
		...
		$ADMIN->HTML .= $SKIN->add_td_row( array( "<b>Количество месяцев для ношения статуса</b>" ,
	...
	function on_add_edits($admin) {
		...
		if(!$is_their) {
			$DB->query("ALTER TABLE `ibf_members` ADD `title_date` INT( 9 ) DEFAULT '0' NOT NULL AFTER `posts`");
		}
		return false;
	 }
...
function do_on_use($minimum,$maximum,$month_count) {
		...
		$DB->query("UPDATE ibf_members SET title='{$ibforums->input['new_title']}',title_date='{$new_time}' WHERE id='{$ibforums->member['id']}' LIMIT 1");
		$lib->write_log($ibforums->member['id'],
				$ibforums->member['name'],
				$ibforums->member['id'],
				$ibforums->member['name'],
				0,
				"Смена статуса пользователя на ".$month_count." месяцев. Новый Статус: '".$ibforums->input['new_title']."'",
				"",
				"item");

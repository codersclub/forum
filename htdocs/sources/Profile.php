...
//echo "Profile.php started.";
	...
	if ($ibforums->member['g_mem_info'] != 1) $std->Error(array(
						'LEVEL' => 1,
						'MSG' => 'no_permission' ) );
	...
	//--------------------------------------------
  	// Check input..
    	...
    	if ( empty($id) ) $std->Error( array( 'LEVEL' => 1,
					      'MSG' => 'incorrect_use' ) );
	...
	$i = 0;
		...
		if($i == 0) $info['member_name'] = $row['author_name'];

		$TotalPosts += $row['counter'];

		if($row['inc_postcount']) $TotalThematic += $row['counter'];
	...
	} else
	{
	  ...
	  $this->nav        = array( "<a href='index.php?showuser={$info['member_id']}'>{$ibforums->lang['page_title']} {$info['member_name']}</a>",
    	...
    	if ( $tmp_title and $info['member_title'] != $ibforums->lang['no_info'] ) 
	{
        ...
        } else $member[ $rep ] .= " ".$ibforums->vars['rep_postfix'];
...
// vot echo "view_profile started.";
	...
	if ($ibforums->member['g_mem_info'] != 1) $std->Error(
					array(	'LEVEL' => 1,
						'MSG' => 'no_permission' ) );
	...
	//--------------------------------------------
  	// Check input..
    	...
    	if ( empty($member['id']) ) $std->Error( array( 'LEVEL' => 1,
							'MSG' => 'incorrect_use' ) );
	...
	if ( !$rank ) $rank = $ibforums->lang['no_info'];

    	$info['member_title'] = $member['title'] ? $member['title'] : $rank;


	// Song * online

	if ( $member['s_id'] ) $online = "<{ONLINE}>"; else $online = "";
	$info['online'] = $online;
    	...
    	if ( $ibforums->vars['sig_allow_html'] == 1 )
	{
    	...
    	} else $info['birthday'] = $ibforums->lang['no_info'];
...
//        }
	...
	while( $row = $DB->fetch_row() )
	{
		...
		} else $field_data[ $row['fid'] ] = ($field_data[ $row['fid'] ] == "") ? $ibforums->lang['no_info'] : nl2br($field_data[ $row['fid'] ]);

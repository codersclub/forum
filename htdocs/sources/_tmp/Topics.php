...
// Song * safe code tag withing wrapping long lines, 03.11.2004
	...
	$index= '"'.preg_replace('/(\d)/', '"$1' , strval($this->code_counter++)).'#';

	$this->code_text[ $index ] = $code;
...
// Song * quote rights
	...
	if ( $qr == TRUE ) {
		...
		if ( $std->user_reply_flood($this->topic['start_date']) ) $qr = FALSE;
...
// Song * correct cached fields for new post id
		...
		$row['post_css'] = $post_count % 2 ? 'post1' : 'post2';			
			...
			( $ibforums->member['g_is_supmod'] or
				( $this->mod[$ibforums->member['id']] and 
					$this->mod[$ibforums->member['id']]['post_q'] 
					) 
				 ) and 
		     $row['queued'] 
		   ) {
		...
		if ( $ibforums->vars['post_wordwrap'] > 0 and !$row['use_sig'] ) {
			   ...
			   )
			{
				...
				if ( $row['post'] != $old_old_post )
				{
					...
					if ( $row['post'] != $old_post ) foreach( $this->code_text as $idx => $code )
					{
		...
		if ( $ibforums->input['hl'] ) {
       			...
       			$row['post'] = preg_replace("#(&[^\s]+?;)#ies", "\$this->cut_html_tag_text('\\1')", $row['post']);

       			$opentag = $this->cut_html_tag_text("<span class='searchlite'>");
       			$closetag = $this->cut_html_tag_text("</span>");
...
// vot							$row['post'] = preg_replace( "/(^|\s)(".preg_quote($keywords, '/').")(\s|$)/i", "\\1<span class='searchlite'>\\2</span>\\3", $row['post'] );
							$row['post'] = preg_replace( '/(?<!")('.preg_quote($keywords, '/').")/i", $opentag."\\1".$closetag, $row['post'] );
						}
					}
				}
			} else {
// vot				while( preg_match( "/(^|\s)(".preg_quote($keywords, '/').")(\s|,|$)/i", $row['post'] ) ) {
// vot				while( preg_match( "/(".preg_quote($keywords, '/').")/i", $row['post'] ) ) {
// vot					$row['post'] = preg_replace( "/(^|\s)(".preg_quote($keywords, '/').")(\s|,|$)/i", "\\1<span class='searchlite'>\\2</span>\\3", $row['post'] );
					$row['post'] = preg_replace( '/(?<!")('.preg_quote($keywords, '/').")/i", $opentag."\\1".$closetag, $row['post'] );
// vot				}
			}
			...
			// vot: restore code tag text from an array if post has been wrapped
                       	$row['post'] = str_replace(array_keys($this->code_text), $this->code_text, $row['post']);
		...
		{
			$row['add_to_faq'] = "<a href='{$this->base_url}act=Mod&amp;CODE=35&amp;auth_key=".$this->md5_check."&amp;f={$this->forum['id']}&amp;t={$this->topic['tid']}&amp;p={$row['pid']}'>".( ( $row['added_to_faq'] ) ? "<{P_FAQ_EXISTS}>" : "<{P_FAQ_ADD}>")."</a> &middot;";
		}
		...
		{
			$row['delete_delayed'] = ( $qr ) ? $this->delayed_delete_button($row, $poster, $post_count) : "";
		}
		...
		if ( !$pinned ) $row['checkbox'] = $this->checkbox($row['pid']);
		...
		if ( $row['attach_id'] and !$row['use_sig'] ) {

			if ( !$row['attach_size'] ) $this->set_attach(&$row);
		...
		// /Song * attachments, 16.03.05
			...
			if (  $ibforums->member['view_img'] and ( $this->forum['siu_thumb'] or $ibforums->vars['show_img_upload'] )
				   and
				 (
			...
			{
				if ( ( $this->forum['siu_thumb'] or $ibforums->vars['siu_thumb'] ) AND 
				     $ibforums->vars['siu_width'] AND $ibforums->vars['siu_height'] )
				{
					//----------------------------------------------------
					// Tom Thumb!
					//----------------------------------------------------
					     ...
					     $img_size[1] <= $ibforums->vars['siu_height'] )
					{
                                                ...
                                                $row['attachment'] = $this->html->Show_attachments_img( $row['pid'] );
				...
				} else $row['attachment'] = $this->html->Show_attachments_img( $row['pid'] );
			...
			{
				$image = ( $ibforums->member['view_img'] ) ? "<img src='{$ibforums->vars['mime_img']}/{$this->mimetypes[ $row['attach_type'] ][1]}' border='0' alt='{$ibforums->lang['attached_file']}'>"
									   : "";

				$row['attachment'] = $this->html->Show_attachments( 
					...
					array (
						...
						'pid'   => $row['pid'],
						'size'  => sprintf( $ibforums->lang['attachment_size'], $row['attach_size']),
			  		      )  	  
										);
			}
		}
		...
		if ( $poster['signature'] and $ibforums->member['view_sigs'] and !$row['use_sig'] )
		{
		        if ( !$row['g_use_signature'] ) 
			{
				$poster['signature'] ="[color=gray][size=1]Подпись была выключена в связи с наложенным наказанием.[/size][/color]";
                        ...
                        $data = array(  TEXT          => $poster['signature'],
                                        SMILIES       => 1,
                                        CODE          => 1,
                                        SIGNATURE     => 0,
		...
		// Song * leave link from guests nick too
                ...
                if ( !$row['author_id'] ) $poster['name'] .= "<span class='unreg'>";
		$poster['name'] .= $name;

                if ( !$row['author_id'] ) $poster['name'] .= "</span>";
		$poster['name'] .= "</a>";
		   ...
		   ) 
		{
			...
			$link .= "<a href='{$ibforums->base_url}act=modcp&amp;CODE=domodtopics&amp;f={$this->forum['id']}&amp;TID_{$this->topic['tid']}=remove'>{$ibforums->lang['modcp_permit']}</a> · ";
		}
	...
	if ( $ibforums->input['t'] < 0  ) $std->Error( array( LEVEL => 1, MSG => 'missing_files') );
        ...
        } else $this->topic = $ibforums->topic_cache;
        ...
        if ( !$this->forum['id'] or !$this->topic['tid'] ) 
	{
        ...
        if ( $this->topic['state'] == 'link' )
        {
        	...
        	$print->redirect_screen( $ibforums->lang['topic_moved'],
					 "showtopic={$f_stuff[0]}" );
        }
	...
	if ( $ibforums->member['id'] )
	{
		...
		if ( $DB->get_num_rows() )
		{
			...
			$this->log_time = $log_time['logTime'];         	

		} else $this->log_time = 0;
	}
		...
		if ( $ibforums->input['view'] == 'new' or $ibforums->input['view'] == 'old' )
		{
			...
			$sort = ($ibforums->input['view'] == 'old') ? " DESC" : "";
        	...
        	} else if ( $ibforums->input['view'] == 'getlastpost' ) $this->return_last_post(); else

		if ( $ibforums->input['view'] == 'getnewpost' )
		{
			// if new topic, directing to the begining

			if ( !$this->log_time ) $std->boink_it($ibforums->base_url."showtopic=".$this->topic['tid']); else
			{
				$DB->query("SELECT
						pid,
						post_date
				...
				} else $std->boink_it($ibforums->base_url."showtopic=".$this->topic['tid']."&amp;view=getlastpost");
			}
		...
		} elseif ( $ibforums->input['view'] == 'findpost' )
		{
			...
			if ( $pid > 0 )
			{
				...
				if ( !$cposts = $DB->fetch_row() or !$cposts['posts'] )
				{
					...
					$DB->query("SELECT topic_id
						    FROM ibf_posts
						    WHERE pid='".$pid."'");
					
					if ( $topic = $DB->fetch_row() ) $tid = $topic['topic_id'];
	...
	$std->song_set_topicread( $this->log_time, $this->forum['id'], $this->topic['tid'] );
	$std->song_set_forumread( $this->forum['id'] );
	...
	$this->topic['why_close'] = ($this->topic['state'] == 'closed') ? $this->topic['why_close'] : '';
	...
	if ( $this->topic['description'] ) $this->topic['description'] = ', '.$this->topic['description'];
...
//		$favs = explode(",",$ibforums->member['favorites']);
	...
	$DB->query("SELECT
        ...
        if ( $DB->get_num_rows() ) 
	{
		if ( !$links ) $links = $ibforums->lang['attached_links']."<ul>";
	...
	if($this->first < 0) $this->first = 0; // vot
	...
	// 4 pinned post
	if ( $this->topic['pinned_post'] ) {
		$main = $DB->query($query_top_post);

		if ( !$DB->get_num_rows($main) ) $pinned++;

		// show_post
		$this->process_posts($main, 1);
	}
	     ...
	     $std->user_reply_flood($this->topic['start_date']) == FALSE )
	{
		$show = "none";
	        ...
	        if ( !$ibforums->member['id'] or $ibforums->member['quick_reply'] ) $show = "show";
		...
		{
			$topic_decided = ( $ibforums->member['id'] == $this->topic['starter_id'] ) ? $skin_universal->topic_decided() : "";
		}
	...
	$print->add_output("$this->output");
		...
		'TITLE'    => str_replace( array("!", "&#33;"), array("", ""), "{$this->topic['title']} -> ".$ibforums->vars['board_name']),
		'JS'       => "",
		'NAV'      => $this->nav_extra,
	...
	if ( ( !$ibforums->member['id'] or $ibforums->member['show_ratting'] ) and $member['show_ratting'] and !$member['use_sig'] ) {
		...
		$rep 	    = ( $this->forum['inc_postcount'] ) ? $member['rep'] : $member['ratting'];

		$rep_suffix = ( $this->forum['inc_postcount'] ) ? "t" : "f";

		$rep_link   = $ibforums->lang['rep_name'].$ibforums->lang['rep_'.$rep_suffix];

		// /dual ratting, 15.01.05

		$tmp_rep = empty($rep) ? 0 : $rep;
		...
		if ( $ibforums->vars['rep_badnum']  and $tmp_rep <= $ibforums->vars['rep_badnum'] ) {
			...
			if ( !is_numeric( $rep ) ) {
                ...
                } else $rep .= " ".$ibforums->lang['rep_postfix'];
        
		if ( $ibforums->member['id'] ) {
			$stuff = array( 't' => $this->topic['tid'], 'f' => $this->forum['id'], 'mid' => $member['id'], 'p' => $member['pid'] );
	...
	if ( !$member['use_sig'] ) {	
		...
		if ( $member['avatar'] ) $member['avatar'] .= "<br>";
		...
		if ( $pips ) {
			if ( !$ibforums->member['id'] or $ibforums->member['view_img'] ) $pip = "<{A_STAR}>"; else $pip = "*";

			if ( !$ibforums->member['id'] or $ibforums->member['show_status'] )
			  if ( preg_match( "/^\d+$/", $pips ) ) {
				for ($i = 1; $i <= $pips; ++$i) $member['member_rank_img'] .= $pip;
		...
		// add crlf
		if ( $member['title'] ) $member['title'] .= "<br>";
		...
		// status
		if ( $this->mod[ $member['id'] ] and !$member['title'] ) $member['title'] = "Moderator";
	...
	global $ibforums;
		...
		if ( !$row['delete_after'] )
		{
			$button = ( !$this->first and !$post_count ) 
				? ""
				: "<a href='{$ibforums->base_url}act=Mod&amp;CODE=28&amp;f={$this->forum['id']}&amp;t={$this->topic['tid']}&amp;p={$row['pid']}&amp;auth_key={$this->md5_check}' onclick=\"return JSRequest(this.href,unique_id(this));\"'><{P_X}></a>";
		} else
		{
			$days = $row['delete_after'] - time();
			...
			if ( $days > 0 )
			{
				...
				if ( $days > 1 ) 
				{
					...
					$days = sprintf($ibforums->lang['delete_through'], $days);
				} else
				{
					$days = $ibforums->lang['delete_waiting'];
				}
			} else
			{
				$days = $ibforums->lang['delete_waiting'];
			...
			$button = "<a href='{$ibforums->base_url}act=Mod&amp;CODE=29&amp;f={$this->forum['id']}&amp;t={$this->topic['tid']}&amp;p={$row['pid']}&amp;auth_key={$this->md5_check}' onclick=\"return JSRequest(this.href,unique_id(this));\"'>{$days}</a>";
		}
	...
	function moderation_panel() {
		...
		$actions = array( 'MOVE_TOPIC', 'CLOSE_TOPIC', 'OPEN_TOPIC',
				  'DELETE_TOPIC', 'EDIT_TOPIC', 
				  'PIN_TOPIC', 'UNPIN_TOPIC', 'UNSUBBIT',
				  'MERGE_TOPIC', 'SPLIT_TOPIC', 
				  'ATTACH_LINKS', 'UNPIN_POST', 'PIN_POST',
				  'DELETE_POSTZ', 'MOVE_POSTZ',
				  'DELETE_DELAYED', 'FAQ_POSTS', 'HIDE_TOPIC',
				  'SHOW_TOPIC');
	...
	function append_link( $key="" ) {
		...
		if ( $this->topic['pinned'] and $key == 'PIN_TOPIC')   return "";

		if ( !$this->topic['pinned'] and $key == 'UNPIN_TOPIC') return "";


		// Song * pin/unpin post

		if ( $key == 'PIN_POST' and $this->topic['pinned_post'] ) return "";
		if ( $key == 'UNPIN_POST' and !$this->topic['pinned_post'] ) return "";


		// Song * hide/show topic, 11.04.05

		if ( $key == 'HIDE_TOPIC' and $this->topic['hidden'] ) return "";
		if ( $key == 'SHOW_TOPIC' and !$this->topic['hidden'] ) return "";

		// Song * delete posts delayed

		if ( $key == 'DELETE_DELAYED' and !$this->forum['days_off'] ) return "";


		// Song * add to faq, 02.05.05

		if ( $key == 'FAQ_POSTS' and !$this->forum['faq_id'] ) return "";
	...
	global $ibforums, $std;
		...
		if ( $this->topic['state'] == 'closed' )
		{
			...
			} else return "<{A_LOCKED_B}> · ";
		}
		
		if ( $std->check_perms($this->forum['reply_perms'] ) == FALSE ) return "";

		// Song * Old Topics Flood, 15.03.05

		if ( $std->user_reply_flood($this->topic['start_date']) ) return "";
	...
	$ibforums->lang  = $std->load_words($ibforums->lang, 'lang_post', $ibforums->lang_id);
        ...
        if ( !$poll_data['pid'] ) return;
        
        if ( !$poll_data['poll_question'] ) $poll_data['poll_question'] = $this->topic['title'];
...
// vot: BAD QUERY
	...
	// /Song * poll life, 25.03.05
        ...
        if ( $this->moderator['edit_post'] ) $can_edit = 1;

        if ( $this->moderator['delete_post'] ) $can_delete = 1;
        
        if ( $this->moderator['close_topic'] ) $can_close = 1;
        ...
        if ( !$ibforums->member['id'] ) 
	{
		...
		reset($poll_answers);
		foreach ($poll_answers as $entry) $total_votes += $entry[2];
		...
		if ( $ibforums->member['id'] ) 
		{
	        	...
	        	foreach ($poll_answers as $entry)
	        	{
	        		...
	        		if ( strlen($choice) < 1 ) continue;
        		
	        		if ($ibforums->vars['poll_tags']) $choice = $this->parser->parse_poll_tags($choice);
				...
				$percent = $votes == 0 ? 0 : $votes / $total_votes * 100;
		...
		} else $html .= "</table><b>".$ibforums->lang['guests_no_view']."</b>";
		...
		$type = ( $poll_data['is_multi_poll']) ? "checkbox" : "radio";
		$name = 'poll_vote';

		if ($poll_data['is_weighted_poll'] ) $html .= $this->poll_html->weighted_js($poll_count);
		if ($poll_data['is_weighted_poll'] || $poll_data['is_multi_poll']) $i = 1;
		...
		if ( $ibforums->member['id'] ) 
		{
				...
				} else $html   .= $this->poll_html->Render_row_form($votes, $id, $choice, $type, $name);
	...
	function return_last_post()
	{
		...
		$DB->query("SELECT max(pid) as pid
			    FROM ibf_posts
			    WHERE
				queued != 1 AND
				topic_id='".$this->topic['tid']."'");
	...
	if ( !$ibforums->member['g_post_new_topics']
	   or $std->check_perms($this->forum['start_perms']) == FALSE ) return 0;

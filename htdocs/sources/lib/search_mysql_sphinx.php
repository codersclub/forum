...
class search_lib extends Search
{
	...
	/**
	 *  оличество найденых постов/топиков
	 * @var integer
	 */
	...
	function do_main_search()
	{
		...
		{
			// @ - служебный символ дл€ сфинкса в режиме match=extend
		...
		if ($type != 'nameonly')
		{
			...
			// sphinx_snippets(p.post, 'test1', 'ѕупкин'),
			$keywords = $DB->quote($keywords);
			$sphinx_query = $DB->quote("{$posts_datecut}{$forums}{$topics_name}");
			...
			/*
			 * SQL_NO_CACHE нужен дл€ того, чтобы обновилось/установилось значение переменной состо€ни€ 
			 * sphinx_total_found (см. ниже, где присваиваетс€ значение $this->documents_found)
			 */
			if ($this->is->search_in == 'posts') {
				...
				$sphinx_query= $DB->quote("{$posts_datecut}{$forums}{$posts_name}");

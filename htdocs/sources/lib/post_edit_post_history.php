...
class post_functions extends Post
{
	...
	function show_form($class)
	{
		...
		{
			$items[$i]['new_text'] = $class->parser->post_db_parse($class->parser->prepare(array(
			                                                                                    'TEXT'    => $prevoius_text,
			...
			$items[$i]['old_text'] = $class->parser->post_db_parse($class->parser->prepare(array(
			                                                                                    'TEXT'    => $prevoius_text,
			                                                                                    'CODE'    => $class->forum['use_ibc'],
			                                                                                    'SMILIES' => 1,
			                                                                                    'HTML'    => $class->forum['use_html']
		...
		} else
		{
			...
			$class->output .= View::make("post.edit_history",
				[
					'data'     => $items,

...
// ******************************************************************************
	...
	if ( !isset($this->absent_highlight[ $syntax ]) )
	{
		...
		} else $this->absent_highlight[ $syntax ] = $syntax;
	}
...
///////////////////////////////////////////////////////////////////////////
	...
	if ( $ibforums->member['id'] )
	{
		if ( $ibforums->member['syntax'] != 'none' )
		{
			...
			if ( $ibforums->vars['plg_offline_client'] ) $temp = 'server';
			...
			if ( $length < 50*1024 or $temp == 'client' )
			{
				if ( !$syntax )
				{
					if ( $id != -1 ) $cfg = $this->syntax_find_by_id($id, $temp);	
				} else
				{
					...
					if ( $syntax == "1С" or $syntax == "1с" ) $syntax = "1C";


					if ( $syntax != 'no' )	$cfg = $this->syntax_find_by_code($syntax, $temp);
				}
			}
		}
	}
				...
				if($result)
				{
					...
					for($n = 0; $n < 10; $n ++)
					{
						if($rule->actions[$n] == 'tag')
						{
							...
							$txt = $rule->tags[$n].
									str_replace("\n", $rule->tags[($n+1)%10] . "**[*]**". $rule->tags[$n], $txt)
									.$rule->tags[($n+1)%10];
	...
	$use_line_numbering = $ibforums->member['syntax_use_line_numbering'] !== NULL ? $ibforums->member['syntax_use_line_numbering'] : false;
	if (!$use_line_numbering) {
		$view = str_replace(array('<li>','</li>'), array('<div class="code_line">','</div>'), $view);
	}
	$html = $this->wrap_style( array( 'STYLE' => 'CODE', 'BCOLOR' =>  $cfg->back_color, 'FCOLOR' =>  $cfg->fore_color, 'CODE_ID' => $this->code_count ) );

// vot	15/03/06
	$view = "{$html['START']}<div>$view</div>{$html['END']}";
	if ( $temp == 'client' && $cfg ) {
		$use_line_numbering = $use_line_numbering ? 'true' : 'false';
	...
	private function convert_spoiler($matches){
		...
		if ( ! $matches[4] ) {
			// Starting Tag
			$this->quote_open++;
			return '<div class=\'spoiler closed\'><div class=\'spoiler_header\' onclick=\'openCloseParent(this)\'>'
				.(trim($matches[3])?$matches[3]:$ibforums->lang["spoiler"])
				.'</div><div class=\'body\'>';
	...
	global $ibforums;
		...
		$txt = preg_replace_callback( "#(\[spoiler(=([^\]]*?))?\])|(\[/spoiler\])#is", array( &$this, 'convert_spoiler'), $txt );
	...
	function parse_attach_tags(&$txt)
	{
		...
		$txt = preg_replace_callback( '#\[attach\s*=\s*(p?)(\d+)?\s*\](.*?)\[/attach\]#i',
		                       array($this, 'regex_attach'), 
		                       $txt );
	...
	if ( !$ibforums->member['id'] or !$message ) return "";
	
	if ( !$ibforums->member['g_is_supmod'] )
	{
		if ( !$mid ) return "";

		if ( $ibforums->member['id'] != $mid ) return "";
	}
...
// Shaman * Main Draw Function
	...
	if ( !isset($in['CODE']) ) $in['CODE'] = 0;
	if ( !isset($in['SMILIES']) ) $in['SMILIES'] = 0;
	if ( !isset($in['SIGNATURE']) ) $in['SIGNATURE'] = 0;
...
//        	$txt = preg_replace( "#\[table\](.+?)\[/table\]#is", "<table class='post_table'>\\1</table>", $txt );
	...
	if ( !$ibforums->member['g_is_supmod'] or !$message ) return $message;
	...
	function convert($in=array(	'TEXT' => "",
					'SMILIES' => 0,
					'CODE' => 0,
					'SIGNATURE' => 0,
					'HTML' => 0),
...
// Song * restore code tag text after html text parsing, 03.11.2004
	...
	if ( $this->code_counter ) foreach( $this->code_text as $idx => $code )
	{
...
// Song * mod tags to upper, 26.11.04
	...
	$txt = preg_replace( "#\[url\](\S+?)\[/url\]#ie"                                       , 
			     "\$this->regex_build_url_auto_parser(array('html' => '\\1','show' => '\\1'))", $txt );

	$txt = preg_replace( "#\[url\s*=\s*\&quot\;\s*(\S+?)\s*\&quot\;\s*\](.*?)\[\/url\]#ie" , 
			     "\$this->regex_build_url_auto_parser(array('html' => '\\1','show' => '\\2'))", $txt );
	...
	global $ibforums, $DB;
        ...
        if ( $type == 'boardrules' ) return "[URL={$ibforums->vars['gl_link']}{$section}][B][COLOR=RED]{$word}[/COLOR][/B][/URL]"; else
	 ...
	 if ( $type == 'faq' and $fid ) 
	 {
		...
		if ( $row['parent_id'] != -1 ) $fid = $row['parent_id'];

		$DB->query("SELECT id FROM ibf_forums WHERE parent_id='".$fid."' and name LIKE '%FAQ%' LIMIT 1");

		if ( !$DB->get_num_rows() ) return $word; else
		{
			$row = $DB->fetch_row();
			return "[URL={$ibforums->base_url}showforum=".$row['id']."][B]{$word}[/B][/URL]"; 	
		}

	 } else return "[URL={$ibforums->base_url}act={$type}][COLOR=RED]{$word}[/COLOR][/URL]";
...
//+-----------------------------------------------------------------------------------------
	...
	function convert_emoticon($code="", $image="") {
		...
		{
			if ( !$ibforums->member['view_img'] or $ibforums->member['sskin_id'] == 0 ) return $code;
...
// vot: BAD MESSAGE. Require language file
			...
			if ( !$ibforums->member['id'] )
			{
				...
				$use_line_colouring = $ibforums->member['syntax_use_line_colouring'] !== NULL ? $ibforums->member['syntax_use_line_colouring'] : true;
				$use_line_numbering = $ibforums->member['syntax_use_line_numbering'] !== NULL ? $ibforums->member['syntax_use_line_numbering'] : false;
				if ($use_wrap) {
					$class .= " code_wrap ";
				}
			...
			if (!$this->rss_mode) {
				$label  =  '<span onclick=\'return syntax_collapse(this,'.$code_id.');\' title=\''.$ibforums->lang['code_collapse_button'].'\'>';
					$label	.= '<span id=\'code_collapse_on_'.$code_id.'\' '.($lines_count == 0 ? 'style=\'display:none\'' : '').'><{CODE_COLLAPSE_ON}></span>';
					$label	.= '<span id=\'code_collapse_off_'.$code_id.'\' '.($lines_count == 0 ? '' : 'style=\'display:none\'').'><{CODE_COLLAPSE_OFF}>';
					$label	.= '</span></span>';
				
				$label .= '<span onclick=\'return syntax_wrap(this,'.$code_id.');\' title=\''.$ibforums->lang['code_word_wrap_button'].'\'>';
					$label .= "<span id='code_wrap_on_$code_id' ".($use_wrap ? '' : "style='display:none'")."><{CODE_WRAP_ON}></span>";
					$label .= "<span id='code_wrap_off_$code_id' ".($use_wrap ? "style='display:none'" : '')."><{CODE_WRAP_OFF}>";
					$label .= '</span></span>';
				...
				if ($ibforums->member['id'] &&  $ibforums->member['syntax'] != 'none') {
					$label .= '<span onclick=\'return syntax_numbering(this,'.$code_id.');\' title=\''.$ibforums->lang['code_line_numbers_button'].'\'>';
						$label .= "<span id='code_numbering_on_$code_id'' ".($use_line_numbering ? '' : "style='display:none'")."><{CODE_NUMBERING_ON}></span>";
						$label .= "<span id='code_numbering_off_$code_id'' ".($use_line_numbering ? "style='display:none'" : '')."><{CODE_NUMBERING_OFF}>";
						$label .= '</span></span>';
				}
			}
...
// vot			    );
	...
	function regex_sql_tag($sql="") {
        ...
        function regex_mod_tag($txt="") {
                ...
                $html = "</div><BR><TABLE {$ibforums->skin['white_background']} style='border:2px solid blue;'><TD align=middle bgColor=#6060ff valign=center width=1%><FONT color=#ffffff size=+7 style='FONT-FAMILY: Times'><B>М</B></FONT></TD><TD width=3><BR></TD><TD valign=top>{$txt}</TD></TABLE><div class='postcolor'>";
...
//		return "<!--QuoteBegin-->{$this->quote_html['START']}<!--QuoteEBegin-->";
	...
	global $ibforums, $std;
		...
		if ( !$alt ) $alt = "user posted image"; else 
		{
...
// Song * Mastilior skin patch for red color, 30.04.05 
	...
	function regex_build_url($url=array()) {
		...
		if ( !trim($url['show']) ) $url['show'] = $url['html'];
...
//vot	if ( stristr($show, "forum.sources.ru") )
	...
	{ 
		if (preg_match("/showtopic=(\d+)/",$show,$find) || preg_match("/&t=(\d+)/",$show,$find)) 
		 {
			$res = $DB->query("SELECT title FROM ibf_topics WHERE tid=".intval($find[1]));
			$record = $DB->fetch_row( $res );
		...
		if ( preg_match("/showforum=(\d+)/",$show,$find) ) 
		 {
			$res = $DB->query("SELECT name FROM ibf_forums WHERE id=".intval($find[1]));
			$record = $DB->fetch_row( $res );
		...
		if ( preg_match("/showuser=(\d+)/",$show,$find) ) 
		 {
			$res = $DB->query("SELECT name FROM ibf_members WHERE id=".intval($find[1]));
			$record = $DB->fetch_row( $res );
...
//vot	if ( stristr($show, "forum.sources.ru") )
	...
	{ 
		if (preg_match("/showtopic=(\d+)/",$show,$find) || preg_match("/&t=(\d+)/",$show,$find)) 
		{
			$res = $DB->query("SELECT title FROM ibf_topics WHERE club=0 and tid='".intval($find[1])."'");
		...
		{
			$res = $DB->query("SELECT name FROM ibf_forums WHERE id='".intval($find[1])."'");
			if ( $record = $DB->fetch_row( $res ) ) $show = $record['name'];
		}
	...
	} else return $url['st'].$url['html'].$url['end'];

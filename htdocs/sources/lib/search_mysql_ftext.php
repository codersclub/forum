...
class search_lib extends Search
{
	...
	function do_simple_search()
	{
		...
		if (!$ibforums->input['sid'])
		{
			...
			$stmt = $ibforums->db->query("SELECT
				...
				WHERE
					...
					MATCH(post) AGAINST ('$check_keywords' $boolean)");
			...
			$cache = "SELECT
					MATCH(post) AGAINST ('$check_keywords' $boolean) as score,
					t.tid,
					t.title,
					t.posts,
					t.views,
		...
		} else
		{
			...
			$stmt = $ibforums->db->query("SELECT id, name FROM ibf_categories");
			...
			$this->output .= View::Make('search.result_simple_header', ['data'=> array(
			                                                            'links'   => $this->links,
			                                                            'start'   => $this->is->first,
			                                                            'end'     => $show_end,
			...
			while ($row = $stmt->fetch())
			{
				...
				$row['relevance'] = sprintf("%3d", ($row['score'] > 1.0)
					? 100
					: $row['score'] * 100);
			...
			$this->output .= View::Make('search.result_simple_footer', ['data' => array(
			                                                            'links' => $this->links,
			                                                       )]);
	...
	function do_main_search()
	{
		...
		if ($type != 'nameonly')
		{
			$topics_query = "SELECT t.tid
					FROM ibf_topics t
					WHERE
						$topics_datecut t.forum_id IN ($forums)
						$topics_name AND
						t.approved=1 AND
			...
			$posts_query = "SELECT p.pid
					FROM ibf_posts p
					WHERE
						$posts_datecut  p.forum_id IN ($forums) AND
						p.queued <> 1
						$posts_name AND
						MATCH(post) AGAINST ('" . trim($keywords) . "' $boolean)";
		} else
		{
			$topics_query = "SELECT t.tid
					FROM ibf_topics t

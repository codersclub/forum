...
class post_functions extends Post
{
	...
	function add_new_topic(Post $class)
	{
		...
		$this->topic = array(
			...
			'approved'         => ($class->obj['moderate'] == 1 || $class->obj['moderate'] == 2)
				? 0
				: 1,
			'pinned'           => $pinned,
		...
		if ($this->post['attach_exists'])
		{
			...
			$ibforums->db->exec("UPDATE ibf_posts
				    SET post = " . $ibforums->db->quote($this->post['post']) . "
		...
		{
			$ibforums->db->insertRow('ibf_tracker', [
		...
		if ($ibforums->member['id'])
		{
			...
			$ibforums->db->exec("UPDATE ibf_members
				    SET
					$pcount
					$mgroup " . $module->post_points($ibforums->vars['pointsper_topic']) . "
	...
	function show_form(Post $class)
	{
		...
		if ($class->obj['preview_post'])
		{
			...
			$this->upload = array_merge($this->upload
				? : array(), $draft->getAttachments());

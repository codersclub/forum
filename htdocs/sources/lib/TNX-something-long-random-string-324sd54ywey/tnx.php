...
class TNX_n
{
    ...
    function TNX_n($login, $cache_dir)
    {
        ...
        if (!empty($this->_exceptions)) {
            ...
            for ($i = 0; $i < sizeof($exceptions); $i++) {
                if ($_SERVER['REQUEST_URI'] == $exceptions[$i]) {
                    return false;
                }
                if ($exceptions[$i] == '/' and preg_match("#^\/index\.\w{1,5}$#", $_SERVER['REQUEST_URI'])) {
                    return false;
                }
                if (mb_strpos($_SERVER['REQUEST_URI'], $exceptions[$i]) !== false) {
                    return false;
                }
            }
        }
        ...
        if (!empty($this->_forbidden)) { // 21.09.07
            ...
            for ($i = 0; $i < sizeof($forbidden); $i++) {
                if ($_SERVER['REQUEST_URI'] == $forbidden[$i]) {
                    return false;
                }
            }
        }
    ...
    function show_link($num = false)
    {
        ...
        if (!isset($this->_content_array_count)) {
            $this->_content_array_count = sizeof($this->_content_array);
        }
        ...
        if ($num === false or $num >= $this->_content_array_count) {
            ...
            return (!empty($this->_encoding)) ? iconv("CP1251", $this->_encoding, $links) : $links;
    }
    ...
    function check_down()
    {
        ...
        if ($this->down_filesize() >= 39) {
            ...
            } else {       // если же время в допустимой норме, то просто обновляем в 0
                    $this->clean_down();
            }
        }
    }

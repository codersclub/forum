...
class task_functions
{
	...
	{
		global $DB, $std, $ibforums, $ROOT_PATH;
	...
	function run_task()
	{
		...
		if ( $this->type == 'internal' )
		{
			...
			$this_task = $DB->simple_exec_query( array(
					'select' => '*',
					'from'   => 'task_manager',
	...
	function save_next_run_stamp()
	{
		...
		$this_task = $DB->simple_exec_query( array( 'select' => 'task_next_run', 'from' => 'task_manager', 'where' => 'task_enabled = 1', 'order' => 'task_next_run ASC', 'limit' => array(0,1) ) );
		...
		$cache       = $DB->simple_exec_query( array( 'select' => '*', 'from' => 'cache_store', 'where' => "cs_key='systemvars'" ) );
		
		$cache_array = unserialize(stripslashes($cache['cs_value']));
		
		$cache_array['task_next_run'] = $this_task['task_next_run'];
		
		$DB->do_update( 'cache_store', array( 'cs_value' => addslashes(serialize($cache_array)) ), "cs_key='systemvars'" );
	}

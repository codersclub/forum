...
class post_functions extends Post {
	...
	function process(Post $class) {
		...
		if ( $ibforums->input['QPost'] )
		{
			...
			// add it to the post
			$temp_text  = '[QUOTE='.$this->quoted_post['AUTHOR']['name'].','.
						$this->quoted_post['DATE'].','.
						$ibforums->input['p'].']'.
						$this->quoted_post['post'].'[/QUOTE]'."\n";
...
// Jureth: done
		...
		if ( $ibforums->member['id'] )
		{
			...
			$module = new module();
			$module->topic_overcheck(array('posts' => intval($posts['posts']-1),
			...
			$DB->query("UPDATE ibf_members
				    SET ".$pcount.$mgroup.$module->post_points($ibforums->vars['pointsper_reply']).
	...
	function show_form($class) {
		...
		$DB->query("SELECT pid, post, author_id, author_name, post_date 
			    FROM ibf_posts

    ...
    function &Read()
    {
        ...
        while (fReadLine($fh, $_)) {
            if (mb_strpos($_, $this->delimeter[0]) === 0) {
                if (count($arr = explode($this->equation, mb_substr($_, 1), 2)) != 2) {
                    continue;
                }
                ...
                mb_substr($arr[1], -1) === $this->delimeter[3]) {
                    ...
                    while (fReadLine($fh, $_) && mb_strlen($_)) {
                        $this->hash[$arr[0]][$key][] = $_;
                    }
                } else {
                    $arr[1] = explode($this->delimeter[1], $arr[1], 2);

                    if (!mb_strlen($arr[1][0] = trim($arr[1][0])) ||
                    (($bool = isset($arr[1][1])) && !mb_strlen($arr[1][1] = trim($arr[1][1])))) {
                        continue;
                    }
                    ...
                    if (isset($this->hash[$arr[0]])) {
                        if (is_array($this->hash[$arr[0]])) {
                            if ($bool) {
                                $this->hash[$arr[0]][$arr[1][0]] = $arr[1][1];
                            } else {
                                $this->hash[$arr[0]][] = $arr[1][0];
                            }
                        }
                    } elseif ($bool) {
                        $this->hash[$arr[0]] = array($arr[1][0] => $arr[1][1]);
                    } else {

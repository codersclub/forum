...
class HiLight_Format
{
    ...
    function func1(&$arr, $sch, $c, &$cfg)
    {
        $arr = array();
        foreach ($sch as $k => $v) {
            if ($cfg->hash['CMP'](mb_substr($v, 0, mb_strlen($c)),$c)) {
                $arr[] = $v;
            }
        }
        return count($arr);
    ...
    function func6(&$arr, $sch, $c, &$cfg)
    {
        $arr = array();
        foreach ($sch as $k => $v) {
            if ($cfg->hash['CMP'](mb_substr($v[0], 0, mb_strlen($c)),$c)) {
                $arr[] = $v;
            }
        }
        return count($arr);
        ...
        while (strGetC($str, $c)) {
            if (!(mb_strpos($this->cfg->hash['CHARS'][0], $c) === false)) {
                ...
                if (!$res) {
                    $this->addStr(
                        $nstr,
                        $this->cfg->hash['STYLE'][is_numeric($cstr) ? 'Digits' : 'Text'],
                        array($cstr)
                    );
                }
            }
            ...
            if (in_array($c, $this->cfg->hash['QUOTATION'])) {
                ...
                foreach (array('PREFIX','DELIMITER') as $k => $v) {
                    ...
                    if (($key = ucfirst(mb_strtolower($v))) === 'Prefix') {
                        ...
                        $str = preg_replace(
                            '/^([' . quotemeta($this->cfg->hash['CHARS'][0]) . ']+)/e',
                            'func3($c,' . "'\\1'" . ')',
                            $str
                        );
                        if (!mb_strlen($c)) {

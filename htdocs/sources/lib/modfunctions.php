...
class modfunctions
{
	...
	function topic_delete($ftid) {
		...
		// mark topics deleted
//		$DB->query("UPDATE ibf_topics SET approved=0, deleted=1 WHERE tid".$tid);
		...
		$DB->query("DELETE FROM ibf_polls
			    WHERE tid".$tid);
...
//			    WHERE tid".$tid);
	...
	function leave_message($source, $moveto, $topic,$is_mirror = false) {
		...
		$post_text = "[MOD]".($is_mirror ? $ibforums->lang['topic_mirrored'] : $ibforums->lang['topic_moved']);
	...
	function get_forum_name_by_id($forum_id) {
		...
		if ( $source_forum = $DB->fetch_row() )
		{
			if ( $source_forum['parent_id'] == -1 ) {
				...
				if ( $source_parent_forum = $DB->fetch_row() ) {
					return array(
						'name' => $source_forum['name'],
...
// vot: BAD - strange function. WHY TO CHANGE the code tag???
	...
	if ( $syntax ) return $this->syntax($code,$syntax);

	if ( !$id ) return $this->error($code);

	if ( !$this->cache_syntax[$id] )
	{
		$hid = $std->get_highlight_id($id);

		if ( !$hid ) return $this->error($code);

		$DB->query("SELECT syntax
			    FROM ibf_syntax_list
			    WHERE id='".$hid."'");

		if ( !$syntax = $DB->fetch_row() ) return $this->error($code);
...
//Jureth					 "\$this->regex_code_syntax('\\3', '\\2', ".$source.")", $post['post']) and 
...
//Jureth			else $this->leave_message($source[$topic],$moveto,$topics); //Jureth
			else $this->leave_message($source[$topics],$moveto,$topics); //Jureth
	        }
		...
		} else
		{
			...
			foreach($moveto as $current_move_forum) {
				...
				$oq = $DB->query("SELECT t.*,
						  ...
						  WHERE tid $tid
						  ");
				while ( $current_topic = $DB->fetch_row($oq) )
				{
					...
					$mirror_exists_q = 
							...
							LIMIT 1");
					if ($DB->fetch_row($mirror_exists_q)) {
					...
					$current_topic_id = $DB->get_insert_id();
	...
	function topic_delete_mirrors($topic_id, array $forum_ids) {
		...
		if ($forum_ids) {
			$query = "DELETE FROM ibf_topics WHERE `mirrored_topic_id` = $topic_id ".
		...
		if ($row['cnt'] == 0) {
			$DB->query("UPDATE ibf_topics SET has_mirror = false WHERE tid = $topic_id");
		}
...
/* <--- Jureth --- */ 	
//Useless. All functionality moved to forum_recount() function.
...
/* >--- Jureth --- */
	...
	function forum_recount($fid="")
	{
		...
		//---------------------------------------------
		$DB->query("SELECT `approved` 
			    FROM ibf_topics
			    WHERE
				approved=0
				and forum_id=$fid
			    LIMIT 0,1;");
		$topics_premod=$DB->get_num_rows();
		...
		//---------------------------------------------
		$DB->query("SELECT `queued`
			    FROM ibf_posts
			    WHERE
				queued = 1
				AND forum_id=$fid
			    LIMIT 0,1;");
		$posts_premod = $DB->get_num_rows();
		...
		$db_string = $DB->compile_db_update_string( array (
				...
				'has_mod_posts'    => (intval($posts_premod)+intval($topics_premod)) < 1 ? 0 : 1
				 )        );

	...
	class Attachment
	{
		...
		public function saveToDB($save_to = 'post')
		{
			...
			if (!$this->from_post_row)
			{
				...
				if (!$this->attach_id)
				{
					...
					if ($this->post_id || $save_to != 'post')
					{ // post - default value, for backward compatibility
		...
		private function checkPostAccess($post_id, $member)
		{
			...
			if (!$topic['approved'])
			{
				...
				if (!$std->premod_rights($topic['starter_id'], $mod[$member['id']]['topic_q'], // TODO: find where mod[]
					$topic['app'])
				)
				{

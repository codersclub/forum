...
class PostParser
{
    ...
    private function convert_spoiler($matches)
    {
        ...
        if (!$matches[4]) {
            // Starting Tag
            $this->quote_open++;
            return '<div class=\'spoiler closed\'><div class=\'spoiler_header\' onclick=\'openCloseParent(this)\'>' . (trim(
                $matches[3]
            )
                ? $matches[3]
    ...
    function parse_poll_tags($txt)
    {
        ...
        $txt = preg_replace_callback(
            ...
            function ($m) {
                return $this->regex_check_image($m[2], $m[1]);
            },
            $txt
        );
        $txt = preg_replace_callback(
        ...
        $txt = preg_replace_callback(
            "#\[img\](.+?)\[/img\]#i",
            function ($m) {
                return $this->regex_check_image($m[1]);
            },
            $txt
        );
        ...
        $txt = preg_replace_callback(
            "#\[url\](\S+?)\[/url\]#i",
            function ($m) {
                return $this->regex_build_url(['html' => $m[1], 'show' => $m[1]]);
            },
            $txt
        );
        $txt = preg_replace_callback(
            ...
            function ($m) {
                return $this->regex_build_url(['html' => $m[1], 'show' => $m[2]]);
            },
            $txt
        );
        $txt = preg_replace_callback(
        ...
        while (false !== preg_match("#\[color=([a-zA-Z0-9]*)\](.+?)\[/color\]#is", $txt)) {
            $txt = preg_replace_callback(
                ...
                function ($m) {
                    return $this->regex_font_attr(['s' => 'col', '1' => $m[1], '2' => $m[2]]);
                },
                $txt
            );
        }
    ...
    private function renderPreview(AttachImage $attach, $text)
    {
        ...
        if ($attach->hasOption('img')) { //image as itself
    ...
    function parse_smiles($txt)
    {
        ...
        if (count($this->emoticons) > 0) {
            foreach ($this->emoticons as $a_id => $row) {
                ...
                $txt = preg_replace(
                    "!(?<=[^\w&;])$code(?=.\W|\W.|\W$)!e",
                    "\$this->convert_emoticon('$code', '$image')",
                    $txt
                );
            }
        }
    ...
    function prepare(
        ...
        if (mb_strpos($txt, "[") === false) {
            ...
            $txt = preg_replace_callback(
                ...
                function ($m) {
                    return $this->regex_build_url(['html' => $m[2], 'show' => '', 'st' => $m[1]]);
                },
                $txt
            );
        ...
        if ($in['CODE'] == 1) {
            ...
            $txt = preg_replace_callback(
                "#\[code\s*?(=\s*?(.*?)|)\s*\](.*?)\[/code\]#is",
                function ($m) use ($in) {
                    return $this->regex_code_syntax($m[3], $m[2], $in['HID']);
                },
                $txt
            );
            ...
            if ($in['SIGNATURE'] != 1) {
                ...
                while (preg_match("#\n?\[list=(a|A|i|I|1)\](.+?)\[/list\]\n?#is", $txt)) {
                    $txt = preg_replace_callback(
                        "#\n?\[list=(a|A|i|I|1)\](.+?)\[/list\]\n?#is",
                        function ($m) {
                            return $this->regex_list($m[2], $m[1]);
                        },
                        $txt
                    );
                }
            ...
            $txt = preg_replace_callback(
                "#(\[spoiler(=.+?)?].*\[/spoiler\])#is",
                function ($m) {
                    return $this->regex_parse_spoiler($m[1]);
                },
                $txt
            );
             ...
             * @deprecated
             * (c) (r) and (tm)
             * Feature remove: http://forum.sources.ru/index.php?showtopic=385805&view=findpost&p=3487088
             */
            ...
            while (preg_match("#\[size=([^\]]+)\](.+?)\[/size\]#is", $txt)) {
                $txt = preg_replace_callback(
                    "#\[size=([^\]]+)\](.+?)\[/size\]#is",
                    function ($m) {
                        return $this->regex_font_attr(['s' => 'size', '1' => $m[1], '2' => $m[2]]);
                    },
                    $txt
                );
            }
            ...
            while (preg_match("#\[font=([^;<>\*\(\)\]\"']*?)\](.*?)\[/font\]#is", $txt)) {
                $txt = preg_replace_callback(
                    "#\[font=([^;<>\*\(\)\"\]']*?)\](.*?)\[/font\]#is",
                    function ($m) {
                        return $this->regex_font_attr(['s' => 'font', '1' => $m[1], '2' => $m[2]]);
                    },
                    $txt
                );
            }
     ...
     */
        ...
        $txt = preg_replace_callback(
            "#\[gm\](.+?)\[/gm\]#is",
            function ($m) use ($in) {
                return $this->regex_global_moderator_message($m[1], $in['MID']);
            },
            $txt
        );
        ...
        $txt = preg_replace_callback(
            "#\[mm\](.+?)\[/mm\]#is",
            function ($m) {
                return $this->regex_moderator_message($m[1]);
            },
            $txt
        );
        ...
        $txt = preg_replace_callback(
            "#\[ex\](.+?)\[/ex\]#is",
            function ($m) {
                return $this->regex_exclaime_tag($m[1]);
            },
            $txt
        );
        ...
        //'#(?<!\w|&shy;|href=.|src=.)((https?|news|s?ftp):\/\/([[:alpha:]][[:alnum:]-]*[[:alnum:]]\.?)+(:\d+)?([[:alnum:]_\.\-:\/\?\#\[\]@\!\$\&\'\(\)\*\+\,\;\=]|%[A-Z0-9]{2})*)#ie',
            ...
            function ($m) {
                return $this->regex_build_url(['html' => $m[1], 'show' => '', 'st' => '']);
            },
            $txt
        );
    ...
    function convert(
        ...
        $txt = preg_replace_callback(
            "#(\?|&amp;|;|&)s=([0-9a-zA-Z]){32}(&amp;|;|&|$)?#",
            function ($m) {
                return $this->regex_bash_session($m[1], $m[3]);
            },
            $txt
        );
        ...
        $txt = preg_replace_callback(
            "#\[code\s*?(=\s*?(.*?)|)\s*\](.*?)\[/code\]#is",
            function ($m) {
                return $this->cut_code_tag_text($m[3], $m[2]);
            },
            $txt
        );
        ...
        $txt = preg_replace_callback(
            "#\[mm\](.+?)\[/mm\]#is",
            function ($m) {
                return $this->mod_messages_check($m[1]);
            },
            $txt
        );
        ...
        $txt = preg_replace_callback(
            "#\[gm\](.+?)\[/gm\]#is",
            function ($m) {
                return $this->global_mod_messages_check($m[1]);
            },
            $txt
        );
        ...
        if (in_array($ibforums->member['mgroup'], $ibforums->vars['mm_groups'])
            ...
            $txt = preg_replace_callback(
                "#\[(mod|ex)\](.+?)\[/(mod|ex)\]#is",
                function ($m) {
                    return $this->regex_mod_tag_convert($m[1], $m[2]);
                },
                $txt
            );
        } else {
    ...
    function macro($txt, $fid = "0")
    {
        ...
        $txt = preg_replace_callback(
            "#\[url\](\S+?)\[/url\]#i",
            function ($m) {
                return $this->regex_build_url_auto_parser(['html' => $m[1], 'show' => $m[1]]);
            },
            $txt
        );
        ...
        $txt = preg_replace_callback(
            "#\[url\s*=\s*\&quot\;\s*(\S+?)\s*\&quot\;\s*\](.*?)\[\/url\]#i",
            function ($m) {
                return $this->regex_build_url_auto_parser(['html' => $m[2], 'show' => $m[2]]);
            },
            $txt
        );
        ...
        $txt = preg_replace_callback(
            "#\[url\s*=\s*(\S+?)\s*\](.*?)\[\/url\]#i",
            function ($m) {
                return $this->regex_build_url_auto_parser(['html' => $m[1], 'show' => $m[2]]);
            },
            $txt
        );
        ...
        $txt = preg_replace_callback(
            "#\[sf\](.+?)\[/sf\]#is",
            function ($m) use ($fid) {
                return $this->regex_word_search($m[1], 'sf', 'word', $fid);
            },
            $txt
        );
        $txt = preg_replace_callback(
            "#\[sall\](.+?)\[/sall\]#is",
            function ($m) {
                return $this->regex_word_search($m[1], 'all', 'word');
            },
            $txt
        );
        $txt = preg_replace_callback(
            "#\[st\](.+?)\[/st\]#is",
            function ($m) use ($fid) {
                return $this->regex_word_search($m[1], 'sf', 'title', $fid);
            },
            $txt
        );
        $txt = preg_replace_callback(
            "#\[stall\](.+?)\[/stall\]#is",
            function ($m) {
                return $this->regex_word_search($m[1], 'all', 'title');
            },
            $txt
        );
        $txt = preg_replace_callback(
            "#\[sf\s*=\s*(\S+?)\s*\](.*?)\[\/sf\]#i",
            function ($m) {
                return $this->regex_word_search($m[2], 'sf', 'word', $m[1]);
            },
            $txt
        );
        $txt = preg_replace_callback(
            "#\[st\s*=\s*(\S+?)\s*\](.*?)\[\/st\]#i",
            function ($m) {
                return $this->regex_word_search($m[2], 'sf', 'title', $m[1]);
            },
            $txt
        );
        ...
        $txt = preg_replace_callback(
            "/\.правила?(, п\.\d+)*/ui",
            function ($m) {
                return $this->wordreplacer($m[0], 'boardrules');
            },
            $txt
        );
        $txt = preg_replace_callback(
            "/\.поиск[а-я]{0,}/ui",
            function ($m) {
                return $this->wordreplacer($m[0], 'Search');
            },
            $txt
        );
        $txt = preg_replace_callback(
            "/\.FAQ(?![a-z,0-9])/i",
            function ($m) use ($fid) {
                return $this->wordreplacer($m[0], 'faq', $fid);
            },
            $txt
        );
    ...
    function user_link($user_name = "")
    {
        ...
        $stmt = $ibforums->db->query(
            "SELECT id FROM ibf_members WHERE LOWER(name)='" . mb_strtolower(addslashes(trim($user_name))) . "'"
        );
        if (!$stmt->rowCount()) {
            return $user_name;
        }
    ...
    function wordreplacer($word, $type, $fid = "")
    {
        ...
        if ($type == 'boardrules') {
            ...
            if ($type == 'faq' and $fid) {
                ...
                $stmt = $ibforums->db->query(
                    "SELECT id FROM ibf_forums WHERE parent_id='" . $fid . "' and name LIKE '%FAQ%' LIMIT 1"
                );
    ...
    function unconvert($txt = "", $code = 1, $html = 0)
    {
        ...
        if ($code == 1) {
            ...
            $txt = preg_replace(
                ...
                "\$this->unconvert_sql(\"\\2\")",
                $txt
            );
            $txt = preg_replace(
                "#<!--html-->(.+?)<!--html1-->(.+?)<!--html2-->(.+?)<!--html3-->#e",
                "\$this->unconvert_htm(\"\\2\")",
                $txt
            );
            ...
            $txt = preg_replace(
                "#<span style=['\"]font-size:(.+?)pt;line-height:100%['\"]>(.+?)</span>#e",
                "\$this->unconvert_size('\\1', '\\2')",
                $txt
            );
    ...
    function wrap_style($in = array())
    {
        ...
        return array(
            'START' => "<div class='{$possible_use[$in['STYLE']]['wrapper_class']}'>{$label}<div class='" . mb_strtolower(
                    $possible_use[$in['STYLE']]['class']
                ) . " $class'{$extra}>",
            'END' => "</div></div>"
        );
    }
    ...
    function regex_parse_quotes($matches)
    {
        ...
        $txt = preg_replace_callback(
            '#\[quote\s*=([^\],]+?),(\d{2,4}\.\d{2}\.\d{2},\s*\d{2}:\d{2}(?:\d{2})?)(?:,(\d+))?\]#i',
            [$this, 'regex_quote_tag'],
            $txt
        );
        ...
        if ($this->quote_open == $this->quote_closed and !$this->quote_error) {
            $txt = preg_replace_callback(
                ...
                function ($m) {
                    return $this->regex_preserve_spacing($m[1]);
                },
                trim($txt)
            );
    ...
    function regex_build_url_auto_parser_cut(array $url)
    {
        ...
        if (stristr($show, $ibforums->vars['board_url'])) {
            if (preg_match("/showtopic=(\d+)/", $show, $find) || preg_match("/&t=(\d+)/", $show, $find)) {
                $stmt = $ibforums->db->query(
                    "SELECT title FROM ibf_topics WHERE club=0 and tid='" . intval($find[1]) . "'"
                );

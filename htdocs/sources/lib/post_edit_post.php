...
class post_functions extends Post
{
	...
	function show_form($class)
	{
		...
		$start_table = View::Make("post.table_top", ['data' => "{$ibforums->lang['top_txt_edit']} {$this->topic['title']}"]);
		...
		if ($class->obj['can_upload'])
		{
			if ($attachments = Attachment::getPostAttachmentsFromRow($this->orig_post))
			{
				...
				$upload_field .= View::Make("post.edit_upload_field", ['data' => $std->size_format($ibforums->member['g_attach_max'] * 1024),'files' => $attachments]);
			} else
			{

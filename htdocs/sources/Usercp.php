...
// Song * delete profile link, 04.03.05
 	...
 	function forum_tracker() 
	{
		...
		if ( $ibforums->member['disable_mail'] )
		{
 		...
 		$res_d = $DB->query("SELECT t.frid, t.start_date, f.*, c.id as cat_id, c.name as cat_name
 		...
 		if ( $DB->get_num_rows($res_d) )
 		{
 			...
 			while( $forum = $DB->fetch_row($res_d) )
 			{
	...
	$this->nav        = array( "<a href='".$this->base_url."act=UserCP&amp;CODE=00'>".$ibforums->lang['t_title']."</a>" );
 	...
 	function do_openid_change()
 	{
		...
		if ($check && $check != $ibforums->member['id'])
		{
 		...
 		$DB->query("UPDATE ibf_members SET openid_url='$openid' WHERE id='".$this->member['id']."'");
 	...
 	global $ibforums, $DB, $std, $print, $skin_universal;
		...
		if ( $ibforums->member['disable_mail'] )
		{
 		...
 		$res_d = $DB->query("SELECT s.trid, s.member_id, s.topic_id, s.last_sent, s.start_date as track_started, t.*, 
			   f.id as forum_id, f.name as forum_name, f.read_perms 
...
// Song * NEW
				...
				$read_mark = ($ibforums->member['board_read'] > $read_mark) ? $ibforums->member['board_read'] 
											    : $read_mark;
		
				$read_mark = ($read_mark < (time() - 60*60*24*30 )) ? (time() - 60*60*24*30) 
										    : $read_mark;

				$last_time = ($read_mark > $topic['topic_read']) ? $read_mark : $topic['topic_read'];
				
				if ( !$last_time ) 
				{
					$last_time = '1';
					if ( $topic['last_post'] < $read_mark ) $last_time = $read_mark;
				}
...
// Song * add text smile skin
		...
		$DB->query("SELECT id, name FROM ibf_emoticons_skins");
        
		while ( $l = $DB->fetch_row() )
		{
			$smile_select .= $l['id'] == $this->member['sskin_id'] ? "<option value='{$l['id']}' selected>{$l['name']}</option>"
...
// SergeS * smile skin
 	...
 	function board_prefs() {
		...
		$dst_check = ( $ibforums->member['dst_in_use'] ) ? "checked" : "";
		...
		$lines_count = $this->member['syntax_lines_count'] !== NULL ? $this->member['syntax_lines_count'] : 10;
		foreach (array(0,5,10,15,20,30,50,100,150,200,300,400,500) as $i) {
			$syntax_lines_count .= "<option value='$i' ".($lines_count == $i ? "selected='selected'" : '').">$i</option>\n";
		}
 	...
 	function signature()
 	{
		...
		$data = array(  TEXT          => $this->member['signature'],
                                                           SMILIES       => 1,
                                                           CODE          => 1,
                                                           SIGNATURE     => 0,
...
//echo "required_output =".$required_output."<br>";
	...
	if ( isset($children[ $id ]) and count($children[ $id ]) > 0)
	{
		foreach($children[ $id ] as $r)
		{
			...
			$r['sh'] = ( !isset($fs[ $r['id'] ]) or $fs[ $r['id'] ] or !count($fs) ) ? "checked='checked'" : "";
	...
	foreach($list as $l)
	{
		if ( substr($l,1,1) == "c" ) $cs[ substr($l,2) ] = substr($l,0,1); else
		 if ( substr($l,1,1) == "f" ) $fs[ substr($l,2) ] = substr($l,0,1);
	}
	...
	$DB->query("SELECT id,parent_id,category,read_perms,name FROM ibf_forums ORDER BY position");
	...
	foreach ( $cats as $c )
	{
		...
		{
			$c['qid'] = "c,";
			$c['sh'] = ( $cs[ $c['id'] ] or !count($cs) ) ? "checked='checked'" : "";
	...
	global $ibforums, $DB, $std, $print;
		...
		$DB->query("SELECT id,state FROM ibf_categories");
		...
		$DB->query("SELECT id,category,read_perms FROM ibf_forums WHERE category IN (".implode(",", $cid).")");
		...
		$out = ( $fcnt == $fall and $ccnt == $call ) ? "NULL" : "'".substr($out,0,-1)."'";

		$DB->query("UPDATE ibf_members SET board_layout=$out WHERE id='".$this->member['id']."'");
 	...
 	//*******************************************************************/
      ...
      global $ibforums, $DB, $print;
		...
		$DB->query("UPDATE ibf_members SET profile_delete_time='".$time."' WHERE id='".$this->member['id']."'");
      ...
      global $print, $DB;
		...
		$DB->query("UPDATE ibf_members SET profile_delete_time=0 WHERE id='".$this->member['id']."'");

...
// Song * delete profile link, 04.03.05
 	...
 	function forum_tracker() 
	{
		...
		if ( $ibforums->member['disable_mail'] )
		{
 		...
 		$res_d = $DB->query("SELECT t.frid, t.start_date,
					f.*,
					c.id AS cat_id,
					c.name AS cat_name
	...
	if ($ibforums->vars['bot_antispam'])
	{
		...
		$DB->query("DELETE FROM ibf_reg_antispam
			    WHERE ctime < '$r_date'");
		...
		$DB->query("INSERT INTO ibf_reg_antispam
				({$str['FIELD_NAMES']}) VALUES
				({$str['FIELD_VALUES']})");
	}
	...
	$this->nav        = array( "<a href='".$this->base_url."act=UserCP&amp;CODE=00'>".$ibforums->lang['t_title']."</a>" );
 	...
 	function do_email_change()
 	{
		...
		if ($ibforums->vars['reg_auth_type'])
		{
			...
			$DB->query("INSERT INTO ibf_validating
					({$db_str['FIELD_NAMES']}) VALUES
					({$db_str['FIELD_VALUES']})");
 	...
 	function do_openid_change()
 	{
		...
		if (!preg_match('#^https?://[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,5}(/\S*)?$#', $openid)) {
 		...
 		//--------------------------------------------
		$check = $DB->get_one( "SELECT id
					FROM ibf_members
					WHERE openid_url='".$DB->quote($openid)."'");
...
//echo "required_output =".$required_output."<br>";
	...
	function do_personal() {
 	...
 	function splash() {
		...
		//---------------------------------------------
 		...
 		$DB->query("SELECT COUNT(*) AS msg_total
			    FROM ibf_messages
			    WHERE member_id='".$this->member['id']."'");
 		$total = $DB->fetch_row();
	...
	$DB->query("SELECT id, parent_id, category, read_perms, name
		    FROM ibf_forums
		    ORDER BY position");
	...
	global $ibforums, $DB, $std, $print;
		...
		$DB->query("SELECT id, category, read_perms
			    FROM ibf_forums

...
class UserCP
{
    ...
    function photo()
    {
        ...
        if ($p_max) {
            $this->output = str_replace(
                "<!--IPB.UPLOAD-->",
                View::make("ucp.photo_page_upload", ['max_filesize' => $p_max * 1024]),
                $this->output
            );
        }
    ...
    function email_change($msg = "")
    {
        ...
        if ($ibforums->vars['bot_antispam']) {
            if ($ibforums->vars['bot_antispam'] == 'gd') {
                $this->output = str_replace(
                    "<!--ANTIBOT-->",
                    View::make("ucp.email_change_gd", ['regid' => $regid]),
                    $this->output
                );
            } else {
                $this->output = str_replace(
                    "<!--ANTIBOT-->",
                    View::make("ucp.email_change_img", ['regid' => $regid]),
                    $this->output
                );
            }
        }
    ...
    function avatar()
    {
        ...
        if ($ibforums->vars['avatar_url']) {
            $this->output                  = str_replace(
                "<!--IBF.EXTERNAL_TITLE-->",
                View::make("ucp.avatar_external_title"),
                $this->output
            );
            $this->output                  = str_replace(
                "<!--IBF.URL_AVATAR-->",
                View::make("ucp.avatar_url_field", ['avatar' => $url_avatar]),
                $this->output
            );
            $this->output                  = str_replace("<!--IPB.SIZE-->", $size_html, $this->output);
        ...
        if ($ibforums->member['g_avatar_upload'] == 1) {
            ...
            $this->output                     = str_replace(
                "<!--IBF.UPLOAD_AVATAR-->",
                View::make("ucp.avatar_upload_field"),
                $this->output
            );
            $this->output                     = str_replace("<!--IPB.SIZE-->", $size_html, $this->output);

...
// /Select (No search) functions
	...
	//---------------------------------------
        ...
        $print->do_output( array( 'TITLE' => $this->page_title,
    			 	  'JS'       => ( $this->modfunctions ) ? "rows_js.js?{$ibforums->vars['client_script_version']}" : 0, //Jureth
				  'NAV' => $this->nav ) );
	...
	function show_boolean_explain()
 	{
 		...
 		$print->pop_up_window(  $ibforums->lang['be_link'],
					$this->html->boolean_explain_page() );
...
// vot				}
		...
		{
			if ( is_array( $forum_keys[$cat_id] ) &&
			     count( $forum_keys[$cat_id] ) > 0 )
			{
				$the_html .= $cat_text;
...
// vot    	return " ".preg_quote($words)." ";
	...
	foreach ($ibforums->input as $key => $value)
	{
		if ( preg_match( "/^f_(\d+)$/", $key, $match ) )
		{
			...
			{
				$DB->query("INSERT INTO ibf_search_forums
	...
	global $ibforums, $DB, $std, $print;
		...
		$DB->query(
			"INSERT INTO ibf_search_results
				({$str['FIELD_NAMES']})
			VALUES ({$str['FIELD_VALUES']})");
...
// vot		$query .= " ORDER by t.last_post";
		...
		$res_d = $DB->query("INSERT INTO ibf_search_results
					({$str['FIELD_NAMES']})
				     VALUES ({$str['FIELD_VALUES']})");
 	...
 	function get_replies()
 	{
		...
		if ( !$topics ) $std->Error( array( 'LEVEL' => 1,
						    'MSG' => 'no_search_results' ) );
		...
		if (! isset($ibforums->input['searchid']) )
		{
			...
			$query = "SELECT
				  ...
				  LEFT JOIN ibf_topics t
					ON (p.topic_id=t.tid)
				  WHERE
					p.post_date BETWEEN ".$this->st_stamp.
				"   AND ".$this->end_stamp.
...
//					     WHERE search_date < '$t_time'");
		...
		$this->output .= $this->html->active_start(
						array(
						'SHOW_PAGES' => $links
						) );

...
// Song * delete profile link, 04.03.05
    	...
    	if ( $folder_links ) $menu_html = str_replace( "<!--IBF.FOLDER_LINKS-->", $folder_links, $menu_html );
...
// Song * show friends online, 05.03.05
 		...
 		$this->output .= $this->html->address_edit( array( 'SHOW_ONLINE' => $show_online, 'SELECT' => $html, 'MEMBER' => $memb ) );
...
// Song * friends show online, 05.03.05
 		...
 		$ibforums->input['show_online'] = $ibforums->input['show_online'] == 'yes' ? 1 : 0;
...
// Song * friends show online, 05.03.05
 		...
 		$DB->query("UPDATE ibf_contacts SET contact_desc='".$ibforums->input['mem_desc']."', 
...
// Song * friends show online, 06.03.05
				...
				$row['text'] .= $row['show_online'] ? ") (".$ibforums->lang['show_online'] : "";
...
// Song * friends show online, 06.03.05
 		...
 		} else $this->output .= $this->html->Address_none();
 		...
 		if ( $ibforums->input['MID'] )
 		{
 			if ( preg_match( "/^(\d+)$/", $ibforums->input['MID'] ) )
 			{
 				...
 				if ( $memb['id'] ) $name_to_enter = $memb['name'];
 			}
 		}
...
// Song * friends show online, 06.03.05
	...
	global $ibforums, $DB, $std, $print;
 	...
 	function view_msg() {
 		...
 		if ( !$ibforums->input['MSID'] ) $std->Error( array( 'LEVEL' => 1, 'MSG' => 'no_such_msg' ) );
 		...
 		$DB->query("SELECT m.*, s.id as s_id FROM ibf_messages m 
 		...
 		if ( !$msg['msg_id'] ) $std->Error( array( 'LEVEL' => 1, 'MSG' => 'no_such_msg' ) );
		...
		$member['VID'] = $this->msg_stats['current_id'];
                ...
                if ( $msg['s_id'] ) $online = "<{ONLINE}>"; else $online = "";
	...
	$ibforums->lang = $std->load_words($ibforums->lang, 'lang_post', $ibforums->lang_id);
    	...
    	if ( $preview == 1 )
    	{
    		...
    		$old_msg = $this->parser->prepare( array( TEXT    => $std->remove_tags($ibforums->input['Post']),
							  SMILIES => 1,
							  CODE    => $ibforums->vars['msg_allow_code'],
							  HTML    => $ibforums->vars['msg_allow_html']
							)
	...
	global $ibforums, $DB, $std, $print;
		...
		$this->email = new emailer();
 		...
 		$DB->query("SELECT name, id, view_pop, mgroup, email_pm, language, email, disable_mail FROM ibf_members WHERE ".$query);
		...
		$ibforums->input['Post'] = $this->parser->convert(	 array( 
 		...
 		if ( $ibforums->input['save'] != "" )
 		{
			...
			if ( $ibforums->input['OID'] )
			{
				...
				$DB->query("SELECT msg_id FROM ibf_messages WHERE msg_id='".$ibforums->input['OID']."' AND 
 		...
 		if ($ibforums->member['g_max_mass_pm'] > 0)
 		{
 			...
 			if (isset($ibforums->input['carbon_copy']) and $ibforums->input['carbon_copy'] != "")
 			{
 				...
 				if ( is_array($new_array) and count($new_array) > 0 )
 				{
 					...
 					$DB->query("SELECT m.id, m.name, m.msg_total, m.view_pop, m.email_pm, m.language, m.email, m.disable_mail,
 					...
 					if ( !$DB->get_num_rows() )
 					{
 	 				...
 	 				{
 	 				    	while( $r = $DB->fetch_row() ) $cc_array[$r['id']] = $r;
						...
						$DB->query("SELECT m.name, c.allow_msg FROM ibf_members m, ibf_contacts c 
 		...
 		foreach ($cc_array as $user_id => $to_member)
 		{
			...
			if ( $to_member['email_pm'] == 1 and !$to_member['disable_mail'] )
			{
				...
				$this->email->build_message( array(
								...
								'MSG_BODY' => $std->remove_tags($ibforums->input['Post']) // by Mastilior
								  )
							   );
	...
	global $ibforums, $DB, $std, $print;
 		...
 		if ($ibforums->member['g_max_messages'] > 0)
 		{
 			...
 			if ( $info['img_width'] > 300 ) $info['img_width'] = 300;
 		...
 		if ( $this->vid == 'sent' )
 		{
 			...
 			$DB->query("SELECT m.*, IFNULL(mp.name,'Unknown member') as from_name
			            FROM ibf_messages m 
 		...
 		} else $this->output .= $this->html->No_msg_inbox();
...
// Song * smile skin
		...
		$DB->query("SELECT typed, image from ibf_emoticons WHERE clickable='1' and skid='".$id."'");
...
// Song * smile skin
	...
	function build_contact_list()
	{
		...
		$contacts = "";
 		...
 		} else $contacts = $ibforums->lang['address_list_empty'];

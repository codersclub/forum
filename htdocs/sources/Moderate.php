...
class Moderate {
    ...
    function Moderate()
    {
        ...
        if ($ibforums->input['CODE'] != '04' &&
	    $ibforums->input['CODE'] != '02' &&
	    $ibforums->input['CODE'] != '20' && 
	    $ibforums->input['CODE'] != '22' &&
	    $ibforums->input['CODE'] != '70' &&
	    $ibforums->input['CODE'] != '72' &&
            $ibforums->input['CODE'] != '82' &&
	    $ibforums->input['CODE'] != '18' &&
	    $ibforums->input['CODE'] != '19' && 
	    $ibforums->input['CODE'] != '24' &&
	    $ibforums->input['CODE'] != '25' &&
	    $ibforums->input['CODE'] != '28' && 
	    $ibforums->input['CODE'] != '29' &&
	    $ibforums->input['CODE'] != '33' &&
	    $ibforums->input['CODE'] != '34' &&
	    $ibforums->input['CODE'] != '35' &&
	    $ibforums->input['CODE'] != '36' )
        {
		if ($_POST['act'] == '')
		{
	...
	//-------------------------------------
        ...
        if ( !$this->forum['id'] ) $this->Error( array(
					LEVEL => 1,
					MSG => 'missing_files') );
	...
	function split_complete() {
		...
		if ( !$ibforums->input['title'] ) $this->Error( array(
					'LEVEL' => 1,
					'MSG' => 'complete_form' ) );
		...
		//------------------------------------------
 		...
 		if ( $affected_ids < 1 ) $this->Error( array(
					'LEVEL' => 1,
 		...
 		if ( $affected_ids >= $count['cnt'] ) $this->Error( array(
					'LEVEL' => 1,
					'MSG' => 'split_too_much' ) );
		...
		while ( $post = $DB->fetch_row($res) )
		{
			if ( $txt = preg_replace("#\[code\s*?(=\s*?(.*?)|)\s*\](.*?)\[/code\]#ies", 
						 "\$this->modfunc->regex_code_syntax('\\3', '\\2', ".$post['forum_id'].")", $post['post']) and 
						  $txt != $post['post'] )
			{
	...
	function unsubscribe_all_form() {
		...
		$DB->query("SELECT COUNT(trid) as subbed
			    FROM ibf_tracker
			    WHERE topic_id='".$this->topic['tid']."'");
	...
	function poll_delete_form() {
		...
		$DB->query("SELECT *
	        ...
	        if ( !$poll_data['pid'] ) $this->moderate_error();
...
// vot: BAD MESSAGE
	...
	function poll_edit_do() {
		...
		else if ($this->moderator['edit_post'] )
		{
			$passed = 1;
		}
		else
...
//Jureth		$this->modfunc->forum_recount_queue($moveto);
	...
	function delete_mirror_topic_form() {
		...
		$DB->query("SELECT f.name, f.id FROM ibf_forums f
	...
	function do_add_attach() {
	 global $ibforums, $DB, $std, $print;
		...
		if ( !$id ) $this->Error( array( 'LEVEL' => 1,
						 'MSG' => 'mt_no_topic' ) );
	        ...
	        if ( $DB->get_num_rows() )
		{
			...
			$this->Error( array( 'LEVEL' => 1,
                ...
                $DB->query("INSERT INTO ibf_topiclinks
	...
	function do_delete_attach() {
	 global $ibforums, $DB, $std, $print;
		...
		if ($ibforums->member['g_is_supmod'] != 1) 
		 if ( $this->moderator['can_attach'] == 0 ) $this->moderate_error();

		if ($ibforums->input['linkid'] == "") $this->Error( array( 'LEVEL' => 1, 'MSG' => 'mt_no_topic' ) );
		...
		$DB->query("DELETE FROM ibf_topiclinks
			    WHERE 
	...
	function do_add_data() {
	 global $ibforums, $DB, $std, $print;
		...
		$ibforums->input['tid'] = intval($ibforums->input['tid']);
                ...
                $DB->query("INSERT INTO ibf_topicsinfo
				(tid,name,link,date)
			    VALUES
				('".$ibforums->input['tid']."','".
				addslashes($ibforums->input['name'])."','".
				addslashes($ibforums->input['link'])."','".
				time()."')");
	...
	function do_delete_data() {
	 global $ibforums, $DB, $std, $print;
		...
		$DB->query("DELETE FROM ibf_topicsinfo
...
// vot: BAD MESSAGE
	...
	function attach_form() {
		...
		else if ($this->moderator['can_attach'] == 1) {
			$passed = 1;
		}
		else { $passed = 0; }
	...
	function delete_post() {
		...
		if ($post['new_topic'] == 1) $this->moderate_error('no_delete_post');
	...
	function delete_topic()
	{
		...
		$mirror_topics_q = $DB->query('SELECT tid,forum_id FROM ibf_topics WHERE state=\'mirror\' AND mirrored_topic_id ='.$this->topic['tid']);
...
//+---------------------------------------------------------------------------------------------
	...
	if ( !$fid ) $this->Error( array( 'LEVEL' => 1,
					  'MSG' => 'move_no_source' ) );
...
//		    WHERE id='".$fid."'");
	...
	function html_start_form($additional_tags=array()) {
		...
		$form = "<form name='REPLIER' action='{$this->base_url}' method='POST'>".
	...
	function pin_post() {
		...
		else if ($this->moderator['can_pin_post'] == 1)
		{
			$passed = 1;
		}
		else
	...
	global $std, $ibforums, $DB, $print;
		...
		// Decrease the members post counters
                if ( $this->forum['inc_postcount'] ) foreach ($ids as $mid => $count) 
		{
			$DB->query("UPDATE ibf_members
...
//Jureth:not needed			$this->modfunc->forum_recount_queue($new['forum_id']);
	...
	if ( !$ibforums->input['linkID'] ) $std->Error($error); else
	{
	        switch ( $ibforums->input['CODE'] )
	        {
	        	case '18':
	...
	global $ibforums, $DB, $print, $std, $skin_universal;
	...
	if ( !$pid or !$tid or !$fid ) $this->moderate_error();
	...
	if ($ibforums->input['ajax'] == 'on') {
		...
		$state = ( $state ) ? $std->delayed_time("", $this->forum['days_off'], 1) : 0;
	}
	...
	{
		$url = $ibforums->base_url."act=Mod&CODE=".( ( $state ) ? "29" : "28" )."&f={$this->forum['id']}&t={$this->topic['tid']}&p={$ibforums->input['p']}&auth_key=".$std->return_md5_check();

		$print->redirect_js_screen("{$ibforums->input['linkID']}", ( ( $state ) ? "post_delete2" : "post_delete1"), $url);
	}
	...
	foreach ($ibforums->input as $key => $value)
	{
		if ( preg_match( "/^pozt(\d+)$/", $key, $match ) )
		{
			if ( $ibforums->input[ $match[0] ] ) $source[] = $match[1];
		}
	}

	if ( !count($source) ) $this->moderate_error('no_msg_checked');
	...
	while ( $post = $DB->fetch_row() )
	{
		if ( $post['new_topic'] or $post['forum_id'] != $this->forum['id'] ) continue;

		$ids[] = $post['pid'];
	}

	if ( !count($ids) ) $this->moderate_error('no_msg_checked');
	...
	if ( !$topic = $DB->fetch_row() or !$topic['approved'] )
	{
		$this->moderate_error();
	}
	...
	$update_string = $DB->compile_db_update_string(array('decided' => $state));
	
	$DB->query("UPDATE ibf_topics SET $update_string
		    	WHERE tid='".$this->topic['tid']."'");
	...
	} else
	{
		...
		$print->redirect_js_screen( array('qsolveTop', 'qsolveBottom'), 
					    $label,
					    $url
					  );
	}
	...
	if ( $ibforums->input['p'] )
	{
		...
		if ( !$post = $DB->fetch_row() or $post['forum_id'] != $this->forum['id'] or $post['topic_id'] != $this->topic['tid'] )
		{
			$this->moderate_error();
		}
	...
	} else
	{
		...
		foreach ($ibforums->input as $key => $value)
		{
			if ( preg_match( "/^pozt(\d+)$/", $key, $match ) )
			{
				if ( $ibforums->input[ $match[0] ] ) $arr[] = $match[1];
			}
		}

		if ( !count($arr) ) $this->moderate_error();
		...
		if ( !count($ids) ) $this->moderate_error();
	}
	...
	if ( !$forum = $DB->fetch_row() or !$forum['sub_can_post'] or !$forum['status'] )
	{
	...
	$DB->query("SELECT min(pid) as min
		    FROM ibf_posts
		    WHERE topic_id='".$tid."'");
...
//vot	$this->forum['topics']++;

//vot	$DB->query("UPDATE ibf_forums SET last_title='".addslashes($this->topic['title'])."',
//vot					  last_id='".$this->topic['tid']."',
//vot					  last_post='".$time."',
//vot					  last_poster_name='".addslashes($ibforums->member['name'])."',
//vot					  last_poster_id='".$ibforums->member['id']."',
//vot					  topics='".$this->forum['topics']."' 
//vot		    WHERE id='".$fid."'");

	// Recount topic & posts in the target FAQ forum
	$this->recount($fid);
//Jureth:not needed	$this->modfunc->forum_recount_queue($fid);

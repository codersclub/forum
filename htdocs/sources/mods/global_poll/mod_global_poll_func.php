...
function getOutput() 
{
        ...
        global $ibforums, $std, $DB, $root_path ;
	...
	// We need the user's language:
        $this->lang_id = $ibforums->member['language']==""?($ibforums->vars['default_language']==""?'en':$ibforums->vars['default_language']):$ibforums->member['language'];
        ...
        $this->html = $std->load_template('mod_global_poll_skin');
       		...
       		$DB->query("SELECT starter_id, choices, votes, poll_question, tid
		    FROM ibf_polls
...
function get_poll($poll_data)
{
	...
	global $ibforums, $DB, $std;
       	...
       	$DB->query("SELECT p.author_id,p.author_name,p.post,p.use_emo,
	...
	$header["form_url"] = $ibforums->base_url."act=Poll&amp;t=".$poll_data["tid"];
        ...
        $DB->query("SELECT member_id from ibf_voters WHERE member_id='".$ibforums->member['id']."' and tid='".$ibforums->vars['global_poll']."'");
        $voter = $DB->fetch_row();
        ...
        else
        {
        	...
        	$poll_footer = "<input type='submit' name='submit'   value='{$this->lang['poll_add_vote']}' class='forminput'>&nbsp;".
        	               "<input type='submit' name='nullvote' value='{$this->lang['poll_null_vote']}' class='forminput'>";
        }

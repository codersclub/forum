...
class Forums {
	...
	$ibforums->input['view'] = ( $ibforums->member['show_new'] and $ibforums->input['view'] != "all" ) ? "new" : $ibforums->input['view'];
	...
	$this->main_id = $this->forum['id'];
        ...
        {
        	$DB->query("SELECT f.id as forum_id, f.name as forum_name, f.parent_id, f.read_perms, c.id, c.name 
	...
	// quick jump
	$this->forum['FORUM_JUMP'] = $std->build_forum_jump_topics();

        // Are we viewing the forum, or viewing the forum rules?
        if ( $ibforums->input['act'] == 'SR' ) $this->show_rules(); else
	{
		...
		if ( count($this->forums_id) > 0 and $ibforums->member['show_filter'] and $this->mode_id == -1 )
		{
			...
			foreach ($list as $l) if ( substr($l,1,1) == "f" )
			{
	...
	// Song * RSS, 31.01.05
	$sub = ( $ibforums->member['show_filter'] ) ? "&view=sub" : "";
...
// ******************************************************************
	...
	// querying moderators of current forum and parent forums
        $DB->query("SELECT mid, member_id as mod_id, member_name as mod_name,
...
//			$result .= "<a href='{$ibforums->base_url}showuser={$mod['mod_id']}'>{$mod['mod_name']}</a>, ";
	...
	function show_rules() {
	global $DB, $ibforums, $std, $print, $skin_universal;
	...
	} else $std->Error( array( 'LEVEL' => 1, 'MSG' => 'no_view_topic') );
	...
	global $ibforums, $std, $DB, $print, $skin_universal;
	...
	global $ibforums, $DB, $std, $print;
		...
		$bad_entry = $this->check_access();
				...
				return TRUE;
                	...
                	} else $std->Error( array( LEVEL => 1, MSG => 'no_permission') );
                }
		...
		$Prune = $prune_value != 100 ? (time() - ($prune_value * 60 * 60 * 24)) : -1;
		...
		$DB->query("SELECT COUNT(tid) as max FROM ibf_topics WHERE forum_id{$this->ids} 
		...
		$topic = ( $this->allow_topic() ) ? $topic : "";

		$poll = "<a href='".$this->base_url."act=Post&amp;CODE=10&amp;f=".$this->forum['id']."'><{A_POLL}></a>";
		$poll = ( $topic and $this->forum['allow_poll'] ) ? $poll : "";
		...
		{
			// draw combobox
			$this->forum['filter'] = $std->forum_filter($this->forum, $this->forums_id, $this->mode_id, $this->main_id);
		...
		if ( $ibforums->member['id'] )
		{
			...
			// Song * quick search
			$this->forum['quick_search'] = ( $ibforums->member['quick_search'] ) ? $this->html->quick_search($this->forum) : "";
		...
		if ( $total_possible['max'] < 1 ) $this->output .= $this->html->show_no_matches();
		...
		if ( !$ibforums->member['g_other_topics'] ) $query .= " and starter_id='".$ibforums->member['id']."'";
		...
		{
			// Song * club tool
			if ( $topic['club'] and $std->check_perms( $ibforums->member['club_perms'] ) == FALSE ) continue;
		...
		if ( $ibforums->member['id'] and count($tids) > 0 )
		{
			...
			$DB->query("SELECT pid,topic_id,forum_id,author_id,queued FROM ibf_posts WHERE topic_id IN ({$tids})");
...
//		$this->favs = explode(",", $ibforums->member['favorites']);
		...
		for ( $i = 1; $i <= $total_topics_printed; $i++	) $this->output .= $this->render_entry( $topiclist[$i] );
...
// Song * NEW
		...
		if ( $ibforums->vars['no_au_forum'] != 1 )
		{
			...
			$DB->query("SELECT s.member_id, s.member_name, s.login_type, s.location, s.org_perm_id,
			...
			while ( $result = $DB->fetch_row() )
			{
				...
				{
					$result['g_perm_id'] = ( $result['g_perm_id'] ) ? $result['g_perm_id'].",".$result['org_perm_id'] 
											: $result['org_perm_id'];
				}
				...
				if ( !$result['member_id'] ) $active['guests']++; else
				{
...
// Song * new pages, 14.02.05
			...
			if ( $pages - $page_no > 2 ) for ($i = $pages - 2; $i < $pages; ++$i ) 
			{
				$real_no = $i * $ibforums->vars['display_max_posts'];
...
// Song * link "all"
		...
		if ( !$topic['posts'] ) $topic['posts'] = "<b>{$topic['posts']}</b>";
...
// Song * NEW
		...
		// Topic icon
		$topic['folder_img'] = $std->folder_icon($topic, $this->dots[ $topic['tid'] ], $this->read_array[ $topic['tid'] ], $this->read_mark[ $topic['forum_id'] ]);
		...
		$class = ( $topic['hidden'] ) ? "darkrow3" : "row4";
		$p_start = "";
		$p_end   = "";
...
//	$this->mods[ $topic['old_forum_id'] ][ $ibforums->member['id'] ]
		...
		// Song * favorite topics
               ...
               if ( in_array($topic['tid'], $this->favs) ) $class = "darkrow2";
		...
		if ( $q or ( !$topic['approved'] and $topic['app'] ) )
		{
			...
			if ( !$topic['approved'] ) $topic['queued_link'] = $topic_link; else
			{
				if ( $q ) 
				{
...
// Song * club tool
		...
		if ( $topic['club'] ) $topic['prefix'] = "<span class='clubprefix'>{$ibforums->vars['pre_club']}</span> ";
...
// Song * forum filter, 06.01.05
		...
		if ( $this->mode_id )
		{
			...
			if ( $title ) $topic['forum_title'] = " title='".$ibforums->lang['in_forum'].": $title'";
		}
...
// Song * decided topics, 20.04.05
		...
		if ( $topic['decided'] ) $topic['topic_icon'] = "<{B_DECIDED}>";

// Song * decided topics, 20.04.05

		if ( $topic['pinned'] )
		{
			if ( !$topic['prefix'] ) $topic['prefix'] = "<span class='pinnedprefix'>{$ibforums->vars['pre_pinned']}</span> ";
		...
		} else
		{
			...
			if ( $this->pinned_print ) $this->pinned_print = 0;
...
// Song * NEW
	...
	$DB->query("SELECT fid,tid,logTime FROM ibf_log_topics WHERE mid='".$ibforums->member['id']."' and 
			tid IN ({$tids})");
	...
	if ( !$this->forum['sub_can_post'] or 
	     ...
	     $std->check_perms($this->forum['start_perms'] ) == FALSE ) return 0;

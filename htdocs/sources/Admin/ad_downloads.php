...
class ad_downloads {
	...
	function save_config( $new )
	{
 	...
 	function show_cat( $type = "add" ) {
		...
		while( $row = $DB->fetch_row( ) ) {
			...
			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Description (optional)</b>" ,
													  $SKIN->form_input("cdesc","","text"),
											     )      );

			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want screenshots to automatically display on the category view page for this category?</b><br>--Note: If you have set this globally to Yes or No, this setting is ignored." ,
													  $SKIN->form_dropdown("dis_screen_cat",$s_array,"text"),
											     )		);

			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want screenshots to automatically display on the download page for this category?</b><br>--Note: If you have set this globally to Yes or No, this setting is ignored." ,
													  $SKIN->form_dropdown("dis_screen",$s_array,"text"),
											     )		);

			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want screenshots to be required in this category?</b><br>--Note: If you have set this globally to Yes or No, this setting is ignored." ,
													  $SKIN->form_dropdown("screen_req",$abc_array,"text"),
											     )		);

			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want to accept files in this category before being available for download?</b><br>--Note: If you have set this globally to Yes or No, this setting is ignored." ,
													  $SKIN->form_dropdown("authorize",$as_array,"text"),
											     )		);

			$ADMIN->html .= $SKIN->add_td_row( array( "<b>In which forum do you wish for files submitted to this category to automautically create a topic?</b><br />--Note: If this has a global setting other than \"Per-Category\" this setting will be ignored." ,
												  $SKIN->form_dropdown("fordaforum",$tt_array,"text"),
											    )		);

			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Which groups should have access to this category?</b><br>(NOTE: You can use \"*\" for all groups, or a comma-separated list for more control)" ,
												  $SKIN->form_input("group_perm","","text"),
											    )		);

			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want to show category-specific notes for this category?</b>" ,
												  $SKIN->form_dropdown("show_notes",$sn_array,"text"),
											    )		);


			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Enter your category-specific notes:</b>" ,
													  $SKIN->form_textarea("cnotes",""),
											     )		);



			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want this category to be open?</b>" ,
													  $SKIN->form_dropdown("copen",$a_array,"text"),
											     )		);

			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Choose the category of which this category should be a sub category of.</b><br>If you do not want this category to be sub category then choose 'None'" ,
		...
		elseif( $type == "del" ) {
			...
			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Choose which category to delete</b>" ,
													  $SKIN->form_dropdown("del",$d_array,"text"),
									 	    )      );

			$ADMIN->html .= $SKIN->add_td_row( array( "<b>To which category would you like to transfer the scripts belonging to this category</b>" ,
		...
		elseif( $type == "edit" ) {
			...
			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Choose which category to edit</b>" ,
		...
		while( $row = $DB->fetch_row( ) ) {
			...
			$screen_req = $row['screen_req'] == 1 ? 1 : 0;

			$open = $row['copen'] == 1 ? 1 : 0;
			//+-------------------------------
			...
			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want screenshots to automatically display in the category view page for this category?</b><br>--Note: If you have set this globally to Yes or No, this setting is ignored." ,
			...
			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want to accept files in this category before being available to the public?</b><br>--Note: If you have set this globally to Yes or No, this setting is ignored." ,
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>In which forum do you wish for files submitted to this category to automautically create a topic?</b><br />--Note: If this has a global setting other than \"Per-Category\" this setting will be ignored." ,
			...
			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want to show category-specific notes for this category?" ,
	...
	function do_cat( $type = "add" ) {
		...
		if( $type == "add" ) {
			...
			$DB->query( "SELECT sub FROM ibf_files_cats WHERE cid = '" . $IN['sub'] . "'" );

			if( $DB->get_num_rows( ) > 0 ) {
				$ADMIN->error("The ID specified for the category does not exists or that category is already a sub category...");
			}

			$DB->query( "INSERT INTO ibf_files_cats ( cid , sub , cname , cdesc , copen, dis_screen, dis_screen_cat, screen_req, authorize, fordaforum, show_notes, cnotes, group_perm ) VALUES( '$last_id' , '" . $IN['sub'] . "' , '" . $IN['cname'] . "' , '" . $IN['cdesc'] . "' , '" . $IN['copen'] . "', '" . $IN['dis_screen'] . "', '" . $IN['dis_screen_cat'] . "', '" . $IN['screen_req'] . "', '" . $IN['authorize'] ."', '" . $IN['fordaforum'] ."', '" . $IN['show_notes'] ."', '" . $IN['cnotes'] ."', '" . $IN['group_perm'] ."'  )" );
	...
	function switch_download( ) {
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want to set the download section offline?</b>" ,
	...
	function show_edit_vars(  ) {
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>What is the path to the download directory?</b>" ,
												  $SKIN->form_input("d_download_dir",$INFO['d_download_dir'],"text"),
											    )		);

		$ADMIN->html .= $SKIN->add_td_row( array( "<b>What is the path to the screenshots directory?</b>" ,
												  $SKIN->form_input("d_screen_dir",$INFO['d_screen_dir'],"text"),
											    )		);

		$ADMIN->html .= $SKIN->add_td_row( array( "<b>What is the url to the download directory?</b>" ,
												  $SKIN->form_input("d_download_url",$INFO['d_download_url'],"text"),
											    )		);

		$ADMIN->html .= $SKIN->add_td_row( array( "<b>What is the url to the screenshots directory?</b>" ,
												  $SKIN->form_input("d_screen_url",$INFO['d_screen_url'],"text"),
											    )		);


        $ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want to upload files to the current server?</b>" ,
												  $SKIN->form_yes_no("d_upload",$INFO['d_upload'] ),
											    )		);
											    
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want to have linking downloads?<br>(Download files from external servers... Please do not leech)</b>" ,
												  $SKIN->form_yes_no("d_linking", $INFO['d_linking'] )
									            )      );

		$ADMIN->html .= $SKIN->add_td_row( array( "<b>What must be the maximum size of files that can be uploaded if uploads are allowed?</b><br />Your php.ini upload_max_filesize is set to ".$max_size."<br />In KB(KiloBytes)" ,
												  $SKIN->form_input("d_max_dwnld_size",$INFO['d_max_dwnld_size'],"text"),
											    )		);

		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want to allow uploading of screenshots?</b>" ,
												  $SKIN->form_yes_no("d_screenshot_allowed", $INFO['d_screenshot_allowed'] )
									            )      );
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want to allow linked screenshots?</b>" ,
												  $SKIN->form_yes_no("d_screenshotl_allowed", $INFO['d_screenshotl_allowed'] )
									            )      );


		$ADMIN->html .= $SKIN->add_td_row( array( "<b>What must be the maximum size of screenshots that can be uploaded?</b><br>In KB(KiloBytes)" ,
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>What are the allowable file extensions for files?</b><br>Must be seperated by '|' eg \".txt|.zip\"" ,
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>What are the allowable file extensions for screenshots?</b><br>Must be seperated by '|' eg \".gif|.jpeg\"" ,
	...
	if( $INFO['d_files_perpage'] ==""){
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>What options do you want in the number of files per page dropdown menu?</b><br />Must be separated by '|' eg \"10|20|30\"" ,
												  $SKIN->form_input("d_files_perpage",$pages,"text"),
											    )		);

		$ADMIN->html .= $SKIN->add_td_row( array( "<b>What is the default number of files to show per page?</b>" ,
												  $SKIN->form_input("d_perpage",$INFO['d_perpage'],"text"),
											    )		);


        $ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want Force Download Active?(This increases file security greatly and also renames the files correctly, however doesn't work on all servers.)</b>" ,
												  $SKIN->form_yes_no("d_force",$INFO['d_force'] ),
											    )		);

		$ADMIN->html .= $SKIN->add_td_row( array( "<b>What is the Bandwidth limit?(How fast somebody can download in Kb/s. Force Download must be on. For no limit put 0.)</b>" ,
												  $SKIN->form_input("d_speed",$INFO['d_speed'],"text"),
											    )		);

		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want the script to generate thumbnails for you?</b><br /> " ,
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want to automautically check links to see if they are broken when displaying a file?</b><br />(NOTE: If your host doensn't allow fopen resource to operate outside of your root directory, this won't work)" ,
												  $SKIN->form_yes_no("d_link_check", $INFO['d_link_check'] )
									            )      );
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want to pull file sizes automatically from links?</b><br />(NOTE: If you get slow page loads on large linked files, turn this OFF)" ,
												  $SKIN->form_yes_no("d_link_check1", $INFO['d_link_check1'] )
									            )      );


		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want to use the commenting system?</b><br />(NOTE: It's recommended not to use this in combination with auto-created topics, but instead use one or the other.)" ,
												  $SKIN->form_yes_no("d_use_comments", $INFO['d_use_comments'] )
									            )      );

											    
        $ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you wish to add \"[category name]\" to the topic title?</b><br />This only applies if you set the script to automautically create a topic on new submissions." ,
												  $SKIN->form_yes_no("d_cat_add",$INFO['d_cat_add'] ),
											    )		);

        $ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you wish for administrator-submitted files to auto-accept (this overrides any acceptance configuration currently set)?</b>" ,
												  $SKIN->form_yes_no("d_admin_auto",$INFO['d_admin_auto'] ),
											    )		);
                                                									
        $ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want to show the number of files a member has submitted in topics?</b>" ,
												  $SKIN->form_yes_no("d_topic",$INFO['d_topic'] ),
											    )		);
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want to show the number of downloads a member has downloaded in topics?</b>" ,
												  $SKIN->form_yes_no("d_downloads",$INFO['d_downloads'] ),
												)       );

			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want to show the Top Submitters Box?</b>" ,
												  $SKIN->form_yes_no("d_show_top_up", $INFO['d_show_top_up'] ),
											    )		);

			$ADMIN->html .= $SKIN->add_td_row( array( "<b>How many top submitters would you like to spotlight?</b>" ,
												  $SKIN->form_input("d_how_many_top", $INFO['d_how_many_top'] ),
											    )		);


			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want to show Global Notes?</b><br />Note: You may choose no and still display per-category notes, or you may say yes and show both global and per-category notes." ,
												  $SKIN->form_yes_no("d_show_global_notes", $INFO['d_show_global_notes'] ),
											    )		);

		$return_nl = str_replace("<br />", "\n", $INFO['d_global_notes']);
			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Enter your global notes:</b>" ,
		...
		$ADMIN->html .= $SKIN->add_td_basic( 'If you would like the setting to be for all categories, choose a global answer (eg. yes, no, or a specific topic).<br>&nbsp;&nbsp;If you would like to configure these settings on a per-category basis, choose Per-Category.', 'left', 'catrow2' );
 		...
 		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Do you want to accept files before being available for downloading?" ,
												  $SKIN->form_dropdown("d_authorize",$as_array,$INFO['d_authorize']),
									     )      );

		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Would you like to display screenshots automautically on the category view page?</b>" ,
												  $SKIN->form_dropdown("d_dis_screen_cat",$ds_array,$INFO['d_dis_screen_cat']),
											    )		);

		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Would you like to display screenshots automautically on the download page?</b>" ,
												  $SKIN->form_dropdown("d_dis_screen",$ds_array,$INFO['d_dis_screen']),
											    )		);
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>If you allow screenshots, do you want to make it required?</b>" ,
												  $SKIN->form_dropdown("d_screenshot_required", $ss_array,$INFO['d_screenshot_required'] )
											    )		);

		$ADMIN->html .= $SKIN->add_td_row( array( "<b>If you want a topic to be created for each download submitted then choose the forum name.</b><br>If you do not want a topic created, choose none, or if you'd like to configure this on a per category basis, choose \"Per-Category\"" ,
	...
	function main_screen( ) {
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Individual authors</b>" ,$authors
											    )		);

		$ADMIN->html .= $SKIN->add_td_row( array( "<b>File with highest number of downloads</b>" ,
												  $maxd['sname']
											    )		);

		$ADMIN->html .= $SKIN->add_td_row( array( "<b>File with highest number of views</b>" ,
												  $maxv['sname']
											    )		);

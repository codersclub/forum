...
// Song
		...
		else
		{
			...
			if ( preg_match( "/FULLTEXT KEY/i", $tbl_info['Create Table'] ) )
			{
				...
				$ADMIN->html .= $SKIN->add_td_row(
							...
							$SKIN->form_dropdown( "search_sql_method", array( 0 => array( 'ftext', 'Full Text' ), 1 => array( 'man', 'Manual' ), 2 => array( 'index', 'Indexed' ) ), $INFO['search_sql_method'] )
										 )      );
	...
	function message()
	{
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Turn the message system off?</b>" ,
										  $SKIN->form_yes_no( "is_message_on", $INFO['is_message_on'] )
								 )      );
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>The message to display</b>" ,
...
// Song
    ...
    function reput()
    {
        ...
        $ADMIN->html .= $SKIN->add_td_row( array( "<b>Minimum number of rep points letting member have a 'positive' prefix to their title</b><br>Blank or 0 if you don't want to allow this" ,
    ...
    global $DB, $std;
        ...
        $query = $DB->query("SELECT id FROM ibf_members");

        while ( $user = $DB->fetch_row($query) ) $std->rep_recount($user['id']);
...
// Song
	...
	function url()
	{
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Отсылать ПМ новым пользователям?</b><br>(Если Да, то новые пользователи после регистрации будут получать ПМ с Вашим приветствием)" ,
										  $SKIN->form_yes_no( "auto_pm_on", $INFO['auto_pm_on'] )
								 )      );
								 
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>От кого посылать сообщение??</b><br>(ID пользователя, от которого будет отослано ПМ)<br>(Например, 1 - ID администратора)" ,
										  $SKIN->form_input( "auto_pm_from", $INFO['auto_pm_from'] )
								 )      );
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Заголовок</b><br>(Заголовок отсылаемого сообщения)" ,
										  $SKIN->form_input( "auto_pm_subject", $INFO['auto_pm_subject'] )
								 )      );
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Содержание сообщения</b><br>(Используйте *username*, для обращения к новому пользователю.)<br>(*username* будет заменено на имя пользователя )" ,
	...
	function post()
	{
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Topic prefix for club topics</b>" ,

...
//echo "-------------------------<br>\n";
		...
		if($IN['code'] == 'delete')
		{
			...
			$DB->query("select id, syntax from ibf_syntax_list");
			...
			foreach($syntax_list as $syntax)
			{
				...
				{
					$id = $syntax['id'];
					$DB->query("delete from ibf_syntax_list where id = ".$id);
					$DB->query("delete from ibf_syntax_rules where syntax_id = '".$id."'");
					$DB->query("delete from ibf_syntax_access where syntax_id = '".$id."'");
				}
			}
		}
		...
		if($IN['code'] == 'forums')
		{
			...
			$DB->query("select id, parent_id, redirect_on from ibf_forums");

			while ( $row = $DB->fetch_row() )
			{
				$forum_list[$n++]			= array('id' => $row['id'], 'parent_id' => $row['parent_id'], 'redirect_on' => $row['redirect_on'] );
			}
			...
			foreach($forum_list as $forum)
			{
				...
				} else
				{
					...
					{
						if($IN[ 'forum_'.$forum['parent_id'] ]) $syntax_id= $IN[ 'forum_'.$forum['parent_id'] ];
					}
				}
...
//echo "-------------------------<br>\n";
		...
		$syntax_list			= array();
		$n						= 0;
		$DB->query("select * from ibf_syntax_list");
		...
		foreach ($cats as $c)
		{
			...
			{	
				if ( $r['category'] == $last_cat_id )
				{
					...
					if ( !$syntax_id ) $syntax_id 	= 'null';
					if ( !$off ) $syntax_id 	= 'null';
...
// Song * endless forums, 04.01.05
	...
	if ( !isset($children[$id]) || count($children[$id]) <= 0 ) return;
	...
	foreach ($children[ $id ] as $idx => $rd)
	{
		...
		for ($i = 0; $i < $level; $i++) $t_level_char .= "--";
...
// Song * endless forums, 04.01.05
	...
	function syntax_access () 
	{
		...
		if($IN['code'] == 'grant')
		{
			...
			$DB->query("delete from ibf_syntax_access where member_id = ".$member_id);
			...
			foreach($syntax_list as $syntax)
			{
				if($IN['syntax_'.$syntax['syntax']] == '1')
				{
					$DB->query("insert into ibf_syntax_access (syntax_id, member_id) values (".$syntax['id'].", ".$member_id.")");
				}
			}
		}
		...
		$DB->query("select l.id, l.syntax, l.description, a.member_id from ibf_syntax_list l
		...
		while ($row = $DB->fetch_row())
		{
			...
			if($row['member_id'] == $member_id) $checkbox = "<center><input type='checkbox' name='syntax_{$row['syntax']}' value='1' checked></center>";
			else $checkbox = "<center><input type='checkbox' name='syntax_{$row['syntax']}' value='1'></center>";
	...
	function syntax_edit () 
	{
		...
		if($IN ['code'] == 'create')
		{
			...
			$DB->query ("select max(id) as syntax_id from ibf_syntax_list");
			if($row = $DB->fetch_row())
			{
				$syntax_id		= $row['syntax_id'] + 1;
				
				$DB->query ("insert into ibf_syntax_list (id, syntax, syntax_description, description, back_color, fore_color, tab_length, example) 
					     values (".$syntax_id.", '".$syntax."', '".$syntax_description."', '".$description."', '".
							$back_color."', '".$fore_color."', ".$tab_length.", '".$example."')");
			}
		}
		...
		if($IN ['code'] == 'edit')
		{
			...
			$DB->query ("update ibf_syntax_list set 
					syntax = '".$syntax."', 
		...
		if($syntax_id != 'new')
		{
			$DB->query("select * from ibf_syntax_list where id = ".$syntax_id);
			if ($row = $DB->fetch_row())
			{
				$syntax			= $row['syntax'];

...
class ad_mysql {
	...
	function ad_mysql() {
		...
		switch($IN['code'])
		{
			...
			case 'runsql':
				$q = $_POST['query'] == "" ? $_GET['query'] : $_POST['query'];
	...
	if ( $process_id )
	{
		...
		$DB->query("KILL $process_id",1);
	
		if ( $DB->error )
		{
			$result = $DB->error;

		} else $result = "Query $process_id has been dropped successfully.";
...
// Song * drop query
			...
			if ( $sql == "SHOW FULL PROCESSLIST" )
			{
			        $row = $SKIN->start_form( array( 1 => array( 'act'  , 'mysql' ),
				          2 => array( 'code' , 'drop'),
...
// Song * admin logs
				$ADMIN->html .= $SKIN->add_td_row( array( $table,

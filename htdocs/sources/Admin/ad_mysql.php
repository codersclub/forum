...
class ad_mysql
{
	...
	function __construct()
	{
		...
		switch ($IN['code'])
		{
			...
			case 'processes':
				$this->view_sql("SHOW FULL PROCESSLIST", $_GET['result']);
				break;
	...
	/**
	 * Fields info of the query
	 * Warn! The result of this function is different to db_driver::get_result_fields
	 * @param PDOStatement $stmt
	 * @return array
	 */
	private function get_result_fields($stmt)
	{
		$fields    = [];
	...
	function drop_process($process_id = 0)
	{
		...
		if ($process_id)
		{
			try
			{
				$ibforums->db->query("KILL $process_id", 1);
				$result = "Query $process_id has been dropped successfully.";
	...
	function do_safe_backup($tbl_name = "")
	{
		...
		if ($tbl_name == "")
		{
			$tmp_tbl = $ibforums->db->query('SHOW TABLES;');
	...
	function get_table_sql($tbl, $create_tbl, $skip = 0)
	{
		...
		if ($create_tbl)
		{
			...
			if ($IN['addticks'])
			{
				$stmt = $ibforums->db->query("SHOW CREATE TABLE `" . $INFO['sql_database'] . "." . $tbl . "`");
			} else
			{
				$stmt = $ibforums->db->query("SHOW CREATE TABLE " . $INFO['sql_database'] . "." . $tbl);
			}
		...
		$stmt = $ibforums->db->query("SELECT * FROM $tbl");
	...
	function view_sql($sql, $result = "")
	{
		...
		if ($result)
		{
			$ADMIN->html .= $SKIN->start_table("Drop Query Result");
			$ADMIN->html .= $SKIN->add_td_row(array("<center>" . $result . "</center>"));
			$ADMIN->html .= $SKIN->end_table();
		}

		$map = array(
			'processes' => "SQL Processes",
		...
		} catch (PDOException $e)
		{
			...
			$ADMIN->html .= $SKIN->add_td_row(array($e->getMessage()));
			...
			if (preg_match("/^SELECT/i", $sql))
			{
				...
				if (!preg_match("/LIMIT[ 0-9,]+$/i", $sql))
				{
					...
					if ($rows_returned > $limit)
					{
						...
						$stmt = $ibforums->db->query($sql); /// bypass table swapping
					}
				}
	...
	function list_index()
	{
		...
		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Manual Query</b><br>Advanced Users Only",
		...
		if ($this->mysql_version >= 32303)
		{
			...
			while ($r = $stmt->fetch())
			{
				...
				$iBit = ($r['Index_length'] > 0)
					? 1
					: 0;
				$tBit = ($r['Data_length'] > 0)
					? 1
					: 0;
			...
			$ADMIN->html .= $SKIN->add_td_row(array(
			                                       "&nbsp;",
			                                       "&nbsp;",
			                                       "<div class='right'><b>{$tbl[0]} {$tbl[1]}</b></div>",
			                                       "<div class='right'><b>{$idx[0]} {$idx[1]}</b></div>",
			                                       "<div class='right'>Totals (<b>{$tot[0]} {$tot[1]}</b>)</div>",
		...
		} else
		{
			...
			$tables = $ibforums->db->query('SHOW TABLES;');
			...
			foreach ($tables as $tbl)
			{
				...
				$stmt = $ibforums->db->query("SELECT COUNT(*) AS Rows FROM $tbl");

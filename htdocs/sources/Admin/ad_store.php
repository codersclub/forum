...
if (!defined('IN_ACP')) exit("<h1>Incorrect access</h1> You may not access this file directly.");
...
class ad_store {
	...
	function ad_store() {
		...
		if($IN['code'] == 'itemproperties' && $IN['itemtype']) {
			if($IN['dowhat'] == 1) $IN['code'] = "do_item_delete";
		}
		switch($IN['code'])
		{
			case 'storesettings':
	...
	function dorecount() {
		global $ADMIN,$SKIN,$DB,$MEMBER,$INFO,$IN;
		if($IN['doaction'] == 'pointsrecount') {
			require(ROOT_PATH."/sources/store/edit_check.php");
			if(row_check("ibf_members","post_addon")) {
				$extra = ",post_addon";
			}
			$query = $DB->query("SELECT id,name,posts".$extra.",points FROM ibf_members WHERE posts>0 AND id>0 ORDER BY posts DESC");
			while($user = $DB->fetch_row($query)) {
	...
	function delete_item() {
		...
		$this->check();
		$DB->query("DELETE FROM ibf_store_shopstock WHERE id='{$IN['id']}' LIMIT 1");
		$DB->query("DELETE FROM ibf_store_inventory WHERE item_id='{$IN['id']}' LIMIT 1");
	...
	function item_logs() {
		global $DB,$SKIN,$ADMIN,$IN,$INFO,$MEMBER;
		if($MEMBER['mgroup'] != '4') {
	...
	function clearlog() {
		global $DB,$ADMIN,$SKIN,$IN;
		$DB->query("DELETE FROM ".$IN['type']);
...
//-------------------------------------------------------------
		...
		while($quiz = $DB->fetch_row()) {
			...
			$ADMIN->html .= $SKIN->add_td_row( array("<b>Answer: </b>",
				...
				foreach($answers as $answer) {
					if(!preg_match("#{answer(1|2|3|4)_(0|1):(.+)}#is",$answer,$match)) continue;
	...
	function update_questions() {
		...
		foreach($_POST as $field => $info) {
			if(!preg_match("#mid_(.+?)_type_(.+)#",$field,$match)) continue;
			$mid = $match[1];
			...
			if($match[2] == 'single') {
				...
				if(!$IN['q_'.$mid.'_1_correct']) $IN['q_'.$mid.'_1_correct'] = 0;
				if(!$IN['q_'.$mid.'_2_correct']) $IN['q_'.$mid.'_2_correct'] = 0;
	...
	function quiz_settings() {
		...
		$this->common_header($IN['quiztype'] ? "do_quiz_update" : "do_quiz_settings", 'IBStore Quizs', 'Add a Quiz');
	...
	function do_quiz_settings() {
		...
		if($IN['addtype'] == 'single') {
			$this->addsingle();
		} else if($IN['addtype']) $this->addmulti();
		
		if($IN['action'] == 'finish') {
			$this->finishquiz();
		} else if($IN['action'] == 'multiq' || $IN['action'] == 'dropdown') {
			$this->multi();
		} else $this->single();
	...
	function addsingle() {
		...
		if(!$IN['quiz_question'] || !$IN['answer']) return false; 
	...
	function addmulti() {
		...
		if(!$IN['quiz_question']) return false;
		if(!$IN['answer_1'] && !$IN['answer_2'] && !$IN['answer_3'] && !$IN['answer_4']) return false;
		...
		if(!$IN['correct_1']) $IN['correct_1'] = $correct;
		if(!$IN['correct_2']) $IN['correct_2'] = $correct;
		if(!$IN['correct_3']) $IN['correct_3'] = $correct;
...
*/
		...
		if($IN['itemtype']) $ADMIN->html .= "<input type='hidden' name='itemtype' value='1'>";
		if($IN['itemtype']) $ADMIN->html .= "<input type='hidden' name='id' value='{$item['id']}'>";
		$ADMIN->html .= $SKIN->end_table();
		$this->common_footer();
	}
	function check() {
	...
	function do_stockedits() {
		...
		if(isset($IN['item_waitin'])) {
			...
			} else if($IN['item_waitin'] == 'h') {
				$IN['stock_wait'] = 3600 * $IN['stock_wait'];
			} else if($IN['item_waitin'] == 'd') {
				$IN['stock_wait'] = 86400 * $IN['stock_wait'];
			} else if($IN['item_waitin'] == 'w') {
		...
		if($html = $item->on_add_edits($this)) {
			...
			if($IN['itemtype']) $ADMIN->html .= "<input type='hidden' name='itemtype' value='1'>";
			if($IN['itemtype']) $ADMIN->html .= "<input type='hidden' name='restock_type' value='{$IN['restock_type']}'>";
			//if($IN['itemtype']) $ADMIN->html .= "<input type='hidden' name='protect_groups' value='{$IN['protect_groups']}'>";
			if($IN['itemtype']) $ADMIN->html .= "<input type='hidden' name='id' value='{$IN['id']}'>";
			$ADMIN->html .= $SKIN->start_table("Edits");
			$ADMIN->html .=	$html;
		} else {		
			if($IN['itemtype']) {
				$this->do_stockedits_edit();
			} else $this->do_stockadd_two();
		}
		...
		if ( is_array($new) )
		{
			if ( count($new) > 0 )
			{
				foreach( $new as $field )
				{
					...
					if(in_array($field,$this->make_safe)) $master[$field] = $this->store_lib->make_numsafe($master[$field]);
				}

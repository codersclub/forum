	...
	function do_bulk_mail() {
		...
		if ($IN['preview'] != "")
		{
			$this->bulk_mail_form(1, 'Preview',
									 "<b>".$std->txt_stripslashes($_POST['title'])."</b><br><br>"
...
//vot    	$row['A_STAR'] = str_replace( "<#IMG_DIR#>", $mid['img_dir'], $row['A_STAR'] );
	...
	function delete_form() {
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>The members last visit was over [x] days ago.</b><br>([x] = number entered)<br>(Leave blank to omit from query)" ,
	...
	function doprune() {
		...
		} else $ADMIN->error("Could not find any members that matched the prune criteria");
	...
	function do_edit_form() {
		...
		while( $row = $DB->fetch_row() )
		{
			...
			if ( $row['ftype'] == 'drop' )
			{
				...
				if ( !isset($field_data[ $row['fid'] ]) ) $field_data[ $row['fid'] ] = "0";
...
// Song * reputation
		...
		if ( $IN['rep_recount'] ) $std->rep_recount($IN['mid']);
...
// Song * reputation
	...
	$reason = ( $action and $IN['reason'] and $IN['reason'] != "Enter reason here" ) ? ",disable_mail_reason='".addslashes($IN['reason'])."'" : "";
	...
	$DB->query("SELECT m.id, m.name, m.email, m.posts, g.g_title, 

...
class ad_settings
{
	...
	function upload_form()
	{
		...
		$stmt = $ibforums->db->query("SELECT skname FROM ibf_tmpl_names WHERE skid='" . $IN['setid'] . "'");
	...
	function show_cats()
	{
		...
		$stmt = $ibforums->db->query("SELECT * from ibf_tmpl_names WHERE skid='" . $IN['id'] . "'");
	...
	function add_templates()
	{
		...
		$ibforums->db->insertRow("ibf_tmpl_names", $final);
		...
		{
			$ibforums->db->exec("DELETE FROM ibf_tmpl_names WHERE skid='$new_id'");
	...
	function remove()
	{
		...
		$ibforums->db->exec("DELETE FROM ibf_tmpl_names WHERE skid='" . $IN['id'] . "'");
	...
	function edit_bit()
	{
		...
		foreach ($sec_arry as $id => $template)
		{
			...
			$ADMIN->html .= $SKIN->add_td_basic("<input type='button' value='Macro Look-up' id='editbutton' title='View a macro definition' onClick='pop_win(\"code=macro_one&suid={$template['suid']}\", \"MacroWindow\", 400, 200)'>" . "&nbsp;<input type='button' value='Compare' id='editbutton' title='Compare the edited version to the original' onClick='pop_win(\"act=rtempl&code=compare&suid={$template['suid']}\", \"CompareWindow\", 500,400)'>" . "&nbsp;<input type='button' value='Restore' id='editbutton' title='Restore the original, unedited template bit' onClick='restore(\"{$template['suid']}\",\"{$IN['expand']}\")'>" . "&nbsp;<input type='button' value='View Original' id='editbutton' title='View the HTML for the unedited template bit' onClick='pop_win(\"act=rtempl&code=preview&suid={$template['suid']}&type=html\", \"OriginalPreview\", 400,400)'>" . "&nbsp;<input type='button' value='Search' id='editbutton' title='Search the templates for a string' onClick='pop_win(\"act=rtempl&code=search&suid={$template['suid']}&type=html\", \"Search\", 500,400)'>" . "&nbsp;<input type='button' value='Edit Box Size' id='editbutton' title='Change the size of the edit box below' onClick=\"pop_win('&act=prefs', 'prefs', '300', '100')\">", "center", "catrow");

			$ADMIN->html .= $SKIN->add_td_basic("<b>Show me the HTML code for:&nbsp;" . "<select name='htmlcode' onChange=\"document.theform.res.value='&'+document.theform.htmlcode.options[document.theform.htmlcode.selectedIndex].value+';'\" id='multitext'><option value='copy'>&copy;</option>
		...
		$stmt = $ibforums->db->query("SELECT * from ibf_tmpl_names WHERE skid='" . $set_id . "'");
	...
	function list_current()
	{
		...
		$stmt = $ibforums->db->query("SELECT DISTINCT(s.set_id), s.sname, t.skid, t.skname from ibf_tmpl_names t, ibf_skins s WHERE s.set_id=t.skid ORDER BY t.skname ASC");
		...
		if (count($used_ids) > 0)
		{
			...
			$stmt = $ibforums->db->query("SELECT skid, skname FROM ibf_tmpl_names WHERE skid NOT IN(" . implode(",", $used_ids) . ")");

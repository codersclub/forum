...
class ad_groups
{
	...
	function delete_mask()
	{
		...
		while ($f = $stmt->fetch())
		{
			...
			if (count($d_arr) > 0)
			{
				$ibforums->db->updateRow("ibf_forums", array_map([$ibforums->db, 'quote'], $d_arr), "id={$f['id']}");
			}
		}
		...
		if ($copy_id != 'none')
		{
			...
			if (($new_id > 0) and ($old_id > 0))
			{
				...
				while ($f = $stmt->fetch())
				{
					...
					if (count($d_arr) > 0)
					{
						$ibforums->db->updateRow("ibf_forums", array_map([$ibforums->db], $d_arr), "id={$f['id']}");
					}
				}
			}
	...
	function preview_forums()
	{
		...
		while ($i = $stmt->fetch())
		{
			...
			} else
			{
				...
				{
					if (preg_match("/(^|,)" . $perm_id . "(,|$)/", $i[$code_word]))
					{
						if ($i['parent_id'] > 0)
						{
							$children[$i['parent_id']][] = array($i['forum_id'], "---- {$i['forum_name']}\n");
					...
					} else
					{
						...
						if ($i['parent_id'] > 0)
						{
							$children[$i['parent_id']][] = array(
								$i['forum_id'],
								"<span style='color:gray'>---- {$i['forum_name']}</span>\n"
							);
						} else
						{
		...
		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "Test with...",
		                                       $SKIN->form_dropdown('t', array(
		                                                                      0 => array('start', 'Start Topics'),
	...
	function remove_mask()
	{
		...
		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "Removed the custom permission mask from <b>{$mem['name']}</b>.",
		                                  ));
		...
		while ($r = $stmt->fetch())
		{
			...
			foreach (explode(",", $r['org_perm_id']) as $pid)
			{
				if ($pid == $IN['id'])
				{
					...
					$ADMIN->html .= $SKIN->add_td_row(array(
					                                       "<div style='font-weight:bold;font-size:11px;padding-bottom:6px;margin-bottom:3px;border-bottom:1px solid #000'>{$r['name']}</div>
															   <li>Posts: {$r['posts']}
															   <li>Email: {$r['email']}
															   $extra",
	...
	function permsplash()
	{
		...
		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Permission Mask Name</b>",
		                                       $SKIN->form_input('new_perm_name'),
		                                  ));

		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Inherit forum permission mask from...</b>",
	...
	function save_group($type = 'edit')
	{
		...
		if ($type == 'edit')
		{
			...
			//			$sql     = "UPDATE ibf_groups
			//	    SET $rstring
			//	    WHERE g_id='" . $IN['id'] . "'";
			//			echo $sql . "<br>";

			$ibforums->db->updateRow("ibf_groups", array_map([
			                                                 $ibforums->db,
			                                                 'quote'
			                                                 ], $db_string), "g_id='" . $IN['id'] . "'");
		...
		{
			$ibforums->db->insertRow("ibf_groups", $db_string);
	...
	function group_form($type = 'edit')
	{
		...
		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Use Forum Permission Access...</b><br>You may choose more than one",
		...
		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>POST: Max upload file size (in KB)</b>" . $SKIN->js_help_link('mg_upload') . "<br>(Leave blank to disallow uploads)",
		                                       $SKIN->form_input("g_attach_max", $group['g_attach_max'])
		                                  ));

		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>PERSONAL PHOTO: Max upload file size (in KB)</b><br>(Leave blank to disallow uploads)",
		                                       $SKIN->form_input("p_max", $p_max) . "<br />" . "Max Width (px): <input type='text' size='3' name='p_width' value='{$p_width}'> " . "Max Height (px): <input type='text' size='3' name='p_height' value='{$p_height}'>"
		                                  ));
		...
		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Can email members from the board?</b><br />Leave bottom section blank to remove limits $guest_legend</b>",
		                                       $SKIN->form_yes_no("g_email_friend", $group['g_email_friend']) . "<br />Only allow " . $SKIN->form_simple_input("join_limit", $limit, 2) . " emails in a 24hr period" . "<br />...and only allow 1 email every " . $SKIN->form_simple_input("join_flood", $flood, 2) . " minutes"
		                                  ));
		...
		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Can use signature?",
		                                       $SKIN->form_yes_no("g_use_signature", $group['g_use_signature'])
		                                  ));

		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Avatar file name for this group?",
		                                       $SKIN->form_input("g_use_avatar", $group['g_use_avatar'])
		                                  ));
		...
		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Can change status of own topics ?</b><br>(decied-undecided)</b>",
		                                       $SKIN->form_yes_no("g_use_decided", $group['g_use_decided'])
		                                  ));
		...
		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Can delayed delete own posts?",
		                                       $SKIN->form_yes_no("g_delay_delete_posts", $group['g_delay_delete_posts'])
		                                  ));
		...
		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Can edit own topic title & description?$guest_legend",
		...
		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Can use [doHTML] tag?$guest_legend</b><br />" . $SKIN->js_help_link('mg_dohtml'),
		...
		$SKIN->td_header[] = array("я", "40%");
		$SKIN->td_header[] = array("я", "60%");

		//+-------------------------------
		$ADMIN->html .= $SKIN->start_table("IBStore Settings", "IBStore Settings");

		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Allow this usergroup to?</b><br>Does not apply to Root Admins.",
		                                       $SKIN->form_dropdown('g_fine_edit', array(
		                                                                                0 => array(0, "Do nothing"),
		...
		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Allow this usergroup to edit otheres Inventorys?</b><br>This allows the usergroup to edit othere members inventory.",
		                                       $SKIN->form_yes_no("g_allow_inventoryedit", $group['g_allow_inventoryedit'])
		                                  ));

		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Discount:</b><br>Leave blank or put zero to disable.",

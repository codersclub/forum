...
class ad_sskins {
	...
	function rename_s () {
		...
		$DB->query ( "SELECT `name` FROM ibf_emoticons_skins WHERE id = {$IN['id']}" );

		// А есть ли такой скин ?
		if ($DB->get_num_rows () == 0)
			$ADMIN->error("Invalid skin");

		$t_array = $DB->fetch_row ();
		...
		if ( rename ( ROOT_PATH.'smiles/'.$t_array['name'], ROOT_PATH.'smiles/'.$IN['name'] )) 
			$DB->query ( "UPDATE ibf_emoticons_skins SET `name` = '{$IN['name']}' WHERE `id` = '{$IN['id']}'" );
	...
	function remove () {
		...
		// Наш скин
		$sskin_td = $DB->fetch_row ( );

		// Один чтоб остался
		$DB->query ( "SELECT * FROM ibf_emoticons_skins WHERE id <> {$IN['id']}" );
		if ($DB->get_num_rows() == 0)
			$ADMIN->error("Only one skin left - dont do this");

		// Скин на который заменить %)
		$fallonskin = $DB->fetch_row ( );

		// Меняем смайлоскин юзверов
		$DB->query ( "UPDATE ibf_members SET `sskin_id` = {$fallonskin['id']} WHERE `sskin_id` = {$IN['id']} " );
		// Чистим базу
		$DB->query ( "DELETE FROM ibf_emoticons_skins WHERE id={$IN['id']}");
	...
	function add () {
	      global $DB, $ADMIN, $IN, $INFO, $std, $SKIN;
	      ...
	      // Проверяем название
	      $DB->query ("SELECT id FROM ibf_emoticons_skins WHERE `name` = '{$IN['name']}'");
	      if ( $DB->get_num_rows() > 0 )
	        $ADMIN->error("Set with this name already exists");
        
	      // Добавляем
	      $DB->query("INSERT INTO ibf_emoticons_skins (`name`) VALUES('{$IN['name']}')");
        
	      $new_id = $DB->get_insert_id();
	      
	      // Название первого скина
	      $DB->query ("SELECT name FROM ibf_emoticons_skins WHERE `id` = 1");
	      if (!($name_a = $DB->fetch_row ( )))
	        $ADMIN->error("No skin with id 1");
              
	      // cоздаём диру
	      if ( ! $ADMIN->copy_dir( ROOT_PATH.'smiles/'.$name_a['name'], ROOT_PATH.'smiles/'.$IN['name'] ) ) 
	      {
	        $DB->query("DELETE FROM ibf_emoticons_skins WHERE name='{$IN['name']}'");
	        $ADMIN->error( $ADMIN->errors );
	      }

	      // Последнее - копируем смайлы
	      $res = $DB->query ("SELECT `typed`, `image`, `clickable` FROM ibf_emoticons WHERE `skid` = 1");
	      while ( $line = $DB->fetch_row ( $res )) 
	        $DB->query ( "INSERT INTO ibf_emoticons ( `typed`, `image`, `clickable`, `skid` ) VALUES ( '{$line['typed']}', '{$line['image']}', '{$line['clickable']}', '{$new_id}')" );
	...
	function list_current () {
	      ...
	      $ADMIN->html .= $SKIN->start_table( "Smiles sets" );
              ...
              // Читаем усё
	      $DB->query ( "SELECT * FROM ibf_emoticons_skins ORDER BY `id`" );
	      ...
	      while ( $line = $DB->fetch_row ( ) ) {
	        ...
	        $ADMIN->html .= $SKIN->add_td_row( array(
	            $line['name'], "<center><a href=\"".$SKIN->base_url."&act=sskin&code=edit&id={$line['id']}\">Edit</a>{$remove_str}</center>" 
                ));
	      }
	...
	function doedit_emoticons()
	{
		...
		$DB->query("UPDATE ibf_emoticons SET $db_string WHERE id='".$IN['sid']."'");
	...
	function edit_emoticons()
	{
		...
		$DB->query("SELECT * FROM ibf_emoticons WHERE id='".$IN['sid']."'");
	...
	function remove_emoticons()
	{
		...
		$DB->query("DELETE FROM ibf_emoticons WHERE id='".$IN['sid']."'");
...
//		$ADMIN->nav[] = array( 'act=sskins', 'Smiles sets' );
		...
		// Переименование
		$ADMIN->html .= $SKIN->start_form( array( 1 => array( 'code'  , 'rename'  ),
												  2 => array( 'act'   , 'sskin'   ),
												  3 => array( 'id'    , $IN['id'] ),
									     )      );
		...
		$DB->query("SELECT * from ibf_emoticons WHERE `skid` = '{$IN['id']}' ORDER BY id DESC");

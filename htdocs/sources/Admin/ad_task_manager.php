...
class ad_task_manager {
	...
	function task_log_delete()
	{
		...
		$prune = $ibforums->input['task_prune'] ? $ibforums->input['task_prune'] : 30;
		...
		$DB->simple_exec_query( array( 'delete' => 'task_logs', 'where' => $where ) );
	...
	function task_log_show()
	{
		...
		$limit = $ibforums->input['task_count'] ? $ibforums->input['task_count'] : 30;
		$limit = $limit > 150 ? 150 : $limit;
		
		if ( $ibforums->input['task_id'] != -1 )
		{
			$DB->simple_construct( array( 'select' => '*', 'from' => 'task_logs', 'where' => "log_title='".$ibforums->input['task_id']."'", 'order' => 'log_date DESC', 'limit' => '0,'.$limit ) );
		}
		else
		{
			$DB->simple_construct( array( 'select' => '*', 'from' => 'task_logs', 'order' => 'log_date DESC', 'limit' => '0,'.$limit ) );
		}
		...
		if ( $DB->get_num_rows() )
		{
			...
			{
				$ibforums->html .= $ibforums->adskin->add_td_row( array( "<b>{$row['log_title']}</b>",
	...
	function task_log_setup()
	{
		...
		$DB->simple_construct( array( 'select' => '*', 'from' => 'task_manager', 'order' => 'task_title' ) );
		$DB->simple_exec();
		
		while ( $pee = $DB->fetch_row() )
		{
		...
		$DB->simple_construct( array( 'select' => '*', 'from' => 'task_logs', 'order' => 'log_date DESC', 'limit' => array(0,5) ) );
		...
		$ibforums->html .= $ibforums->adskin->start_form( array( 1 => array( 'act'  , 'task'    ),
		...
		$ibforums->html .= $ibforums->adskin->add_td_row( array( "<b>View logs for task:</b>",
															  $ibforums->adskin->form_dropdown( 'task_id', $tasks )
													 )      );
													 
		$ibforums->html .= $ibforums->adskin->add_td_row( array( "<b>Show how many log entries?</b>",
		...
		$ibforums->html .= $ibforums->adskin->add_td_row( array( "<b>Delete logs for task:</b>",
															  $ibforums->adskin->form_dropdown( 'task_id', $tasks )
													 )      );
													 
		$ibforums->html .= $ibforums->adskin->add_td_row( array( "<b>Delete logs older than (in days)?</b>",
	...
	function run_task()
	{
		...
		$this_task = $DB->simple_exec_query( array( 'select' => '*', 'from' => 'task_manager', 'where' => 'task_id='.$ibforums->input['id'] ) );
		...
		$DB->do_update( 'task_manager', array( 'task_next_run' => $newdate ), "task_id=".$this_task['task_id'] );
	...
	function delete_task()
	{
		...
		$DB->simple_exec_query( array( 'delete' => 'task_manager', 'where' => 'task_id='.$ibforums->input['id'] ) );
	...
	function do_save($type='edit')
	{
		...
		$save = array( 'task_title'       => $ibforums->input['task_title'],
					   ...
					   'task_cronkey'     => $ibforums->input['task_cronkey'] ? $ibforums->input['task_cronkey'] : md5(microtime()),
		...
		if ( $type == 'edit' )
		{
			$DB->do_update( 'task_manager', $save, 'task_id='.$ibforums->input['id'] );
			$ibforums->main_msg = 'Task Edited Successfully';
		}
		else
		{
			$DB->do_insert( 'task_manager', $save );
	...
	function task_form($type='edit')
	{
		...
		if ( $type == 'edit' )
		{
			...
			$this_task = $DB->simple_exec_query( array( 'select' => '*', 'from' => 'task_manager', 'where' => 'task_id='.$id ) ); 
		...
		$ibforums->html .= $ibforums->adskin->add_td_row( array( "<strong>Task Title</strong>",
															   $ibforums->adskin->form_input( 'task_title', $_POST['task_title'] ? $_POST['task_title'] : $this_task['task_title'] )
													 )      );
													 
		$ibforums->html .= $ibforums->adskin->add_td_row( array( "<strong>Task Short Description</strong>",
															   $ibforums->adskin->form_input( 'task_description', $_POST['task_description'] ? $_POST['task_description'] : $this_task['task_description'] )
													 )      );
													 
		$ibforums->html .= $ibforums->adskin->add_td_row( array( "<strong>Task PHP File To Run</strong><div style='color:gray'>This is the PHP file that is run when the task is run.</div>",
															   "./sources/tasks/ ".$ibforums->adskin->form_simple_input( 'task_file', $_POST['task_file'] ? $_POST['task_file'] : $this_task['task_file'], '20' )
													 )      );
													 
		$ibforums->html .= $ibforums->adskin->add_td_row( array( "<strong>Task Time: Minutes</strong><div style='color:gray'>Choose 'Every Minute' to run each minute or a number for a specific minute of an hour</div>",
															   $ibforums->adskin->form_dropdown( 'task_minute', $dd_minute, $_POST['task_minute'] ? $_POST['task_minute'] : $this_task['task_minute'], 'onchange="updatepreview()"' )
													 )      );
													 
		$ibforums->html .= $ibforums->adskin->add_td_row( array( "<strong>Task Time: Hours</strong><div style='color:gray'>Choose 'Every Hour' to run each hour or a number for a specific hour of a day</div>",
															   $ibforums->adskin->form_dropdown( 'task_hour', $dd_hour, $_POST['task_hour'] ? $_POST['task_hour'] : $this_task['task_hour'], 'onchange="updatepreview()"' )
													 )      );
													 
		$ibforums->html .= $ibforums->adskin->add_td_row( array( "<strong>Task Time: Week Day</strong><div style='color:gray'>Choose 'Every Day' to run each day or a week day for a specific week day of a month</div>",
															   $ibforums->adskin->form_dropdown( 'task_week_day', $dd_wday, $_POST['task_week_day'] ? $_POST['task_week_day'] : $this_task['task_week_day'], 'onchange="updatepreview()"' )
													 )      );
													 
		$ibforums->html .= $ibforums->adskin->add_td_row( array( "<strong>Task Time: Month Day</strong><div style='color:gray'>Choose 'Every Day' to run each day or a month day for a specific month day of a month</div>",
															   $ibforums->adskin->form_dropdown( 'task_month_day', $dd_mday, $_POST['task_month_day'] ? $_POST['task_month_day'] : $this_task['task_month_day'], 'onchange="updatepreview()"' )
													 )      );
													 
		$ibforums->html .= $ibforums->adskin->add_td_row( array( "<strong>Enable Task Logging</strong><div style='color:gray'>Will write to the task log each time the task is run, not recommended for regular tasks run every few minutes.</div>",
															   $ibforums->adskin->form_yes_no( 'task_log', $_POST['task_log'] ? $_POST['task_log'] : $this_task['task_log'] )
													 )      );
													 
		$ibforums->html .= $ibforums->adskin->add_td_row( array( "<strong>Enable Task?</strong><div style='color:gray'>If you are using CRON, you might wish to disable this task from the internal manager.</div>",
															   $ibforums->adskin->form_yes_no( 'task_enabled', $_POST['task_enabled'] ? $_POST['task_enabled'] : $this_task['task_enabled'] )
													 )      );
		
		if ( IN_DEV )
		{
			$ibforums->html .= $ibforums->adskin->add_td_row( array( "<strong>Task Key</strong><div style='color:gray'>This is used to call a task where the ID of the task might change</div>",
															         $ibforums->adskin->form_input( 'task_key', $_POST['task_key'] ? $_POST['task_key'] : $this_task['task_key'] )
													        )      );
													        
			$ibforums->html .= $ibforums->adskin->add_td_row( array( "<strong>Task Safe Mode</strong><div style='color:gray'>If set to 'yes', this will not be editable by admins</div>",
															         $ibforums->adskin->form_yes_no( 'task_safemode', $_POST['task_safemode'] ? $_POST['task_safemode'] : $this_task['task_safemode'] )
													        )      );
		}
	...
	function show_tasks()
	{
		...
		$DB->simple_construct( array( 'select' => '*', 'from' => 'task_manager', 'order' => 'task_safemode, task_next_run' ) );
		$DB->simple_exec();
		...
		while ( $row = $DB->fetch_row() )
		{
			...
			$ibforums->html .= $ibforums->adskin->add_td_row( array( "<table cellpadding='0' cellspacing='0' border='0' width='100%'>
																	<tr>
																	 <td width='99%'>

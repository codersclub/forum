...
// Song * infinite subforums, 17.12.04
		...
		$DB->query("SELECT f.id, f.name, f.parent_id FROM ibf_forums f, ibf_categories c 
...
// Song * forums filter, 19.12.04
		...
		$DB->query("DELETE FROM ibf_forums_order WHERE id='".$IN['f']."'");

// Song * forums filter, 19.12.04

// Song * add to faq, 02.05.05

                $DB->query("UPDATE ibf_forums SET faq_id=0 WHERE faq_id='".$IN['f']."'");

// Song * add to faq, 02.05.05

// Song * NEW system
		
		$DB->query("UPDATE ibf_log_forums SET fid='".$IN['MOVE_ID']."' WHERE fid='".$IN['f']."'");

		$DB->query("UPDATE ibf_log_topics SET fid='".$IN['MOVE_ID']."' WHERE fid='".$IN['f']."'");
...
// Song * infinite subforums, 17.12.04
		...
		$DB->query("SELECT id, name from ibf_categories WHERE id > 0 ORDER BY position");
		while ($r = $DB->fetch_row())
		{
			$category[$r['id']] = $r;
		}

		$DB->query("SELECT id as forum_id, subwrap, name as forum_name, subwrap, parent_id, category from ibf_forums ORDER BY position");
		while ($r = $DB->fetch_row())
		{
			if ($r['parent_id'] > 0)
			{
...
// Song * infinite subforums, 17.12.04
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Use syntax highlight ?</b> ((c) Song)" ,
							  $SKIN->form_yes_no("forum_highlight", 0)
							)      
						 );
									     
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Show full image?</b> ((c) Song)<br>(if no link will be shown)" ,
...
// Song
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>How many days delayed messages will be keep</b>" ,
												  $SKIN->form_input("days_off", 5)
									     )      );

		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Allow decided topics in this forum?</b>" ,
												  $SKIN->form_yes_no("decided_button", 1 )
									     )      );

		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Id of forum-FAQ for this forum<br>(Leave this box empty or 0 value if the forum has not FAQ-forum)" ,
...
// Song * infinite subforums, 17.12.04
		...
		while ($r = $DB->fetch_row()) $category[$r['id']] = $r;

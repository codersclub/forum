...
//	$row['ctime'] = $ADMIN->get_date( $row['ctime'], 'LONG' );
	...
	$ADMIN->html .= $SKIN->add_td_row( array(
				"{$row['id']}",
  				"{$row['name']}",
				"{$row['url']}",
  				"<center><b><a href='{$ADMIN->base_url}&act=rss_edit_source&id={$row['id']}'>Edit</a></b>".
				" | <b><a href='{$ADMIN->base_url}&act=rss_del_source&id={$row['id']}&st={$start}'>Delete</a></b>",
  					 )      );
...
*/
  ...
  function del_source()
  {
    ...
    $DB->query("DELETE FROM ibf_rss_sources WHERE id='".$IN['id']."'");

    $redir_url = $ADMIN->base_url."&act=rss_sources";
    if($IN['st']>0) $redir_url .= "&st=".$start;
...
//=================================================
  ...
  function show_channels()
  {
    ...
    $DB->query("SELECT c.*, s.name as source_name
...
//	$row['ctime'] = $ADMIN->get_date( $row['ctime'], 'LONG' );
	...
	$ADMIN->html .= $SKIN->add_td_row( array(
				...
				"{$row['posting_type']}",
				"{$row['is_active']}",
  				"<center><b><a href='{$ADMIN->base_url}&act=rss_edit_channel&id={$row['id']}'>Edit</a></b>".
				" | <b><a href='{$ADMIN->base_url}&act=rss_del_channel&id={$row['id']}&st={$start}'>Delete</a></b>",
  					 )      );
...
//      $row['url']='';
    ...
    $ADMIN->html .= $SKIN->add_td_row( array(
    			"<b>Posting Type</b>" ,
    			$SKIN->form_dropdown('posting_type',
					     $type_array,
					     $row['posting_type']
					    )
    			 )      );
...
*/
  ...
  function del_channel()
  {
    ...
    $DB->query("DELETE FROM ibf_rss_channels WHERE id='".$IN['id']."'");
...
//==============================================
  ...
  function show_logs()
  {
    ...
    $DB->query("SELECT l.*, s.name as source_name
		FROM ibf_rss_logs l
		LEFT JOIN ibf_rss_sources s
			ON (l.source_id=s.id)
		ORDER BY l.id
...
//    $ADMIN->html .= $SKIN->add_td_basic("<center><b><a href='{$ADMIN->base_url}&act=rss_add_channel'>Add New Channel</a></b>");
  ...
  function del_log()
  {
    ...
    $DB->query("DELETE FROM ibf_rss_logs WHERE id='".$IN['id']."'");

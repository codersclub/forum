...
class ad_settings
{
	...
	function add_splash()
	{
		...
		$ibforums->db->insertRow("ibf_templates", $data);
	...
	function export()
	{
		...
		$stmt = $ibforums->db->query("SELECT * from ibf_templates WHERE tmid='" . $IN['id'] . "'");
	...
	function remove()
	{
		...
		$ibforums->db->exec("DELETE FROM ibf_templates WHERE tmid='" . $IN['id'] . "'");
	...
	function save_wrapper($type = 'add')
	{
		...
		if ($type == 'add')
		{
			$ibforums->db->insertRow("ibf_templates", $barney);
		...
		{
			$ibforums->db->updateRow("ibf_templates", array_map([
			                                                    $ibforums->db,
			                                                    'quote'
			                                                    ], $barney), "tmid='" . $IN['id'] . "'");
	...
	function list_wrappers()
	{
		...
		$stmt = $ibforums->db->query("SELECT DISTINCT(w.tmid), w.name, s.sname from ibf_templates w, ibf_skins s WHERE s.tmpl_id=w.tmid ORDER BY w.name ASC");
		...
		if (count($used_ids) > 0)
		{
			...
			$stmt = $ibforums->db->query("SELECT tmid, name FROM ibf_templates WHERE tmid NOT IN(" . implode(",", $used_ids) . ")");

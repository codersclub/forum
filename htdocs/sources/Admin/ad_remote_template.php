...
class ad_settings
{
		...
		print "<html>
				 <head><title>Search</title></head>
				   <frameset cols='200, *' frameborder='no' border='1' framespacing='0'>
					<frame name='links' scrolling='auto' src='{$SKIN->base_url}&act=rtempl&code=csssearchlinks&id={$IN['id']}&element={$_GET['element']}'>
	...
	function css_preview()
	{
		...
		$stmt = $ibforums->db->query("SELECT img_dir FROM ibf_skins WHERE css_id='" . $IN['id'] . "'");
	...
	function print_css_searchlinks()
	{
		...
		$stmt = $ibforums->db->query("SELECT set_id FROM ibf_skins WHERE css_id='" . $IN['id'] . "'");
		...
		$stmt = $ibforums->db->query("SELECT suid, set_id, group_name, func_name FROM ibf_skin_templates WHERE set_id='" . $set['set_id'] . "' AND section_content LIKE '%" . $like . "%' ORDER BY group_name");
	...
	function print_searchlinks()
	{
		...
		$stmt = $ibforums->db->query("SELECT set_id FROM ibf_skin_templates WHERE suid='" . $IN['suid'] . "'");

		$set = $stmt->fetch();

		$stmt = $ibforums->db->query("SELECT suid, set_id, group_name, func_name FROM ibf_skin_templates WHERE set_id='" . $set['set_id'] . "' AND section_content LIKE '%$search_text%' ORDER BY group_name");
		...
		foreach ($result as $group_name => $sub_array)
		{
			...
			if (is_array($sub_array) and count($sub_array) > 0)
			{
				foreach ($sub_array as $idx => $data)
				{
					$ADMIN->html .= $SKIN->add_td_row(array("+ <a href='{$SKIN->base_url}&act=rtempl&code=preview&suid={$data['suid']}&type=text&hl=" . urlencode(stripslashes($_POST['search'])) . "' target='preview'>{$data['func_name']}</a>"));
				}
			}
		}
	...
	function print_search_box()
	{
		...
		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Enter text to search for</b><br>" . $SKIN->form_input('search')
		                                  ));
	...
	function print_compare_new()
	{
		...
		$stmt = $ibforums->db->query("SELECT * FROM ibf_skin_templates WHERE suid='" . $IN['suid'] . "'");
	...
	function macro_one()
	{
		...
		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Enter the Macro to look up</b><br>(EG: {ibf.skin.tbl_width})",
	...
	function macro_two()
	{
		...
		{
			if (preg_match("/{ibf\.(skin|lang|vars|member)\.(\w+)}$/", $IN['lookup'], $match))
			{
				$is_valid = 1;
				$macro    = $match[2];
				$extra    = $match[1];
			...
			} else
			{
				...
				} else
				{
					if ($extra == 'lang')
					{
						...
						$stmt = $ibforums->db->query("SELECT group_name FROM ibf_skin_templates WHERE suid='{$IN['suid']}'");
		...
		} else
		{
			...
			$stmt = $ibforums->db->query("SELECT macro_id FROM ibf_skins WHERE set_id='" . $template['set_id'] . "'");

			$macrod = $stmt->fetch();

			$stmt = $ibforums->db->query("SELECT macro_value, macro_replace FROM ibf_macro WHERE macro_set='" . $macrod['macro_id'] . "' AND macro_value='$macro'");
	...
	function do_preview()
	{
		...
		$stmt = $ibforums->db->query("SELECT macro_value, macro_replace FROM ibf_macro WHERE macro_set='" . $macrod['macro_id'] . "'");
		...
		if ($IN['type'] == 'text')
		{
			...
			if ($_GET['hl'] != "")
			{
				$hl = urldecode(stripslashes($_GET['hl']));

...
class ad_multimod
{
	...
	function do_save($type = 'new')
	{
		...
		if ($type == 'edit')
		{
			...
			$ibforums->db->updateRow("ibf_topic_mmod", array_map([$ibforums->db, 'quote'], $save), "mm_id=$mm_id");

		} else
		{
			$ibforums->db->insertRow("ibf_topic_mmod", $save);

			$mm_id = $ibforums->db->lastInsertId();
		}
		...
		while ($r = $stmt->fetch())
		{
			...
			{
				if (stristr($r['topic_mm_id'], ',' . $mm_id . ','))
				{
					// Should it still be?
	...
	function do_form($type = 'new')
	{
		...
		foreach ($db_forums as $an_eye_for_an => $i)
		{
			...
			if (($i['subwrap'] == 1) and (count($db_children[$i['forum_id']]) > 0))
			{
				...
				foreach ($db_children[$i['forum_id']] as $idx => $cdata)
				{
					...
					$forum_jump[] = array(
						$cdata['forum_id'],
						"&middot;&middot;&middot;&middot;&nbsp;{$cdata['forum_name']}"
					);
				}
			}
		}
		...
		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Title for this Multi-Moderation Suite?</b>",
		                                       $SKIN->form_input("mm_title", $topic_mm['mm_title'])
		                                  ));

		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Active in Forums...</b><br>You may choose more than one",
		                                       $forums
		                                  ));
		...
		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Add to <i>START</i> of topic title?</b>",
		                                       $SKIN->form_input("topic_title_st", $topic_mm['topic_title_st'])
		                                  ));

		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Add to <i>END</i> of topic title?</b>",
		                                       $SKIN->form_input("topic_title_end", $topic_mm['topic_title_end'])
		                                  ));

		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Alter topic state?</b>",
		                                       $SKIN->form_dropdown("topic_state", $state_dd, $topic_mm['topic_state'])
		                                  ));

		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Alter pinned state?</b>",
		                                       $SKIN->form_dropdown("topic_pin", $pin_dd, $topic_mm['topic_pin'])
		                                  ));

		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Move topic?</b>",
		                                       $SKIN->form_dropdown("topic_move", array_merge(array(
		                                                                                           0 => array(
			                                                                                           '-1',
			                                                                                           'Don\'t Move'
		                                                                                           )
		                                                                                      ), $forum_jump), $topic_mm['topic_move']) . "<br />" . $SKIN->form_checkbox('topic_move_link', $topic_mm['topic_move_link']) . "<strong>Leave a link to the source topic?</strong>"
		                                  ));

		$ADMIN->html .= $SKIN->add_td_basic('Post Options', 'left', 'pformstrip');

		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b>Add a reply to the topic?</b><br>HTML enabled",
		                                       "Enable this reply? &nbsp;" . $SKIN->form_yes_no('topic_reply', $topic_mm['topic_reply']) . "<br />" . $SKIN->form_textarea("topic_reply_content", $topic_mm['topic_reply_content']) . "<br />" . $SKIN->form_checkbox('topic_reply_postcount', $topic_mm['topic_reply_postcount']) . "<strong>Increment poster's post count?</strong>"
		                                  ));

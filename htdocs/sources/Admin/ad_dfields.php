...
class ad_dfields
{
    ...
    {
        global $IN, $INFO, $SKIN, $ADMIN, $std, $MEMBER, $GROUP, $HTTP_POST_VARS;
        $ibforums = Ibf::app();
        ...
        if ($type == 'edit') {
            $ibforums->db->updateRow("ibf_files_custfields", array_map([
                                                                       $ibforums->db,
                                                                       'quote'
                                                                       ], $data), "fid='" . $IN['id'] . "'");
            $ADMIN->done_screen("Custom Download Field Edited", "Custom Download Field Control", "act=dfield");
        } else {
            $ibforums->db->insertRow("ibf_files_custfields", $data);
    ...
    function main_form($type = 'edit')
    {
        ...
        $ADMIN->html .= $SKIN->add_td_row(array(
                                               "<b>Field should be required?</b>",
                                               $SKIN->form_yes_no("freq", $fields['freq'])
                                          ));

        $ADMIN->html .= $SKIN->add_td_row(array(
                                               "<b>Do you want this field to be displayed for editing?</b><br />Please be smart and don't require a field but not show it...all submissions will receive an error if this happens",
                                               $SKIN->form_yes_no("fshow", $fields['fshow'])
                                          ));

        $ADMIN->html .= $SKIN->add_td_row(array(
                                               "<b>If you choose to automautically create topics on new submissions, do you want this field to be included in the topic?</b>",

...
class ad_settings
{
	...
	function edit_set_name()
	{
		...
		$stmt = $ibforums->db->query("SELECT * FROM ibf_macro WHERE macro_set='" . $IN['id'] . "'");
		...
		$ibforums->db->exec("UPDATE ibf_macro_name SET set_name='{$IN['setname']}' WHERE set_id='" . $IN['id'] . "'");
	...
	function macro_remove()
	{
		...
		$stmt = $ibforums->db->query("SELECT * FROM ibf_macro WHERE macro_id='" . $IN['mid'] . "'");

		$row = $stmt->fetch();

		$ibforums->db->exec("DELETE FROM ibf_macro WHERE macro_id='" . $IN['mid'] . "'");
	...
	function macro_edit()
	{
		...
		$key = str_replace("'", "\\'", $std->txt_safeslashes($_POST['macro_value']));
		$val = str_replace("'", "\\'", $std->txt_UNhtmlspecialchars($std->txt_safeslashes($_POST['macro_replace'])));

		$ibforums->db->exec("UPDATE ibf_macro SET macro_value='$key', macro_replace='$val' WHERE macro_id='" . $IN['mid'] . "'");
	...
	function macro_add()
	{
		...
		$stmt = $ibforums->db->query("SELECT * FROM ibf_macro WHERE macro_set='" . $IN['id'] . "' LIMIT 0,1");
		...
		$data = [
			'macro_value'   => $std->txt_safeslashes($_POST['macro_value']),
			'macro_replace' => $std->txt_UNhtmlspecialchars($std->txt_safeslashes($_POST['macro_replace'])),
			'can_remove'    => 1,
	...
	function macro_form($type = 'edit')
	{
		...
		if ($type == 'edit')
		{
			...
			$stmt = $ibforums->db->query("SELECT m.*, mn.set_name from ibf_macro m, ibf_macro_name mn WHERE macro_id='" . $IN['mid'] . "' AND mn.set_id=m.macro_set");
		...
		} else
		{
			...
			$stmt = $ibforums->db->query("SELECT set_name from ibf_macro_name WHERE set_id='" . $IN['id'] . "'");
	...
	function export()
	{
		...
		$stmt = $ibforums->db->query("SELECT macro_replace, macro_value FROM ibf_macro WHERE macro_set='" . $IN['id'] . "'");
	...
	function add_macro_set()
	{
		...
		$stmt = $ibforums->db->query("SELECT * FROM ibf_macro_name WHERE set_id='" . $IN['id'] . "'");
		...
		$stmt = $ibforums->db->query("SELECT macro_value, macro_replace FROM ibf_macro WHERE macro_set='{$IN['id']}'");
		...
		while ($row = $stmt->fetch())
		{
			...
			$q2 = $ibforums->db->insertRow("ibf_macro", $data);
			//$q2 = $ibforums->db->exec("INSERT INTO ibf_macro SET macro_value='{$row['macro_value']}',macro_replace='{$row['macro_replace']}', macro_set='$new_id'");
		}

		$ibforums->db->exec("INSERT INTO ibf_macro_name SET set_id='$new_id', set_name='{$mac['set_name']}'");
	...
	function remove()
	{
		...
		$ibforums->db->exec("DELETE FROM ibf_macro WHERE macro_set='" . $IN['id'] . "'");

		$ibforums->db->exec("DELETE FROM ibf_macro_name WHERE set_id='" . $IN['id'] . "'");
	...
	function show_macros()
	{
		...
		$stmt = $ibforums->db->query("SELECT m.*, ms.* from ibf_macro m, ibf_macro_name ms WHERE m.macro_set='" . $IN['id'] . "' AND ms.set_id=m.macro_set LIMIT 0,1");
	...
	function list_current()
	{
		...
		$stmt = $ibforums->db->query("SELECT DISTINCT(s.macro_id), i.macro_set, ms.set_name, s.sname from ibf_macro i, ibf_macro_name ms, ibf_skins s WHERE s.macro_id=i.macro_set  AND ms.set_id=i.macro_set ORDER BY ms.set_name ASC");
		...
		if ($stmt->rowCount())
		{
			...
			while ($r = $stmt->fetch())
			{
				...
				$ADMIN->html .= $SKIN->add_td_row(array(
				                                       "<b>" . $std->txt_stripslashes($r['set_name']) . "</b>",
				                                       "<#X-{$r['macro_set']}#>",
		...
		if (count($used_ids) > 0)
		{
			...
			$stmt = $ibforums->db->query("SELECT set_id, set_name FROM ibf_macro_name WHERE set_id NOT IN(" . implode(",", $used_ids) . ")");
		...
		$ADMIN->html .= $SKIN->add_td_row(array(
		                                       "<b><u>OR</u> Choose a file from your computer to import</b><br>Note: This must be a valid macro set.",
		                                       $SKIN->form_upload(),
		                                  ));

...
class Post
{
    ...
    function __construct()
    {
        ...
        if ($row['mod_posts']) {
            if ($row['mod_posts'] == 1) {
                ...
                if (time() >= $mod_arr['date_end']) {
                    $ibforums->db->exec("UPDATE ibf_preview_user
						    SET mod_posts=NULL
						    WHERE
        ...
        if ($this->act[0]) {
            ...
            if (!$ibforums->member['id']) {
                ...
                $ibforums->input['UserName'] = $ibforums->input['UserName']
    ...
    function parse_post_mail($post = '', $poster = 0, $mgroup = 0)
    {
        ...
        if ($ibforums->member['id'] != $poster) {
            $post = preg_replace("#\[gm\](.+?)\[/gm\]#is", "", $post);
        }
    ...
    function compile_post()
    {
        ...
        $ibforums->input['enablesig']   = $ibforums->input['enablesig'] == 'yes'
            ? 1
            : 0;
        $ibforums->input['enableemo']   = $ibforums->input['enableemo'] == 'yes'
            ? 1
            : 0;
        $ibforums->input['enabletrack'] = $ibforums->input['enabletrack'] == 1
            ? 1
            : 0;
        ...
        $convert = $this->parser->convert(array(
        ...
        $is_new_post = in_array($this->act[1], array('new_post', 'reply_post', 'q_reply_post'));
        $post        = array(
    ...
    function process_upload()
    {
        ...
        foreach ($attachments as $i => $a) {
            ...
            $real_file_name = sprintf("post-%d-%d-%d%s", $this->forum['id'], time(), $i, Attachment::createExtensionByType($a->type()));
    ...
    function upload_log_typefails($real_file_name)
    {
        ...
        $line = sprintf("date:'%s' user:'%s' filename:'%s' userfilename:'%s' usertype:'%s' detected:'%s'\n", date('r'), $user_name, $real_file_name, $user_file_name, $user_file_type, $detected_file_type);
        ...
        try {
            ...
            $block   = 1;
            $i       = 0;
            // если требуется блокировка, пробуем 5 раза с интервалом 50мсек
            // если не получается, забиваем это дело
    ...
    function html_checkboxes($type = "", $tid = "", $use_emo = "", $offtop = "")
    {
        ...
        } elseif ($use_emo != "" and !$use_emo) {
            $default_checked['emo'] = "";
        }

        $this->output = str_replace(
            '<!--IBF.EMO-->',
            View::make("post.get_box_enableemo", ['checked' => $default_checked['emo']]),
            $this->output
        );
        ...
        if ($ibforums->member['id']) {
            if ($type == "reply") {
                ...
                $this->output = str_replace(
                    ...
                    View::make("post.add_merge_edit_box", ['checked' => $default_checked['merge']]),
                    $this->output
                );
                ...
                $this->output = str_replace(
                    ...
                    View::make("post.add_edit_box", ['checked' => $default_checked['edit']]),
                    $this->output
                );
            }
            ...
            if ($type != "edit") {
                ...
                if ($tid and $cnt > 0) {
                    $this->output = str_replace(
                        '<!--IBF.TRACK-->',
                        View::make("post.get_box_alreadytrack"),
                        $this->output
                    );
                } else {
                    $this->output = str_replace(
                        '<!--IBF.TRACK-->',
                        View::make("post.get_box_enabletrack", ['checked' => $default_checked['tra']]),
                        $this->output
                    );
                }
            }
            ...
            if (!$type) {
                ...
                $this->output = str_replace(
                    '<!--IBF.FAV-->',
                    View::make("post.get_box_enablefav", ['checked' => $default_checked['fav']]),
                    $this->output
                );
                ...
                if ($this->forum['days_off'] and
                    ...
                    $this->output = str_replace(
                        '<!--IBF.OFFTOP-->',
                        View::make("post.get_box_enable_offtop", ['checked' => $default_checked['offtop']]),
                        $this->output
                    );
                }
                ...
                if ($type == "edit" and ($this->moderator['edit_post'] or $ibforums->member['g_is_supmod'])) {
                    ...
                    $this->output = str_replace(
                        '<!--IBF.BUMP-->',
                        View::make("post.get_box_bump", ['checked' => $default_checked['bump']]),
                        $this->output
                    );
                }
            }
    ...
    function html_topic_summary($topic_id)
    {
        ...
        if (!$ibforums->member['show_history']) {
            return;
        }
        ...
        while ($row = $stmt->fetch()) {
            ...
            $row['post'] = $this->parser->post_db_parse($row['post'], $this->forum['use_html'] and $ibforums->member['g_dohtml']
                ? 1
                : 0);
    ...
    function moderate_log($title = 'unknown', $topic_title)
    {
        ...
        $stmt = $ibforums->db->insert("ibf_moderator_logs", $data);
    }
}

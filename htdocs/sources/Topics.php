...
class Topics
{
    ...
    function process_one_post($row, $pinned, &$post_count, $qr, $preview = false)
    {
        ...
        if ($ibforums->vars['post_wordwrap'] > 0 and !$row['use_sig']) {
            ...
            if ($ibforums->member['syntax'] != 'none' and
                ...
                $row['post'] = preg_replace_callback(
                    '#\[code\s*?(?:=\s*?(.*?)|)\s*\](.*?)\[/code\]#is',
                    function ($a) {
    ...
    function __construct()
    {
        ...
        if ($ibforums->vars['no_au_topic'] != 1) {
            ...
            $this->output = str_replace(
                "<!--IBF.TOPIC_ACTIVE-->",
                View::make("topic.topic_active_users", ['active' => $active]),
                $this->output
            );
        }
    ...
    function html_checkboxes($tid = "")
    {
        ...
        $this->output = str_replace(
            '<!--IBF.EMO-->',
            View::make("topic.get_box_enableemo", ['checked' => $default_checked['emo']]),
            $this->output
        );
        ...
        if ($this->trid) {
            ...
            $this->output = str_replace(
                '<!--IBF.TRACK-->',
                View::make("topic.get_box_enabletrack", ['checked' => $default_checked['tra']]),
                $this->output
            );
        }
        ...
        if ($this->forum['days_off'] and ($this->moderator['delete_post'] or $ibforums->member['g_is_supmod'] or $ibforums->member['g_delay_delete_posts'])) {
            $this->output = str_replace(
                '<!--IBF.OFFTOP-->',
                View::make("topic.get_box_enable_offtop", ['checked' => $default_checked['offtop']]),
                $this->output
            );
        }
    }
...
//              $member['title'] .= "<br>";
    ...
    function multi_moderation()
    {
        ...
        if ($ibforums->member['id']) {
            if ($ibforums->member['g_is_supmod']) {
                $pass_go = true;
            } elseif ($this->moderator['can_mm'] == 1) {
                $pass_go = true;
            }
        }
    ...
    function parse_poll()
    {
        ...
        $html = str_replace(
            "<!--IBF.POLL_JS-->",
            View::make("poll.poll_javascript", ['tid' => $this->topic['tid'], 'fid' => $this->forum['id']]),
            $html
        );

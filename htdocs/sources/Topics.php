...
// Song * correct cached fields for new post id
		...
		if ( ($row['attach_id'] or $row['attach_exists'])and !$row['use_sig'] ) {
			...
			$row['attachment'] = is_array($row['attachment']) ? "<br>".implode("<br>", $row['attachment']) : '';
			unset($attachments);
		}
...
//		$favs = explode(",",$ibforums->member['favorites']);
	     ...
	     $std->user_reply_flood($this->topic['start_date']) == FALSE )
	{
		$show = "none";
	        ...
	        if ( $ibforums->member['quick_reply'] ) $show = "show";
	...
	function moderation_panel() {
		...
		$actions = array( 'MOVE_TOPIC', 'CLOSE_TOPIC', 'OPEN_TOPIC',
				  ...
				  'SHOW_TOPIC',
				'MIRROR_TOPIC', 'DELETE_TOPIC_MIRROR'
				);

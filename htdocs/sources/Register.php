...
class Register
{
    ...
    function lost_password_start($errors = "")
    {
        ...
        if ($ibforums->vars['bot_antispam'] == 'gd') {
            $this->output = str_replace(
                "<!--{REG.ANTISPAM}-->",
                View::make("register.bot_antispam_gd", ['regid' => $regid]),
                $this->output
            );
        } elseif ($ibforums->vars['bot_antispam'] == 'gif') {
            $this->output = str_replace(
                "<!--{REG.ANTISPAM}-->",
                View::make("register.bot_antispam", ['regid' => $regid]),
                $this->output
            );
        }
        // TODO: add recaptcha
    ...
    function show_reg_form($errors = "")
    {
        ...
        if ($ibforums->vars['bot_antispam'] == 'gd') {
            ...
            $this->output = str_replace(
                "<!--{REG.ANTISPAM}-->",
                View::make("register.bot_antispam_recapthca", ['regid' => $regid]),
                $this->output
            );
        }
    ...
    function show_manual_form($type = 'reg')
    {
        ...
        if ($type == 'lostpass') {
            ...
            if ($ibforums->input['uid'] and $ibforums->input['aid']) {
                ...
                $this->output = str_replace(
                    "<!--IBF.INPUT_TYPE-->",
                    View::make("register.show_lostpass_form_auto", ['aid' => $in_validate_key, 'uid' => $in_user_id]),
                    $this->output
                );
            } else {
                $this->output = str_replace(
                    "<!--IBF.INPUT_TYPE-->",
                    View::make("register.show_lostpass_form_manual"),
                    $this->output
                );
            }
        } else {

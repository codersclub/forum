...
class Register {
	...
	$DB->query("SELECT name, id, email, mgroup
		    FROM ibf_members
...
// Song * look at died accounts
	...
	$DB->query("INSERT INTO ibf_check_members
...
//					 'real_group'  => $ibforums->vars['member_group'],
	...
	global $ibforums, $std, $DB, $print;
		...
		$DB->query("SELECT id, name, password, email
			    FROM ibf_members
			    WHERE id=$in_user_id");
...
//echo "this_number=".$this_number."<br>";
	...
	function bash_dead_validations()
	{
		...
		if ( intval($ibforums->vars['validate_day_prune']) > 0 )
		{
			...
			$DB->query("SELECT v.vid, v.member_id,
					m.posts

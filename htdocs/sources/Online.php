...
class Online {
	...
	function list_all() {
		...
		$DB->query("SELECT s.id, s.in_forum, s.in_topic, s.member_name, s.member_id, s.ip_address, s.running_time, 
		...
		if ( count($tid_array) > 0 )
		{
			...
			$DB->query("SELECT tid, title, club FROM ibf_topics WHERE tid IN ($tid_string)");
			
			while ( $t = $DB->fetch_row() )
			{
				if ( $t['club'] and $std->check_perms( $ibforums->member['club_perms'] ) == FALSE ) continue;
		...
		foreach( $final as $idx => $sess )
		{
			...
			if ( isset($sess['location']) )
			{
				...
				do
				{
					...
					if ( isset($act)) $line .= isset($this->where[ $act ]) ? $this->where[ $act ] : $ibforums->lang['board_index'];
					...
					if ($fid != "" and ($act == 'SF' or $act == 'ST' or $act == 'Post'))
					{
						...
						if ( $pass == 1 and $tid > 0 and !$topics[$tid] ) $tid = 0;

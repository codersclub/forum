...
//	}
	...
	$DB->query("UPDATE ibf_members SET points=points+'{$member['points']}' WHERE id='{$member['id']}' LIMIT 1");
	//vot: log added
...
//	$lib->write_log("Поощрён участник ".$ibforums->input['username']." на ".$ibforums->input['fine_amount'].' '.$ibforums->vars['currency_name'],"Fine User");
	...
	$DB->query("UPDATE ibf_members SET points='{$reciver['points']}' WHERE id='{$reciver['id']}' LIMIT 1");
	$DB->query("UPDATE ibf_members SET points='{$ibforums->member['points']}' WHERE id='{$ibforums->member['id']}' LIMIT 1");
...
//	    $lib->write_log("Послан товар ".$send['item_name']." участнику ".$ibforums->input['username'],"donate_i");
	...
	$DB->query("SELECT 1 FROM ibf_store_shopstock WHERE id>'0' ".$extra);
	...
	$list = $DB->query("SELECT * FROM ibf_store_shopstock WHERE id>'0' ".$extra." AND avalible='0' ORDER BY item_name DESC ".$limit_extra);
...
//	require($ibforums->vars['base_dir']."sources/store/rules.php");
	...
	return $link;
		...
		function do_take_quiz() {
			global $ibforums, $DB, $std, $print,$lib,$HTTP_POST_VARS;

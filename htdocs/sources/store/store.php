...
class store {
	...
	} else if($ibforums->vars['store_guest'] && $ibforums->member['id'] == 0) {
//		$ibforums->input['code'] = "show";
	...
	if($ibforums->vars['reset_onnegitive'] && $ibforums->member['points'] < 0 && $ibforums->member['id']) {
		$DB->query("UPDATE ibf_members SET points='0' WHERE id='{$ibforums->member['id']}' LIMIT 1");
	}
...
//	$out_put .= "<br><div align='center' class='copyright'>Powered by <a href=\"http://www.subzerofx.com/shop/\" target='_blank'>IBStore</a> {$this->store_version} &copy; 2003-04 &nbsp;<a href='http://www.subzerofx.com/' target='_blank'>SubZeroFX.</a></div><br>";
	...
	$cat = $DB->query("SELECT * FROM ibf_store_category ORDER BY catid DESC");
	while($temp = $DB->fetch_row($cat)) {
	...
	$member = $DB->fetch_row();
	$DB->query("SELECT *, count(id) as stock FROM ibf_store_inventory AS i
	...
	if($DB->get_num_rows() == 0) $this->temp_output = $this->html->noinventory();
	while($user_inventory = $DB->fetch_row()) {
	...
	if(!$ibforums->vars['allow_resell'] && $ibforums->input['type'] == 'resell') {
	    $this->error("using_off_function");
	} else if(!$ibforums->vars['allow_deleting'] && $ibforums->input['type'] == 'delete') {
	    $this->error("using_off_function");
	}
	...
	$DB->query("SELECT price_payed, item_id FROM ibf_store_inventory WHERE i_id='{$ibforums->input['itemid']}' AND owner_id='{$ibforums->member['id']}' LIMIT 1");
	...
	$DB->query("UPDATE ibf_members SET points='{$ibforums->member['points']}' WHERE id='{$ibforums->member['id']}' LIMIT 1");
	$DB->query("UPDATE ibf_store_shopstock SET stock=stock+1 WHERE id='{$IN['itemid']}' LIMIT 1");

	//vot: get item name added (6 lines below)
	$DB->query("SELECT item_name FROM ibf_store_shopstock
	...
	// vot: log added
	$lib->write_log($ibforums->vars['auto_pm_from'],
			'Shop',
			$ibforums->member['id'],
			$ibforums->member['name'],
			round(($ibforums->vars['resell_percentage']/100)*$price['price_payed']),
			"Товар '".$item['item_name']."' возвращён в магазин",
                        $ibforums->input['user_reson'],
			"resell");
...
//	}
	...
	if ( !$ibforums->input['user_reson'] ) $this->error("blank_reason");

	$member['points'] = $ibforums->input['fine_amount'];

	$DB->query("UPDATE ibf_members SET points=points+'{$member['points']}', fined=fined+'{$member['points']}' WHERE id='{$member['id']}' LIMIT 1");
	//vot: log added
...
//                        "Поощрён участник ".$member['name']." на ".$member['points']." ".$ibforums->vars['currency_name'],$ibforums->input['user_reson'],"fine");
	$lib->write_log($ibforums->member['id'],
			...
			$ibforums->input['username'],
                        $ibforums->input['fine_amount'],
                        "Поощрён '".$ibforums->input['username']."' на ".$ibforums->input['fine_amount'].' '.$ibforums->vars['currency_name'],
                        $ibforums->input['user_reson'],
			"fine");
...
//	$ibforums->member['points'] = $std->do_number_format($ibforums->member['points']);
	...
	$this->output .= $this->html->fine_header();

	// get all the charges for the user
	$rows = $DB->query(	"SELECT * FROM ibf_store_logs ".
				"WHERE type='fine' ".
				"AND toid=".$member['id']." ORDER BY time DESC");

	if($DB->get_num_rows() == 0) $this->temp_output = $this->html->nocharges();

	while($charges = $DB->fetch_row($rows)) {
...
//	    $charges['sum'     ] = str_replace( '&lt;', '<', $charges['sum'  ] );
	...
	$DB->query("SELECT *, count(i_id) as stock FROM ibf_store_inventory AS i
	...
	$DB->query("UPDATE ibf_members SET points='{$receiver['points']}' WHERE id='{$receiver['id']}' LIMIT 1");
	$DB->query("UPDATE ibf_members SET points='{$ibforums->member['points']}' WHERE id='{$ibforums->member['id']}' LIMIT 1");
	...
	$lib->write_log($ibforums->member['id'],
			...
			"Передано ".$ibforums->input['amount']." ".$ibforums->vars['currency_name']." участнику '".$ibforums->input['username']."'",
...
//	$lib->write_log("Послано ".$ibforums->input['amount']." участнику ".$ibforums->input['username'],"donate_m");
	 ...
	 $this->nav = array( $ibforums->lang['item_nav'] );
         ...
         if (!$ibforums->member['id']) {
           ...
           if(empty($ibforums->input['submit'])) {
          	$send_item = $DB->query("SELECT * FROM ibf_store_inventory AS i
        	...
        	$DB->query("SELECT * FROM ibf_store_inventory AS i
        	...
        	$DB->query("UPDATE ibf_store_inventory SET owner_id='{$sendto['id']}' WHERE i_id='{$send['i_id']}' LIMIT 1");
        	...
        	$lib->write_log($ibforums->member['id'],
        			...
        			$ibforums->input['username'],
        			$send['price_payed'],
        			"Передан товар '".$send['item_name']."' участнику ".$ibforums->input['username'],
        			$ibforums->input['message'],
        			"donate_i");
	...
	$DB->query("SELECT 1 FROM ibf_store_shopstock
		    WHERE stock>0 AND id>'0' ".$extra);

	$total_items = $DB->get_num_rows();
	$limit = $ibforums->vars['pages_peritems'] ? $ibforums->vars['pages_peritems'] : 25;
	$current_page = $ibforums->input['page'] ? $ibforums->input['page'] : 0;
	if($ibforums->input['page'] <0) $current_page = 0; // vot
	$limit_extra = "LIMIT $current_page,$limit";
	$info['next'] = $current_page + $limit;
	$info['last'] = $current_page - $limit;

	$list = $DB->query("SELECT * FROM ibf_store_shopstock
			    WHERE
				id>'0' ".$extra." AND
				stock>0 AND
				avalible='0'
			    ORDER BY item_name
			    DESC ".$limit_extra);
	$returned_rows = $DB->get_num_rows($list);

	while($item = $DB->fetch_row($list)) {
	    if($item['soldout_time'] > 0 && $item['stock'] < 1) {
		$restock = $item['soldout_time'] + $item['restock_wait'];
		if($restock < time()) {
		    $item['stock'] = $item['restock_amount'];
		    $query = $DB->query("UPDATE ibf_store_shopstock SET stock='{$item['stock']}', soldout_time='0' WHERE id='{$item['id']}' LIMIT 1");
		    $DB->free_result($query);
		}
	    }

	    if ($ibforums->member['id']) $item['item_buyitem'] = "<a href='{$ibforums->base_url}act=store&code=buyitem&itemid={$item['id']}'>{$ibforums->lang['buyitem']}</a>";
	    $item['item_buyitem'] = $item['stock'] ? $item['item_buyitem'] : $ibforums->lang['soldout'];
	...
	$DB->query("SELECT * FROM ibf_store_shopstock WHERE id='{$item_id}' LIMIT 1");
	$item = $DB->fetch_row();
	if($item['stock'] <= 0) {
	...
	if($item['item_limit'] > 0) {
	    $DB->query("SELECT * FROM ibf_store_inventory WHERE owner_id='{$ibforums->member['id']}' AND item_id='{$item_id}'");
	    $ibforums->input['amount'] = $ibforums->input['amount'] ? $ibforums->input['amount'] : 1;
	    if($DB->get_num_rows()+$ibforums->input['amount'] > $item['item_limit']) {
	...
	$DB->query("UPDATE ibf_members SET points='{$ibforums->member['points']}' WHERE id='{$ibforums->member['id']}' LIMIT 1");
	$DB->query("UPDATE ibf_store_shopstock SET stock='{$item['stock']}' ".$extra." WHERE id='{$item['id']}' LIMIT 1");


	if($ibforums->vars['mass_buyon'] && $amount > 1) {
	    while($i < $amount) {
		$DB->query("INSERT INTO ibf_store_inventory (i_id,owner_id,item_id,price_payed) VALUES('','{$ibforums->member['id']}','{$item_id}','{$item['sell_price']}')");
		$i++;
	    }
	    ...
	    $lib->write_log(	$ibforums->member['id'],
				...
				'Shop',
				round($item['sell_price'] - (($ibforums->member['g_discount']/100)*$item['sell_price'])),
				"Куплен товар '".$item['item_name']."x".$i."' за ".round($item['sell_price'] - (($ibforums->member['g_discount']/100)*$item['sell_price'])).' '.$ibforums->vars['currency_name'],
				$ibforums->input['user_reson'],
				"bought");
...
// vot		    }
	...
	$inventory = $DB->query("SELECT *, count(id) as stock FROM ibf_store_inventory AS i
				INNER JOIN ibf_store_shopstock as s ON i.item_id = s.id WHERE i.owner_id={$ibforums->member['id']}
				GROUP BY i.item_id ORDER BY item_name DESC");
	if($DB->get_num_rows() == 0) $this->temp_output = $this->html->noinventory();
	while($user_inventory = $DB->fetch_row($inventory)) {
	...
	$DB->query("UPDATE ibf_members SET points='{$ibforums->input['points']}' WHERE LOWER(name)='{$ibforums->input['username']}' LIMIT 1");
//	$lib->add_reason($ibforums->member['id'],$ibforums->member['name'],$member['id'],$ibforums->input['username'],$ibforums->input['points'],"Корректировка остатка на счете ".$ibforums->input['username'].": ".$ibforums->input['points']." ".$ibforums->vars['currency_name'],$ibforums->input['user_reson'],"edit");
	$lib->write_log($ibforums->member['id'],
			...
			$member[id],
			$ibforums->input['username'],
			$ibforums->input['points'],
			"Корректировка счета '".$ibforums->input['username']."': ".$ibforums->input['points']." ".$ibforums->vars['currency_name'],
			$ibforums->input['user_reson'],
			"edit");
	...
	$i = 0;
	$ii = 0;
	$inv_edit = $DB->query("SELECT i_id FROM ibf_store_inventory WHERE owner_id='{$ibforums->input['userid']}' ORDER BY i_id DESC");
	while($edit = $DB->fetch_row($inv_edit)) {
	    if($ibforums->input['delete_'.$edit['i_id']] == 1) {
		$temp = $DB->query("DELETE FROM ibf_store_inventory WHERE i_id='{$edit['i_id']}' AND owner_id='{$ibforums->input['userid']}' LIMIT 1");
		$i++;
	    } else {
		if($ibforums->input['price_'.$edit['i_id']] != $ibforums->input['original_'.$edit['i_id']]) {
		    $price = $ibforums->input['price_'.$edit['i_id']];
		    $temp = $DB->query("UPDATE ibf_store_inventory SET price_payed='{$price}' WHERE i_id='{$edit['i_id']}' AND owner_id='{$ibforums->input['userid']}' LIMIT 1");
		}
	    }
	}
	if($i > 0 && $ii == 0) {
...
//	$lib->add_reason($ibforums->member['id'],$ibforums->member['name'],$ibforums->input['userid'],'',0,$msg,$ibforums->input['reson'],'inventory');
	$lib->write_log($ibforums->member['id'],
			$ibforums->member['name'],
			$ibforums->input['userid'],
			$ibforums->input['username'],
			0,
			$msg,
			$ibforums->input['reson'],
			"inventory");
...
//	$html .= "<br/><div align='center' class='copyright'>Powered by <a href=\"http://www.subzerofx.com/shop/\" target='_blank'>IBStore</a> {$this->store_version} &copy; 2003-04 &nbsp;<a href='http://www.subzerofx.com/' target='_blank'>SubZeroFX.</a></div><br>";
	...
	function quiz_check($quiz_id,$let_play) {
		global $DB,$ibforums;
		$DB->query("SELECT * FROM ibf_store_quizwinners WHERE quiz_id='{$quiz_id}'");
		$played = $DB->get_num_rows();
		if($played >= $let_play && $let_play != 0) {
			$DB->query("UPDATE ibf_store_quizinfo SET quiz_status='CLOSED' WHERE q_id='{$quiz_id}' LIMIT 1");
	...
	function quiz() {
		...
		$list_quiz = $DB->query("SELECT * FROM ibf_store_quizinfo ".$extra);
		while($quiz = $DB->fetch_row($list_quiz)) {
			...
			if($quiz['quiz_status'] != 'CLOSED' && $quiz['status_days'] <= 0) {
				$update = $DB->query("UPDATE ibf_store_quizinfo SET quiz_status='CLOSED' WHERE q_id='{$quiz['q_id']}' LIMIT 1");
			}
	...
	function take_quiz() {
		...
		$DB->query("SELECT 1 FROM ibf_store_quizwinners WHERE quiz_id='{$ibforums->input['quiz_id']}' AND memberid='{$ibforums->member['id']}' LIMIT 1");
		if($DB->get_num_rows() > 0) {
		...
		$DB->query("SELECT quiz_status,timeout FROM ibf_store_quizinfo WHERE q_id='{$ibforums->input['quiz_id']}' ".$extra." LIMIT 1");
		if($DB->get_num_rows() == 0 ) {
			$this->error("quiz_is_closed");
		}
		$settings = $DB->fetch_row();
		if($settings['quiz_status'] == 'CLOSED') $this->error("quiz_is_closed");
		$settings['time'] = time();

		$this->output	.=	$this->html->quiz_q_a_header($settings);
		$DB->query("SELECT * FROM ibf_store_quizs WHERE quiz_id='{$ibforums->input['quiz_id']}'");
		if($DB->get_num_rows() <= 0) $this->error("couldnotloadanswer");
		while($quiz = $DB->fetch_row()) {
			if($quiz['type'] == 'single' || $quiz['type'] == 'multiq') {
				if(!$quiz['anwser'] || !$quiz['question']) continue;
				...
				foreach($answers as $answer) {
					if(!preg_match("#{answer(1|2|3|4)_.+?:(.+)}#is",$answer,$match)) continue;
	...
	function do_take_quiz() {
		global $ibforums, $DB, $std, $print,$lib;
		$DB->query("SELECT 1 FROM ibf_store_quizwinners WHERE quiz_id='{$ibforums->input['quizid']}' AND memberid='{$ibforums->member['id']}' LIMIT 1");
		if($DB->get_num_rows() > 0) {
			$this->error("quiz_playedalready");
		}
		$DB->query("SELECT * FROM ibf_store_quizinfo WHERE q_id='{$ibforums->input['quizid']}' LIMIT 1");
		$quiz = $DB->fetch_row();
		if($quiz['quiz_status'] == 'CLOSED') {
			$this->error("quiz_is_closed");
		}
		foreach($_POST as $field => $value) {
			if(!preg_match("#uanswer_(.+)#is",$field,$match)) continue;
			$temp_answer[$match['1']] = $value;
		}
		$correct = 0;
		$total_questions = 0;
		$DB->query("SELECT * FROM ibf_store_quizs WHERE quiz_id='{$ibforums->input['quizid']}' ORDER BY mid");
		while($check = $DB->fetch_row()) {
			...
			} else if($check['type'] == 'dropdown' || $check['type'] == 'multiq') {
				$answers = explode("||",$correct_answer);
				foreach($answers as $answer) {
					if(!preg_match("#{answer.+?_(.+?):(.+)}#is",$answer,$match)) continue;
		...
		$DB->query("INSERT INTO ibf_store_quizwinners VALUES('{$ibforums->input['quizid']}','{$ibforums->member['id']}','{$correct}','{$time_took}')");
		if($correct_percent >= $quiz['percent_needed']) {
			if($quiz['quiz_items']) {
				$quiz_items = explode("|",$quiz['quiz_items']);
				foreach($quiz_items as $items) {
					if(!preg_match("#(.+)=(.+)#is",$items,$match)) continue;
					$DB->query("INSERT INTO ibf_store_inventory(i_id,owner_id,item_id,price_payed) VALUES('','{$ibforums->member['id']}','{$match[1]}','{$quiz['amount_won']}')");				
			...
			} else $extra_location = "code=quiz";
			$ibforums->member['points'] += $quiz['amount_won'];
			$DB->query("UPDATE ibf_members SET points='{$ibforums->member['points']}' WHERE id='{$ibforums->member['id']}' LIMIT 1");
			$msg = $ibforums->lang['quiz_winner'].$extra;

		} else $msg = $ibforums->lang['quiz_notenoughtcorrect'];
	...
	function misc_stats() {
		...
		$DB->query("SELECT points,deposited FROM ibf_members WHERE points>'0' OR deposited>'0'");
		while($money = $DB->fetch_row()) {
			$total_money += $money['points'];
			$total_bank  += $money['deposited'];
		}
		$DB->query("SELECT sell_price,stock FROM ibf_store_shopstock WHERE stock>'0' AND avalible='0'");
		while($items = $DB->fetch_row()) {
			$total_item += $items['sell_price'];
			$total_stock += $items['stock'];
		}
		$stats = array('money' => $std->do_number_format($total_money),'bank' => $std->do_number_format($total_bank), 'item' => $std->do_number_format($total_item), 'stock' => $std->do_number_format($total_stock), 'total' => $std->do_number_format($total_money+$total_bank));
	...
	function stats() {
		...
		if(isset($ibforums->input['type'])) {
			...
			$DB->query("SELECT id,name,points,deposited FROM ibf_members WHERE id>'0' AND points>'0' AND deposited>'0' ORDER BY points+deposited DESC LIMIT ".$ibforums->vars['richest_showamount']);
			while($temp = $DB->fetch_row()) {
	...
	function getmid($username,$addon="",$extra_a="") {
		...
		$extra .= $extra_a;
		$DB->query("SELECT ".$tables." FROM ibf_members WHERE ".$extra." LIMIT 1");
		if($DB->get_num_rows() == 0) {
	...
	function bank() {
		...
		if(!$ibforums->vars['bank_on']) $this->error("bankoffline");
		...
		if($ibforums->member['auto_collect'] == 1) {
		...
		} else if($cancollect >= 1 && $ibforums->member['auto_collect'] == 0) {
			$can_collect = true;
			$info['disabled'] = "";
		} else if($cancollect >= 1 && $ibforums->member['auto_collect'] == 1) {
			$auto_interest = $interest_points * $cancollect;
			$ibforums->member['deposited'] = round($ibforums->member['deposited'] + $auto_interest);
			$DB->query("UPDATE ibf_members SET last_collect='{$time}', deposited='{$ibforums->member['deposited']}' WHERE id='{$ibforums->member['id']}' LIMIT 1");
			...
			$lib->write_log($ibforums->vars['auto_pm_from'],
					...
					$ibforums->member['name'],
					$auto_interest,
					"Начислен процент: ".$auto_interest.' '.$ibforums->vars['currency_name']." Всего денег в банке: ".$ibforums->member['deposited'].' '.$ibforums->vars['currency_name'],
					"",
					"auto_collect");
		}
	...
	function do_bank() {
		...
		$cancollect = $time - $ibforums->member['last_collect'];
		$ibforums->vars['days_interestcollected'] = $ibforums->vars['days_interestcollected'] ? $ibforums->vars['days_interestcollected'] : 86400;
		$cancollect = ceil($cancollect / $ibforums->vars['days_interestcollected'])-1;
		if($cancollect >= 1) $last_collect = ",last_collect='{$time}'";
		if($ibforums->input['type'] == 'collect') {
			if($cancollect <= 0) {
				return $this->bank();
			}
			$ibforums->member['deposited'] = round($ibforums->member['deposited'] + $interest_points);
			$DB->query("UPDATE ibf_members SET deposited='{$ibforums->member['deposited']}',last_collect='{$time}' WHERE id='{$ibforums->member['id']}' LIMIT 1");

			$lib->write_log($ibforums->vars['auto_pm_from'],
					'Shop',
					$ibforums->member['id'],
					$ibforums->member['name'],
					round($interest_points),
					"Начислен процент: ".$std->do_number_format(round($interest_points)).' '.$ibforums->vars['currency_name']." Всего денег в банке: ".$std->do_number_format(round($ibforums->member['deposited'])).' '.$ibforums->vars['currency_name'],
					"",
					"collect");
		...
		if($ibforums->input['type'] == 'deposit') { 
			...
			$DB->query("UPDATE ibf_members SET deposited='{$ibforums->member['deposited']}', points='{$ibforums->member['points']}'".$last_collect." WHERE id='{$ibforums->member['id']}' LIMIT 1");
			$lib->redirect("deposited_in","act=store&code=bank");			
		} else if($ibforums->input['type'] == 'withdraw') {

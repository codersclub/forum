...
class calendar
{
	...
	function edit()
	{
		...
		$ibforums->lang['the_max_length'] = $ibforums->vars['max_post_length']
			? $ibforums->vars['max_post_length'] * 1024
			: 2140000;
		...
		$recur_menu .= $event['repeat_unit'] == 'w'
			? "<option value='w' selected='selected'>{$ibforums->lang['fv_days']}</option>"
			: "<option value='w'>{$ibforums->lang['fv_days']}</option>";

		$recur_menu .= $event['repeat_unit'] == 'm'
			? "<option value='m' selected='selected'>{$ibforums->lang['fv_months']}</option>"
			: "<option value='m'>{$ibforums->lang['fv_months']}</option>";

		$recur_menu .= $event['repeat_unit'] == 'y'
		...
		$this->output .= View::Make("calendar.calendar_choose_date", [
		...
		$this->output .= View::Make("calendar.calendar_event_type", ['pub_select' => $public,'priv_select' => $private]);
		...
		$this->output = str_replace(
			'<!--IBF.EMO-->',
			View::Make("post.get_box_enableemo", [
		...
		while ($elmo = $stmt->fetch())
		{
			...
			/*
						...
						$smilies .= "<td><a href=\"javascript:emoticon('".$elmo['typed']."')\">{$smile}</a>nbsp;</td>\n";
			*/
	...
	function do_edit($del = 0)
	{
		...
		} else
		{
			...
			$ibforums->vars['max_post_length'] = $ibforums->vars['max_post_length']
				? $ibforums->vars['max_post_length']
				: 2140000;
			...
			$allow_emoticons = $ibforums->input['enableemo'] == 'yes'
				? 1
				: 0;
			$private_event   = $ibforums->input['e_type'] == 'private'
				? 1
				: 0;
	...
	function show_month()
	{
		...
		$this->output .= View::Make("calendar.cal_main_content", ['month' => $this->month_words[$this->chosen_month - 1],'year' => $this->chosen_year,'prev' => $prev_month,'next' => $next_month]);
		...
		for ($c = 0; $c < 42; $c++)
		{
			...
			} else
			{
					...
					foreach ($range as $r => $d)
					{
						...
						if ($day_array['year'] == $d['year'])
						{
							...
							//------------------------------
								...
								if ($day_array['mon'] == $d['end_month'] AND ($day_array['mon'] > $d['month'] AND $day_array['mday'] <= $d['end_day']))
								{
									$s = 1;
								...
								{
									if (($day_array['mon'] == $d['end_month']) AND ($day_array['mday'] <= $d['end_day'] AND $day_array['mday'] >= $d['day']))
									{
										$s = 1;
									}
								}
						...
						if ($s == 1)
						{
							$this_day_events .= View::Make("calendar.cal_events_wrap_range", ['link' => 'code=showevent&amp;eventid=' . $d['eventid'],'text' => $d['title'],'ft' => $d['ft'],'bg' => $d['bg']]);
					...
					foreach ($recurring as $rc)
					{
						...
						if ($day_array['year'] > $rc['end_year'])
						{
							...
							if ($day_array['year'] == $rc['end_year'])
							{
								...
								{
									if (($day_array['mon'] == $rc['end_month']) AND ($day_array['mday'] > $rc['end_day']))
									{
										continue;
									}
								}
						...
						if ($day_array['year'] < $rc['year'])
						{
							...
							if ($day_array['year'] == $rc['year'])
							{
								...
								{
									if (($day_array['mon'] == $rc['month']) AND ($day_array['mday'] < $rc['mday']))
									{
										continue;
									}
								}
						...
						if ($sh == 1)
						{
							$this_day_events .= View::Make("calendar.cal_events_wrap_recurring", ['link' => 'code=showevent&amp;eventid=' . $rc['eventid'],'text' => $rc['title']]);
						}
					}
				}
				...
				if (isset($events[$day_array['mday']]) and is_array($events[$day_array['mday']]))
				{
					if (count($events[$day_array['mday']]) > 0)
					{
						foreach ($events[$day_array['mday']] as $idx => $data)
						{
							...
							$this_day_events .= View::Make("calendar.cal_events_wrap", ['link' => 'code=showevent&amp;eventid=' . $data['eventid'],'text' => $data['title']]);
						}
					}
				}
				...
				{
					$this_day_events .= View::Make("calendar.cal_events_wrap", ['link' => 'code=birthdays&amp;y=' . $this->chosen_year . "&amp;m=" . $this->chosen_month . "&amp;d=" . $day_array['mday'],'text' => sprintf($ibforums->lang['entry_birthdays'], $birthdays[$day_array['mday']])]);
				...
				if ($this_day_events != "")
				{
					...
					$this_day_events = View::Make("calendar.cal_events_start") . $this_day_events . View::Make("calendar.cal_events_end");
				}

				if (($day_array['mday'] == $this->now_date['mday']) and ($this->now_date['mon'] == $day_array['mon']) and ($this->now_date['year'] == $day_array['year']))
				{
					$cal_output .= View::Make("calendar.cal_date_cell_today", ['month_day' => $cal_date,'events' => $this_day_events]);
				} else
				{
					$cal_output .= View::Make("calendar.cal_date_cell", ['month_day' => $cal_date,'events' => $this_day_events]);
				}
	...
	function new_event()
	{
		...
		$this->output .= View::Make("calendar.calendar_choose_date", ['days' => $this->get_day_dropdown(),'months' => $this->get_month_dropdown(),'years' => $this->get_year_dropdown(),'end_days' => $this->get_day_dropdown(),'end_months' => $this->get_month_dropdown(),'end_years' => $this->get_year_dropdown(),'recur_days' => $this->get_day_dropdown("1"), 'recur_unit' => "<option value='w'>{$ibforums->lang['fv_days']}</option><option value='m'>{$ibforums->lang['fv_months']}</option><option value='y'>{$ibforums->lang['fv_years']}</option>", 'div' => array(
		                                                                                                                                                                                                                                                                                                                                                                                                                                          'range_on'  => 'none',
		                                                                                                                                                                                                                                                                                                                                                                                                                                          'range_off' => 'show',
		                                                                                                                                                                                                                                                                                                                                                                                                                                          'recur_on'  => 'none',
		                                                                                                                                                                                                                                                                                                                                                                                                                                          'recur_off' => 'show'
		...
		$this->output = str_replace('<!--IBF.EMO-->', View::Make("post.get_box_enableemo", ['checked' => 'checked="checked"']), $this->output);
	...
	function add_new_event()
	{
		...
		$txt = preg_replace("#(^|\s)((http|https|news|ftp)://\w+[^\s\[\]]+)#ie", "\$this->parser->regex_build_url_auto_parser(array('html' => '\\2', 'show' => '\\2', 'st' => '\\1'))", $txt);
		$txt = preg_replace("#\[url\](\S+?)\[/url\]#ie", "\$this->parser->regex_build_url_auto_parser(array('html' => '\\1', 'show' => '\\1'))", $txt);
		$txt = preg_replace("#\[url\s*=\s*\&quot\;\s*(\S+?)\s*\&quot\;\s*\](.*?)\[\/url\]#ie", "\$this->parser->regex_build_url_auto_parser(array('html' => '\\1', 'show' => '\\2'))", $txt);
		$txt = preg_replace("#\[url\s*=\s*(\S+?)\s*\](.*?)\[\/url\]#ie", "\$this->parser->regex_build_url_auto_parser(array('html' => '\\1', 'show' => '\\2'))", $txt);

		$ibforums->input['Post'] = $txt;
		...
		$ibforums->db->insertRow("ibf_calendar_events", $data);
	...
	function show_event()
	{
		...
		$this->output .= View::Make("calendar.cal_page_events_start") . $this->make_event_html($event) . View::Make("calendar.cal_page_events_end");
	...
	function make_event_html($event)
	{
		...
		$event['event_text'] = $this->parser->prepare(array(
		...
		return View::Make("calendar.cal_show_event", ['event' => $event,'member' => $member,'event_type' => $event_type,'edit_button' => $edit_button,'type' => $type,'date_ends' => $de]);
	...
	function show_birthdays()
	{
		...
		$this->output .= View::Make("calendar.cal_page_events_start") . $this->make_birthday_html($month, $day) . View::Make("calendar.cal_page_events_end");
	...
	function make_birthday_html($month, $day, $switch = 0)
	{
		...
		} else
		{
			...
			while ($r = $stmt->fetch())
			{
				...
				$output .= View::Make("calendar.cal_birthday_entry", ['uid' => $r['id'],'uname' => $r['name'],'age' => $age]);
			}
	...
	function get_year_dropdown($year = "")
	{
		...
		$ibforums->vars['start_year'] = (isset($ibforums->vars['start_year']))
			? $ibforums->vars['start_year']
			: 2001;
		$ibforums->vars['year_limit'] = (isset($ibforums->vars['year_limit']))
			? $ibforums->vars['year_limit']
			: 5;

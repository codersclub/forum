...
*/
	...
	function show_month() {
		...
		while ($user = $DB->fetch_row()) $birthdays[ $user['bday_day'] ]++;
	}
	...
	//-----------------------------------------
        ...
        $DB->query("SELECT * FROM ibf_calendar_events WHERE event_repeat <> 1 AND month={$this->chosen_month} AND year={$this->chosen_year}
...
//			$smilies .= "<td><a href={$out_delim}javascript:emoticon($in_delim".$elmo['typed']."$in_delim){$out_delim}><img src=\"".$ibforums->vars['EMOTICONS_URL']."/".$elmo['image']."\" alt='smilie' border='0'></a>&nbsp;</td>\n";
    ...
    function make_event_html($event)
    {
        ...
        $event['event_text'] = $this->parser->prepare( array( 'TEXT'    => $event['event_text'],

...
class ad_settings {
                ...
                switch($IN['code'])
                {
                        ...
                        case 'dosettings':
                                $this->save_config( array( 'csite_on','csite_article_forum','csite_article_max','csite_article_recent_on','csite_article_recent_max','csite_article_chars','csite_discuss_max','csite_discuss_on',
                                                                                    ...
                                                                                    'csite_cms_path', 'csite_max_upload_files', 'cms_uploads_size_limit', 'csite_cms_url', 'cms_uploads_icon_limit', 'csite_show_full_article', 'csite_show_full_upload', 'csite_news_cat_id',
                                                                                    'csite_reg_cat_id','csite_allowed_ext', 'csite_use_quick_comments', 'dsite_max_rate_view', 'dsite_default_group_id', 'dsite_use_mod_rewrite'
                                                                                     ) );
                                break;
        ...
        function settings()
        {
                ...
                //*****************************************************************************************************//
                //                D-Site global configuration settings (c) Anton [23/05/05]                            //
                //*****************************************************************************************************//

                $ADMIN->html .= $SKIN->add_td_basic("<span style='vertical-align:top'><b>Настройки CMS/Uploads</b></span>", "center");


                $ADMIN->html .= $SKIN->add_td_row( array( "<b>Путь к директории CMS:</b><br>Укажите абсолютный путь к директории CMS <br />для хранения загружаемых пользователями файлов." ,
                                                                                  $SKIN->form_input( "csite_cms_path", $INFO['csite_cms_path'] )
                                                                 )      );

                $ADMIN->html .= $SKIN->add_td_row( array( "<b>URL к директории CMS:</b><br>Укажите URL к директории CMS <br />для хранения загружаемых пользователями файлов." ,
                                                                                  $SKIN->form_input( "csite_cms_url", $INFO['csite_cms_url'] )
                                                                 )      );

                $ADMIN->html .= $SKIN->add_td_row( array( "<b>Разрешенные к загрузке файлы:</b><br>Укажите расширения файлов, используя <br />в качестве разделителя символ <b>|</b>." ,
                                                                                  $SKIN->form_input( "csite_allowed_ext", $INFO['csite_allowed_ext'] )
                                                                 )      );


                $ADMIN->html .= $SKIN->add_td_row( array( "<b>Количество файлов, разрешенных к прикреплению</b>" ,
                                                                                  $SKIN->form_input( "csite_max_upload_files", $INFO['csite_max_upload_files'] )
                                                                 )      );

                $ADMIN->html .= $SKIN->add_td_row( array( "<b>Максимальный размер прикрепляемого файла:</b>" ,
                                                                                  $SKIN->form_input( "cms_uploads_size_limit", $INFO['cms_uploads_size_limit'] )
                                                                 )      );

                $ADMIN->html .= $SKIN->add_td_row( array( "<b>Максимальный размер прикрепляемой иконки:</b>" ,
                                                                                  $SKIN->form_input( "cms_uploads_icon_limit", $INFO['cms_uploads_icon_limit'] )
                                                                 )      );

                $ADMIN->html .= $SKIN->add_td_row( array( "<b>Показывать статью на полный экран?</b><br>Параметр определяет, будут ли при просмотре статьи также отображаться и другие блоки сайта, такие как меню и т.п." ,
                                                                                  $SKIN->form_yes_no( "csite_show_full_article", $INFO['csite_show_full_article'] )
                                                                 )      );

                $ADMIN->html .= $SKIN->add_td_row( array( "<b>Разворачивать форму изменения статьи на полный экран?</b><br>Параметр определяет, будут ли при добавлении/изменении статьи также отображаться и другие блоки сайта, такие как меню и т.п." ,
                                                                                  $SKIN->form_yes_no( "csite_show_full_upload", $INFO['csite_show_full_upload'] )
                                                                 )      );
                $ADMIN->html .= $SKIN->add_td_row( array( "<b>Использовать ''быстрые'' коментарии?</b><br>Параметр определяет, будут ли при добавлении/изменении статьи все коментарии к статье или только их количество и ссылка на добавление нового." ,
                                                                                  $SKIN->form_yes_no( "csite_use_quick_comments", $INFO['csite_use_quick_comments'] )
                                                                 )      );

                $ADMIN->html .= $SKIN->add_td_row( array( "<b>Использовать mod_rewrite для переписывания запросов?</b>" ,
                                                                                  $SKIN->form_yes_no( "dsite_use_mod_rewrite", $INFO['dsite_use_mod_rewrite'] )
                                                                 )      );


                $ADMIN->html .= $SKIN->add_td_row( array( "<b>Максимальное количество статей, отображаемое при просмотре рейтинга?</b>" ,
                                                                                  $SKIN->form_input( "dsite_max_rate_view", $INFO['dsite_max_rate_view'] )
                                                                 )      );


                $ADMIN->html .= $SKIN->add_td_row( array( "<b>ID раздела для импорта новостей.</b><br>Укажите раздел, из которого будут выводиться новости по-уомлчанию:" ,
                                                                                  $SKIN->form_simple_input( "csite_news_cat_id", $INFO['csite_news_cat_id'] )
                                                                 )      );
                $ADMIN->html .= $SKIN->add_td_row( array( "<b>ID разделов с регистрационными формами.</b><br>Укажите разделы через запятую, из которого будут выводиться регистрационные формы:" ,
                                                                                  $SKIN->form_simple_input( "csite_reg_cat_id", $INFO['csite_reg_cat_id'] )
                                                                 )      );
        ...
        function save_config( $new )
        {
                ...
                $nav_contents = $std->txt_stripslashes($HTTP_POST_VARS['csite_nav_contents']);
                $fav_contents = str_replace( '&amp;middot;', '&middot;', $std->txt_stripslashes($HTTP_POST_VARS['csite_fav_contents']) );

...
//Jureth        $print->do_output( array( 'TITLE' => $this->page_title, 'JS' => 0, NAV => $this->nav ) );
	...
	global $ibforums, $DB, $std, $print;
		...
		{ 
			...
			$res_d = $DB->query("SELECT id, read_perms
					     FROM ibf_forums
					     WHERE
						id='".$fid."'");
...
//				    ORDER BY c.position, f.position");
		...
		if ( !$forums ) $std->Error( array( 'LEVEL' => 1,
						    'MSG' => 'no_search_forum') );

		$mid    = intval($ibforums->input['mid']);
		
		if ( !$mid ) $std->Error( array( 'LEVEL' => 1,
		...
		if ( !$posts ) $std->Error( array( 'LEVEL' => 1,
						   'MSG' => 'no_search_results' ) );
 	...
 	function get_new_posts($mine = 0)
 	{
		...
		if ( !$ibforums->member['id'] )	$std->Error(
						array(
						'LEVEL' => 1,
						'MSG' => 'no_search_results' ) );
...
// Song * club tool
		...
		if ( $count <= 0 ) $std->Error( array( 'LEVEL' => 1,
						       'MSG' => 'no_search_results' ) );
...
//		$res_d = $DB->query("DELETE FROM ibf_search_results
//				     WHERE search_date < '$t_time'");
		
		$this->unique_id = $ibforums->input['searchid'];
		
		if ( !$this->unique_id ) $std->Error( array(
				'LEVEL' => 1,
				'MSG' => 'no_search_results' ) );
...
// Song * club tool
		...
		{
			$this->nav = array( "<a href='{$this->base_url}&act=Select'>{$ibforums->lang['search_form']}</a>", $ibforums->lang['search_title'] );
		}
	}
...
// Song * club tool
	...
	function parse_entry($topic, $view_as_post=0) {
		...
		if ( $topic['decided'] ) $topic['topic_icon'] = "<{B_DECIDED}>"; // Спящий
		...
		if ( $this->modfunctions ){
			if ( (strpos(','.$ibforums->member['modforums'].',', ','.$topic['forum_id'].',') !== false) or ($ibforums->member['g_is_supmod']) ) {
				$topic['mod_checkbox']= $this->html->mod_checkbox($topic['pinned']?'pinned_topic':'row4', $topic['tid']);
			}else
				$topic['mod_checkbox']= $this->html->mod_nocheckbox($topic['pinned']?'pinned_topic':'row4');
		}
    	...
    	if ( is_array( $_POST['forums'] )  )
    	{
    		...
    		else
    		{
				...
				if ( count( $forums ) )
				{
					...
					{
						$sql_query .=
						" $boolean (id IN(".implode(",",$forums).") or
						  parent_id IN(".implode(",",$forums).") )";
					}
					else
					{
						$sql_query .=
						" $boolean id IN(".implode(",",$forums).")";
					}
				}
...
// Song * forum_list
	...
	$DB->query("SELECT f.*, sf.mid as checked
		    FROM ibf_forums f 

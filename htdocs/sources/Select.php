...
class Search
{
	...
	function show_results()
	{
		...
		if ($this->result_type == 'topics')
		{
			...
			$this->output .= View::Make("search.end", ['Data' => array(
	...
	function parse_entry($topic, $view_as_post = 0)
	{
		...
		if ($this->modfunctions)
		{
			...
			{
				$topic['mod_checkbox'] = View::Make("search.mod_checkbox", [
					'class' => $topic['pinned']
						? 'pinned_topic'
						: 'row4',
	                'tid' => $topic['tid']]
			...
			{
				$topic['mod_checkbox'] = View::Make("search.mod_nocheckbox", [
					'class' => $topic['pinned']
						? 'pinned_topic'
						: 'row4'
					]
	...
	function forums_list()
	{
		...
		$stmt = $ibforums->db->query("SELECT f.*, sf.mid as checked
		    FROM ibf_forums f

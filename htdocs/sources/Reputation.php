...
class Reputation {
    ...
    function Reputation()
    {
        ...
        if ( !$ibforums->member['id'] ) $std->Error( array( 'LEVEL' => 1, 'MSG' => 'not_registered' ) );
        ...
        if ( $ibforums->input['CODE'] != 'totals' )
	{
		...
		{
			if ( !$ibforums->input['mid'] ) $std->Error( array( LEVEL => 1, MSG => 'missing_files') );

	        	$DB->query("SELECT name FROM ibf_members WHERE id='".$ibforums->input['mid']."'");

	        	if ( !$DB->get_num_rows() ) $std->Error( array( LEVEL => 1, MSG => 'missing_files') ); else
	        	{
	        		$r = $DB->fetch_row();
	...
	if ( $ibforums->input['CODE'] == '01' OR $ibforums->input['CODE'] == '02' ) 
	{
		if ( !$ibforums->member['allow_rep'] ) $std->Error( array( LEVEL => 1, MSG => 'rep_cantchange') );
		...
		if( !$ibforums->input['t'] ) $std->Error( array( LEVEL => 1, MSG => 'missing_files') );
        	...
        	if( !$ibforums->input['p'] ) $std->Error( array( LEVEL => 1, MSG => 'missing_files') );
        	
		$DB->query("SELECT msg_date FROM ibf_reputation WHERE member_id='".$ibforums->input['mid']."' AND 
	...
	// enter reason
        if ( !$ibforums->input['process'] ) $this->show_form(intval($ibforums->input['mid'])); else
        {
		// or process ratting

		if ( !$ibforums->input['message'] ) $std->Error( array( 'LEVEL' => 1, 'MSG' => 'stf_no_msg' ) );

	        if ( $ibforums->input['CODE'] == 'totals' ) $std->Error( array( LEVEL => 1, MSG => 'missing_files') );
	...
	{
		$DB->query("UPDATE ibf_members SET {$field}='".$new."' WHERE id='".$memid."'");
	}
    }
	...
	if ( !$row = $DB->fetch_row() ) $std->Error( array( LEVEL => 1, MSG => 'missing_files') );

	$field = ( $row['inc_postcount'] ) ? "rep" : "ratting";

        $DB->query("SELECT {$field} as rep FROM ibf_members WHERE id='".$memid."'");
    ...
    global $ibforums, $DB, $std, $print;
		...
		if ( $ibforums->member['id'] != $memid )
		{
			...
			$ratting_type = ( $field == "rep" ) ? "t" : "f";
			
	        	$print->redirect_screen("", "act=rep&amp;CODE=03&amp;type={$ratting_type}&amp;mid=".$memid );

	    	} else $std->Error( array( LEVEL => 1, MSG => 'rep_self') );
    }
...
// Song * silent check correct parameters
    ...
    if ( !$ibforums->input['p'] or !$ibforums->input['mid'] or !$ibforums->input['t'] or !$ibforums->input['f'] ) return;

    $DB->query("SELECT p.author_id, p.forum_id, p.topic_id, t.title, m.name as member_name, f.name as forum_name  
             ...
             $check['topic_id'] != $ibforums->input['t'] ) 
	{
		...
		$res = $DB->query("SELECT member_id FROM ibf_moderators WHERE member_id != -1 and forum_id='".$check['forum_id']."'");
		while ( $moderator = $DB->fetch_row( $res ) )
		{
...
// Song * silent check correct parameters
			...
			case '03': //Showing stats
				...
				$ratting_type = ( $ibforums->input['type'] == "t" ) ? "1" : "0";

		    		$DB->query("SELECT COUNT(r.msg_id) as total FROM ibf_reputation r, ibf_forums f 
				...
				if ( !$ibforums->input['st'] ) $ibforums->input['st'] = 0;
				
				$links = $std->build_pagelinks(  array( 'TOTAL_POSS'  => $max['total'],
				...
				$ratting = ( $ibforums->input['type'] == "t" ) ? "rep" : "ratting";

				$DB->query("SELECT id, name, {$ratting} as rep FROM ibf_members WHERE id='".$memid."'");
...
//Jureth			$DB->query("SELECT r.*, m.name, t.title, f.read_perms 
                                $DB->query("SELECT r.*, m.name, f.name as forum, t.title, f.read_perms
				...
				while (	$i = $DB->fetch_row() ) 
				{
					...
					if ( $std->check_perms($i['read_perms']) != TRUE ) $i['title'] = "";
...
/* <--- Jureth --- */ 
			...
			case 'delete': //Deleting a rep change
				...
				$DB->query("SELECT member_id FROM ibf_reputation WHERE msg_id='".$ibforums->input['id']."' AND 
...
// Song * disable feature 
    	    			...
    	    			foreach ($max_results as $k => $v) 
				{
			    	...
			    	if ( !isset($sort_key[ $this->sort_key ]) )       $error = 1;
    	    			...
    	    			if ( $error ) $std->Error( array( LEVEL=> 5, MSG =>'incorrect_use') );

				$DB->query("SELECT COUNT(id) as total_members FROM ibf_members");
				...
				$DB->query("SELECT m.name, m.id, m.rep, m.allow_rep, m.allow_anon, COUNT(r.msg_id) AS times 
					    FROM ibf_members m 
				...
				while ( $member = $DB->fetch_row() )
				{
					...
					if (is_numeric($member['rep'])) $member['rep'] .= " ".$ibforums->lang['rep_postfix'].

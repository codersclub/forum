...
// vot: BAD Message -> move to LANG !
	...
	if ( $new_group == $ibforums->vars['member_group'] and $ibforums->member['posts'] == 500 )
	{
		...
		$DB->query("SELECT COUNT(wlog_id) as cnt FROM ibf_warn_logs WHERE wlog_mid='".$ibforums->member['id']."' and wlog_type='neg'");
...
// Song * multiple choices
		...
		$DB->query("UPDATE ibf_forums SET last_title='".addslashes($topic['title'])."',
						  last_id='".$tid."',
						  last_post='".time()."',
						  last_poster_name='Forum_Bot',
						  last_poster_id='8617',
		...
		$DB->query("UPDATE ibf_members SET disable_group=1 WHERE id='".$ibforums->member['id']."'");
	}

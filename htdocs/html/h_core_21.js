...
//#####################################
function parseCode(T,Rul,nums)
{var m=1,R='';
var breaks = nums ? '</li><li>' : '</div><div class="code_line">';
/*************/
function P(){}
var	AcResp=[P,function()/*none*/{R+=reg[j][0]+(reg[j+1]?reg[j+1][0]:'')},
			  ...
			  function()/*tag*/{
				  ...
				  var tmp =reg[j][0]+
					  HTMLescape(m[j-1]).replace(/\n/g, (reg[j+1]?reg[j+1][0]:'')+breaks +reg[j][0])+
					  (reg[j+1]?reg[j+1][0]:'');
				  R+=tmp.replace(reg[j][0]+(reg[j+1]?reg[j+1][0]:''),'');
			  },P];
/************/
	 var RL=Rul.length
 ...
 if (nums) {
	 ...
	 R='<ol type="1"><div class="code_line">'+R+'</li></ol>';
	 R=R.replace(/\n/g, '</div><div class="code_line">').replace(/<div class="code_line">\s*?<\/div>/g, '<div class="code_line">&nbsp;</div>').replace(/<div class="code_line"> /g, '<div class="code_line">&nbsp;');
 }
 R=R.replace(/  /g, ' &nbsp;');
 return	R;
}//parseCode
...
//##################################### 
function parseOne(O,L,nums)
{
 var RUL=window['h_'+L];
 O=document.getElementById(O);
 if(RUL)pasteInPre(O,parseCode(getInnerText(O),RUL,nums))

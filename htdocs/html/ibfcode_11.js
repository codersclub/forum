...
function tag_attach(num) {
	if (!doInsert('[attach=#'+num+']', '[/attach]', false)) {
		doInsert('[/attach]', '', true)
	}
	return false;
}
...
function tag_spoiler()
{
	if (spoiler_open > 0) {
	    ...
	    var enterText   = prompt(text_enter_spoiler, text_spoiler_hidden_text);
	    var openTag = '[spoiler]';
...
//alert("doInsert started");
    ...
    } else if ((is_nav && (myVersion > 4)) || is_webkit) 
    {
        ...
        obj_ta.focus();
        obj_ta. setSelectionRange(selStart + ibTag.length, selEnd + ibTag.length );
    } else
    {
        if ( isSingle ) isClose = true;

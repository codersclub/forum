window['h_mysql']=[
 ...
 [
    /^(select|insert *?into|delete|update|create *?table|create *?database|drop *?table|drop *?database|alter *?table|flush|grant|priveleges|lock *?table|unlock *?tables|join|limit|explain)\b/i,
    ['',0],
    ['<span style=\'color:blue\'><b>',3],
    ['</b></span>',0]
 ],
 [
    /^(distinct|singular|from|where|group( |\n)*?by|order( |\\n)*?by|add|drop|change|asc|desc|values|default|primary( |\n)*?key|not( |\n)*?null|in|write|read|left|right)\b/i,
    ['',0],

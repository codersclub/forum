window['h_1C-SQL']=[
 ...
 [
    /^(select|бшапюрэ|left *?join|кебне *?янедхмемхе|right *?join|опюбне *?янедхмемхе|inner *?join|бмсрпеммее *?янедхмемхе|outer *?join|бмеьмее *?янедхмемхе|join *?on|янедхмемхе *?он|join|янедхмемхе)\b/i,
    ['',0],
    ['<span style=\'color:blue\'><b>',3],
    ['</b></span>',0]
 ],
 [
    /^(distinct|singular|from|where|group( |\n)*?by|order( |\n)*?by|having|add|drop|change|asc|desc|values|default|primary( |\n)*?key|not( |\n)*?null|in|write|read|left|кебне|right|опюбне)\b/i,
    ['',0],

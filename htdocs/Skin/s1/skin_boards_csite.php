...
function CatHeader_Collapsed($info) {
global $ibforums;
return <<<EOF

<div class="tableborder"> <div class='maintitle' align='left'><a href={$ibforums->base_url}expcat={$info['id']}><{C_PLUS}></a>&nbsp;<a href="{$ibforums->base_url}c={$info['id']}">{$info['name']}</a></div></div>
...
<tr>
 {$info['tree']}
 <td class="row4"><b><a href="{$ibforums->base_url}showforum={$info['id']}">{$info['name']}</a></b> <span class='desc'>{$info['description']}</span></td>
...
return <<<EOF
 ...
 <div class="tableborder">
  <div class='maintitle' align='left'>&nbsp;<a href={$ibforums->base_url}colcat={$Data['id']}><{C_MINUS}></a>&nbsp;<a href="{$ibforums->base_url}c={$Data['id']}">{$Data['name']}</a></div>      <table width="100%" border="0" cellspacing="1" cellpadding="4">
        <tr>

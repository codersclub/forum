	  ...
	  <tr>
		...
		<td align='left' class='row1'>{$info['signature']}</td>
          ...
          <tr>
                ...
                <td align='left' class='row1'>{$info['rep']} <a href='{$ibforums->vars['board_url']}/index.php?act=rep&CODE=03&mid={$info['mid']}'>{$ibforums->lang['rep_details']}</a></td>
          </tr>
	  <!--{WARN_LEVEL}-->
	  </table>
	</td>
...
return <<<EOF
  <tr>
	<td class="row3" valign='top'><b>{$ibforums->lang['warn_level']}</b></td>
	<td align='left' class='row1'><a href="javascript:PopUp('{$ibforums->base_url}act=warn&amp;mid={$mid}&amp;CODE=view','Pager','500','450','0','1','1','1')">{$percent}</a>%:{$img}
</td>
  </tr>

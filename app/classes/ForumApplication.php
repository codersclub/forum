...
class ForumApplication extends CoreApplication
{
    ...
    protected function loadSkin()
    {
        ...
        {
            $skin = \Models\Skins::find([ 'sid' => $id ] + $extra_params);
            //			// Didn't get a row?
            if (FALSE === $skin)
            {
                ...
                $skin =  \Models\Skins::find(['sid' => Config::get('app.default_skin', 0)]);
            }
        } else
        {
        ...
        if (($this->input['setskin']) and ($this->member['id']))
        {
            ...
            $this->member['skin'] = $skin['sid'];
        }
        return \Skins\Factory::create($skin);
    }
